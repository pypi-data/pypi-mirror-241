    ...
    def delete_document_from_collection(self, collection_id, document_id):
        ...
        try:
            ...
            if doc_ref.get().exists:
                doc_ref.delete()
                LOG.info(f"Deleted {document_id} document from {collection_id} firestore collection.")
            else:
                LOG.info(f"Document {document_id} does not exist in {collection_id} firestore collection.")
        ...
        if not document_found:
            raise DocumentNotFoundError(f"Document {document_id} does not exist in {collection_id} firestore collection.")
