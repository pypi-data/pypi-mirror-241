...
class TaskDispatcher:
    ...
    def work(self):
        while True:
            free_types = self._locked_listen_free_task_types()
            if len(free_types) == 0:  # 没有空闲的worker
                time.sleep(1)
                continue
            print(
                f"[TaskDispatcher] [{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}] blocking listening task types: {free_types}")
            key, value = self.redis_client.blpop(free_types, timeout=0)
            if not key:
                continue
            ...
            task_data = self.task_client.get_task(task_id)
            if not task_data:
                print(
    ...
    def _locked_listen_free_task_types(self):
