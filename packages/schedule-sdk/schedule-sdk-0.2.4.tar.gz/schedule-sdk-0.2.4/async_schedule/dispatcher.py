...
class TaskDispatcher:
    ...
    def _async_work(self, worker_instance: AbstractWorker, op: TaskOperator, task_type: str, task_context):
        try:
            print(f"[TaskDispatcher] [{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}] task {op.task.__dict__} work start")
            # 调用 worker 的 work 方法
    ...
    def _locked_listen_free_task_queues(self):
        free_types = ["queue_" + task_type for task_type, worker in self.task_worker_map.items()
