...
def get_file_date(file: Path, method: str = "ctime") -> float:
...
def _get_file_date(method: str = "exif") -> Callable:
    ...
    -------
    function
        Returns the appropriate function for the required date/time extraction method.capitalize()$0
...
def create_target_dir(destination: str | Path = "./", quiet: bool = True):
...
def main(args: arg.ArgumentParser | None = None) -> None:
    ...
    args = parser.parse_args() if args is None else parser.parse_args(args)  # type: ignore
    arguments = vars(args)
    # Find files
    ...
    [create_target_dir(x, arguments["quiet"]) for x in target_dirs]  # pylint: disable=expression-not-assigned
    # Move files
