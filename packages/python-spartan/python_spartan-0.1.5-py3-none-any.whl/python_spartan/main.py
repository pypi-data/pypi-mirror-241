...
def deploy_config(source: str = typer.Argument(..., help="Source file path (absolute or relative)")):
    try:
        ...
        print(f"Error configuring deployment: {e}")
...
def create_sqs(
    ...
    type: str = typer.Option("", help="Type of the SQS queue (standard or fifo).", show_default=True),
    dlq: bool = typer.Option(False, help="Create a Dead Letter Queue (DLQ)."),
):
    try:
        infra_service = InfrastructureService()
        infra_service.create_sqs_queue('sqs', name, type, dlq)
        typer.echo(f"Queue '{name}' created successfully with type '{type}'.")
    except Exception as e:
...
def create_dlq(
    ...
    try:
        infra_service = InfrastructureService()
        infra_service.create_sqs_queue('dlq', name, type)
        typer.echo(f"Queue '{name}' created successfully with type '{type}'.")
    except Exception as e:
