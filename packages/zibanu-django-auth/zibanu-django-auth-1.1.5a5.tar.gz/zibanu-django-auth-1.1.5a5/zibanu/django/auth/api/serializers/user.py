...
class UserListSerializer(serializers.ModelSerializer):
    ...
    def get_level(self, instance) -> int:
        level = GroupLevelEnum.OPERATOR
        for group in instance.groups.all():
            if hasattr(group, "group_level") and group.group_level is not None:
                level = group.group_level.level if level > group.group_level.level else level
