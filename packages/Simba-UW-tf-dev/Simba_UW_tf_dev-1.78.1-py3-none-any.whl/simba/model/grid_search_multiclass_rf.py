...
class GridSearchMulticlassRandomForestClassifier(ConfigReader, TrainModelMixin):
    ...
    def perform_sampling(self, meta_dict: dict):
        ...
        if meta_dict[MetaKeys.SAVE_TRAIN_TEST_FRM_IDX.value]:
            train_data = self.frm_idx[self.frm_idx['FRAME_IDX'].isin(self.x_train.index)].set_index('VIDEO')
            test_data = self.frm_idx[self.frm_idx['FRAME_IDX'].isin(self.x_test.index)].set_index('VIDEO')
