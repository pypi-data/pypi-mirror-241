...
def writeEffcurves(effD, ueffD, tdcr, uTdcr,rad, p, kB, V):
    file = open("EfficiencyCurves/"+''.join(rad)+"/Eff_"+''.join(rad)+'_'+''.join(str(p))+'_'+str(kB)+str(V)+".txt","w")
    file.write(str(tdcr)+" "+str(uTdcr)+" "+str(effD)+" "+str(ueffD)+"\n")
    file.close()

def readEffcurves(tdcr, utdcr, rad, p, kB, V):
    file = open("EfficiencyCurves/"+''.join(rad)+"/Eff_"+''.join(rad)+'_'+''.join(str(p))+'_'+str(kB)+str(V)+".txt","w")
