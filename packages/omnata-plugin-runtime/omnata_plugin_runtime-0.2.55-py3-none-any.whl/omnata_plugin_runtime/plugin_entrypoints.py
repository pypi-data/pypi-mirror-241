    ...
    def get_secrets(
        ...
        if oauth_secret_name is not None:
            connection_secrets["access_token"] = StoredConfigurationValue(
                value=thread_local_snowflake._snowflake.get_oauth_access_token(oauth_secret_name)
            )
        if other_secrets_name is not None:
            try:
                secret_string_content = thread_local_snowflake._snowflake.get_generic_secret_string(
                    other_secrets_name
                )
