...
class unicode_with_language(str):  # pylint: disable=invalid-name
    ...
    def __repr__(self):
        return f"<{super().__repr__()!r}({self.lang})>"
...
def normalize_uri(uri, prefixes, raise_on_error=False):
    ...
    try:
        ...
        if raise_on_error:
            raise RDFRegistryError(f"prefix {prefix} not found")
        return uri
...
def guess_rdf_format(filename):
    ...
    try:
        ...
        raise ValueError(
            "can't guess RDF format for %s, please specify it using "
            "`rdf_format`" % filename
        )
...
class RDFRegistry:
    ...
    def register_prefix(self, prefix, namespace, overwrite=False):
        ...
        if not overwrite and self.prefixes.get(prefix, namespace) != namespace:
            raise RDFRegistryError(
                "prefix %r is already defined with different value %r"
    ...
    def register_etype_equivalence(self, etype, rdftype, overwrite=False):
        ...
        if not overwrite and self.etype2rdf.get(etype, rdftype) != rdftype:
            raise RDFRegistryError(
                "entity type %r is already associated to RDF type %r"
    ...
    def register_attribute_equivalence(self, etype, attr, rdftype, overwrite=False):
        ...
        if not overwrite and self.attr2rdf.get((etype, attr), rdftype) != rdftype:
            raise RDFRegistryError(
                "entity attribute %s.%r is already associated to RDF type %r"
                % (etype, attr, self.attr2rdf[(etype, attr)])
            )
        self.attr2rdf[(etype, attr)] = rdftype

    def register_relation_equivalence(
        self, subject_etype, rel, object_etype, rdftype, reverse=False
    ):
        ...
        """
        rdftype = self.normalize_uri(rdftype, raise_on_error=True)
        self.rel2rdf.setdefault((subject_etype, rel, object_etype), set()).add(
            (rdftype, reverse)
        )
...
class AbstractRDFGraph:
    ...
    def load(self, source, rdf_format=None):
        ...
        if rdf_format not in self.supported_rdf_formats:
            raise ValueError(f"RDF format {rdf_format!r} is not supported")
...
class LibRDFRDFGraph(AbstractRDFGraph):
    ...
    def __init__(self, options_string="new='yes',hash-type='memory',dir='.'"):
        ...
        self._node = node
        self._uri_node = (
            lambda x: node(uri=x) if isinstance(x, RDF.Uri) else node(uri_string=x)
        )
    ...
    def _load(self, source, rdf_format):
        ...
        assert isinstance(
            source, str
        ), f"{self.__class__.__name__}._load() expect a path or an URL as source"
        if ":/" not in source:
    ...
    def objects(self, entity_uri, predicate_uri):
        ...
        """
        qs = self._stmt(
            subject=self._uri_node(entity_uri),
...
def rdf_graph_to_entities(reg, graph, etypes, output_cls=ExtEntity):
    ...
    for etype in etypes:
        ...
        for uri in graph.uris_for_type(type_uri):
            ...
            for rtype, predicate_uri, reverse in reg.predicates_for_subject_etype(
                etype
            ):
                if reverse:
            ...
            for rtype, predicate_uri, reverse in reg.additional_object_predicates(
                etype
            ):
                if reverse:
                    uris = graph.objects(uri, predicate_uri)
