...
class ByteArrayAbstract(metaclass=ABCMeta):
    ...
    def read_pad_byte(self, _len: int) -> typing.Tuple:

        return struct.unpack(f'{self._endian}{_len}x', self.read(_len))
    ...
    def read_pascal_bytes(self, _len: int):
    ...
    def read_pascal_string(self, _len: int):
    ...
    def read_python_int(self, _len: int):

        return struct.unpack(f'{self._endian}{_len}P', self.read(_len))[0]

    def write_python_int(self, val: int):
