...
def verify(function, args, config_dir=None, add_data_type=None, custom_checker=None, logger=None):
    ...
    if logger is None:
        logger = skriba.logger.setup_logger(
            logger_name="{module}.{function}".format(module=module_name, function=function_name),
            log_to_term=True
        )
    ...
    if function_name in schema.keys():
        logger.info("Function [{}] {found} in parameter configuration files.".format(
            function_name, found=skriba.messenger.SystemMessage.green("found")
        ))

    else:
        logger.error("Function [{}] {not_found} in parameter configuration files.".format(
            function_name, found=skriba.messenger.SystemMessage.red("not found")
        ))
        raise KeyError
