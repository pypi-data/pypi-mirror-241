...
def alpha_sens_curves(
    ...
    if kind == 'line':
        texts = []
        fig, ax = plt.subplots(len(raw_sens), 2, sharey=True, gridspec_kw=dict(width_ratios=[max_alpha, 4]), figsize=figsize)
        ...
        for out, rs in enumerate(raw_sens):
            ...
            for inp, input in enumerate(input_name):
                ...
                try:
                    con_between_axes = ConnectionPatch(xyA=[alphas[len(alpha_notinf) - 1], alpha_notinf[-1]], 
                    ...
                    ax[1].plot([max_alpha+3, max_alpha+3.7], [max_alpha_mean, max_alpha_mean], color=var_color,
                                        linestyle='dotted', alpha=0.5)
                    # con_of_infinite = ConnectionPatch(xyA=[max_alpha + 3, max_alpha_mean], 
                    #                     xyB=[max_alpha + 3.5, max_alpha_mean], 
                    #                     coordsA="data", 
                    #                     coordsB="data",
                    #                     axesA=ax[0], 
                    #                     axesB=ax[1], 
                    ...
                    #                     lw=1,
                    #                     arrowstyle="->")
                    # ax[1].add_artist(con_of_infinite)
                except IndexError:
                    pass
