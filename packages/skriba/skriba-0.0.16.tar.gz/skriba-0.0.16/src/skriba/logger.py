...
class loggingFormatter(logging.Formatter):
    ...
    middle_msg = "{level:<22}".format(level="%(levelname)3s")
    execution_msg = "  {name:<25}  [ {filename:<25} ]: {exec_info:<30}: ".format(
        name="%(name)s",
        filename=skriba.messenger.SystemMessage.grey("%(filename)s"),
        exec_info=skriba.messenger.SystemMessage.blue_dim("%(module)s.%(funcName)s:%(lineno)d"))
    message = "%(message)s"
