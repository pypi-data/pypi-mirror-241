...
class loggingFormatter(logging.Formatter):
    ...
    execution_msg = "{name} [ {filename} ]: {exec_info}: ".format(
        name="%(name)-10s",
        filename=skriba.messenger.SystemMessage.grey("%(filename)-21s"),
        exec_info=skriba.messenger.SystemMessage.blue_dim("%(callchain)-30s"))
...
def get_logger(logger_name=None):
    ...
    module = inspect.stack()[0].filename
    lineno = inspect.stack()[0].lineno

    extra = {'call_chain': f"{module}.{function}:{lineno}"}
