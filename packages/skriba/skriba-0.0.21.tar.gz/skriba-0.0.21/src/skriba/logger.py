...
class loggingFormatter(logging.Formatter):
    start_msg = "[ {time} ] ".format(time=skriba.messenger.SystemMessage.magenta_dim("%(asctime)2s"))
    middle_msg = "{level}".format(level="%(levelname)-6s")
    execution_msg = " --- {name} [ {filename} ]: {exec_info}: ".format(
        name="%(name)-12s",
        filename=skriba.messenger.SystemMessage.grey("%(filename)-18s"),
        exec_info=skriba.messenger.SystemMessage.blue_dim("%(module)s.%(funcName)s:%(lineno)d".ljust(50, "*")))
    message = "%(message)s"
