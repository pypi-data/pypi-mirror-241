...
class loggingFormatter(logging.Formatter):
    ...
    execution_msg = "  %(name)2s  [ %(filename)s ]: {exec_info}: ".format(
        exec_info=skriba.messenger.SystemMessage.blue("%(module)s.%(funcName)s:%(lineno)2d"))
    message = "%(message)s"

    FORMATS = {
        logging.DEBUG: start_msg + skriba.messenger.SystemMessage.green(middle_msg) + execution_msg + message,
        logging.INFO: start_msg + skriba.messenger.SystemMessage.blue_dim(middle_msg) + execution_msg + message,
