...
class yc_serial():
    ...
    def receivemsg(self):
        while True:
            ...
            if size:
                self.recdata = self.port1.readline().decode(
...
class LOG(logging.Logger):
    def __init__(self,
        ...
        fmt = logging.Formatter(
            "%(asctime)s %(name)s-line:%(lineno)d-%(levelname)s:     %(message)s", "%H:%M:%S")
...
def xw_toExcel(data, excelname,sheet_name="Sheet"):  # xlsxwriter库储存数据到excel
    try:
        workbook = openpyxl.load_workbook(excelname)  # 打开工作簿
        sheet_names = workbook.sheetnames
        if sheet_name not in sheet_names:
            workbook.create_sheet(sheet_name, len(sheet_names))
        worksheet1 = workbook[sheet_name]
        # worksheet1 = workbook.active  # 创建子表
        # worksheet1 = workbook.create_sheet(sheet_name,0)
...
def xw_columnToExcel(data, excelname, sheet_name="Sheet"):  # xlsxwriter库储存数据到excel
    try:
        workbook = openpyxl.load_workbook(excelname)  # 打开工作簿
        sheet_names = workbook.sheetnames
        if sheet_name not in sheet_names:
    ...
    while True:
        try:
            ...
            print(a)
            os.system("pause")
def xr_columnToExcel(excelname,row_num, column_num,len,sheet_name="Sheet1",):  # xlsxwriter库储存数据到excel
    buff = []
    workbook = openpyxl.load_workbook(excelname)  # 打开工作簿
    worksheet1 = workbook[sheet_name]
    for i in range(len):
        a1_cell = (worksheet1.cell(row=row_num+i,column=column_num)).value#省份
        buff.append(a1_cell)
    workbook.close()
    return buff
def xr_rowToExcel(excelname,row_num, column_num,len,sheet_name="Sheet1",):  # xlsxwriter库储存数据到excel
    buff = []
    workbook = openpyxl.load_workbook(excelname)  # 打开工作簿
    worksheet1 = workbook[sheet_name]
    for i in range(len):
        a1_cell = (worksheet1.cell(row=row_num,column=column_num+i)).value#省份
        buff.append(a1_cell)
    workbook.close()
    return buff
