...
def apply_config(config, db, write=False, update=False, original=None,
    ...
    collisions = {}
    all_aliases = {a.lower() for a in db.get_device_alias_list("*")}
    alias_collisions = []
    for dev, (srv, inst, cls, alias) in list(devices.items()):
        for odev, (osrv, oinst, ocls, oalias) in orig_devices.items():
            if odev.lower() == dev.lower():
                ...
                if all([alias is not None,
                        (oalias is None or alias.lower() != oalias.lower()),
                        alias in all_aliases]):
                    orig_device = db.get_device_from_alias(alias)
                    alias_collisions.append((alias, orig_device, dev))
    ...
    return config, original, dbcalls, collisions, alias_collisions


def show_output(config, original, dbcalls, collisions, alias_collisions, db,
