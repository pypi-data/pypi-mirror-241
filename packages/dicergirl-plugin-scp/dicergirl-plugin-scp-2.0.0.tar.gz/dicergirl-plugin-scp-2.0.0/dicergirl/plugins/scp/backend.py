...
logger = multilogger(name="DicerGirl", payload="scp.backend")
...
def deal(event, args):
    ...
    for lvl in weapons.keys():
        allowed_upper.update(
            {
                allow.upper(): [lvl, weapon, allow]
                for allow, weapon in weapons[lvl].items()
            }
        )
...
def begin():
    ...
    reply.append(
        "我们躲在山洞中, 围坐在小小的篝火边, 畏惧那些我们不懂得的事物——那些关于太阳如何升起的解释, 那些人头鸟身的怪物, 那些有生命的石头."
    )
