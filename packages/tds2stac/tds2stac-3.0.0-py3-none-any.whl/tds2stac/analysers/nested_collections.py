...
class NestedCollectionInspector(object):
    ...
    nd 9 in Recognizer class.
    The output will be a list of the tuples:
    (Root collection URL, Collection ID, Collectin Title, corresponding subset URLs)

    Args:
        main_catalog_url (str): The URL of the TDS catalog
        nested_number (int, optional): Number of depth for nested datasets
        auth (tuple, optional): Authentication for TDS catalog e.g.('user', 'password')
    ...
    """
        Authentication for TDS catalog e.g.('user', 'password')
    """
    ...
    def __init__(
        ...
        if self.nested_number is not None:
            ...
            if any(
                scenario in str(recognizer.Recognizer(self.xml_url_catalog))
                ...
                if self.nested_number == 0:
                        ...
                        if isinstance(i, dict):
                            for k, v in i.items():
                                if isinstance(v, dict):
                                    self.end_point_url_extracter_dict(v)
                                if isinstance(v, list):
                                    self.end_point_url_extracter_list(v)
                                if isinstance(v, str):
                                    self.depth_addresses.append(v)
                        # rarely happens
                        if isinstance(i, list):
                            self.end_point_url_extracter_list(i)
                        if isinstance(i, str):
                    ...
                    if "Ninth Scenario" in str(
                        recognizer.Recognizer(self.xml_url_catalog)
                    ):
    ...
    def end_point_url_extracter_dict(self, d: dict):
        """
    ...
    def end_point_url_extracter_list(self, list_: list):
        """
        A function for extracting the end point URLs
        of a nested list.
        ...
        for i in list_:
            if isinstance(i, dict):
                self.end_point_url_extracter_dict(i)
            if isinstance(i, list):
                self.end_point_url_extracter_list(i)
            if isinstance(i, str):
                # if isinstance(self.depth_addresses, list):
