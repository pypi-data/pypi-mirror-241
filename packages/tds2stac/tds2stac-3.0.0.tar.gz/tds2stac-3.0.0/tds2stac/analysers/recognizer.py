...
class Recognizer(object):
    ...
    in the following.
        ...
        First senario: Just `catalogRef` tags are located directly under the dataset element tag.
        ...
        Third senario: One single `dataset` tag is located next to `CatalogRef` tags. All are under a `dataset` tag.
            `https://thredds.imk-ifu.kit.edu/thredds/catalog/regclim/raster/global/chirps/catalog.xml` (nested)
        Fourth senario: An empty datasets.
        ...
        Fifth senario: There is no `CatalogRef` tag and all are `dataset` tag. All of them are under a `dataset` tag.
            `https://thredds.imk-ifu.kit.edu/thredds/catalog/climate/raster/global/chelsa/v1.2/catalog.html`
        Sixth senario: A single dataset
        ...
        Seventh senario: An aggregated dataset
            `https://thredds.imk-ifu.kit.edu/thredds/catalog/catalogues/swabian_moses_2021.xml?dataset=swabian_moses_aggregation`
        Eigth senario: A combination of `caralogRef` and `dataset` tags that is not under a `dataset` tag.It's similar to second senario but with datasets
            `https://thredds.imk-ifu.kit.edu/thredds/catalog/catalogues/transfer.xml`
        Nineth senario: When we have a bunch of single `dataset` tags next to catalogref. It's similar to third senario but with more datasets.
    ...
    def recognition_function(self, url: str, xml_content):
        ...
        # the fifth one is haveing all data under the dataset (https://thredds.imk-ifu.kit.edu/thredds/catalog/climate/raster/global/chelsa/v1.2/catalog.html)
        ...
        # the eighth case is combination of caralogRef and dataset that is not under a dataset tag like seconf case but with dataset (https://thredds.imk-ifu.kit.edu/thredds/catalog/catalogues/transfer.html)
    ...
    def nested_checker(self, url: str):
        ...
        nested datasets in TDS for senarios 1, 3,
        ,and 9
        """
        xml_url_catalog, id_catalog, xml = utils.xml_processing(url, self.auth)
        try:
            tree = etree.XML(xml)
        except BaseException:
            return (
    ...
    def nested_checker_exceptions(self, url: str):
        """
        A function for returning the depth of
        nested datasets in TDS for senarios 2 and 8
        """
        xml_url_catalog, id_catalog, xml = utils.xml_processing(url, self.auth)
    ...
    def __str__(self):
        # TODO: self.logger.info(self.recognition_function(self.xml_url_catalog, self.xml))
