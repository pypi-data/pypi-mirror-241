...
def interp_state_chgt(x_prev,y_prev,yb,t_prev,x_now,y_now,t_now,f,i_prev,h_prev,
                      c_now):
    ind=np.where(np.array_equiv(y_now,yb),-1,np.argmax(np.abs(y_now-yb)))
    val_seuil=yb[ind]
...
def next_step_simulation(x_prev,t_prev,y_prev,i_prev,c_prev,h_prev,f,tol,T,*P):
    ...
    if hasattr(f,'update'):
        if hasattr(f,'commande'):
            ...
            i_now,_,yb=f.update(x_now,y_now,t_now,i_prev,c_now)
        else:
            c_now=c_prev
            i_now,_,yb=f.update(x_now,y_now,t_now,i_prev)
        x_now,y_now,h_now,t_now=cond(np.bitwise_not(np.allclose(i_prev,i_now)),
            lambda state:interp_state_chgt(x_prev,y_prev,yb,t_prev,x_now,y_now,
            t_now,f,i_prev,h_prev,c_now), lambda state:state,(x_now,y_now,h_now,
            t_now))
...
def _odeint45(f,h0,tol,x0,vect_t,T,*P):
    ...
    def scan_fun(state,te):
        ...
        def body_fn(state):
            ...
            x_now,y_now=cond(t_now>=te,lambda state:interpolation(x_prev,y_prev,
                t_prev,x_now,y_now,t_now,te),lambda state:state,(x_now,y_now))
            t_now=np.where(t_now>te,te,t_now)

            return x_now,t_now,y_now,h_now,i_now,c_now

        x_now,t_now,y_now,h_now,i_now,c_now = while_loop(cond_fn,body_fn,state)

        if not hasattr(f,'update'):
            h_now=h0
    ...
    if hasattr(f,'initialize'):
        P=tempP
    vect,(xf,yf,etats)=scan(scan_fun,(x0,vect_t[0],y0,h0,i0,c0),vect_t[1:])
    if hasattr(f,'etat'):
        f.etat=vect[4]
...
def _odeint45_jvp(f,h0,tol, primals, tangents):
    ...
    if hasattr(f,'update'):
        xf,dxf,yf,dyf,etats=odeint45_etendu(f,nPdP,h0,tol,x0,x0,vect_t,T,*P,*dP)
        return (xf,yf,etats),(dxf,dyf,etats)
    else:
...
def odeint45_etendu(f,nPdP,h0,tol,x0,dx0,vect_t,T,*P_and_dP):
    ...
    if hasattr(f,'initialize'):
        P,dP=tempP,tempdP
    vect,(xf,dxf,yf,dyf,etats)=scan(scan_fun,(x0,dx0,vect_t[0],y0,dy0,h0,i0,c0),
                                    vect_t[1:])
    if hasattr(f,'etat'):
        f.etat=vect[6]
    xf=np.transpose(np.concatenate((x0[None], xf)))
