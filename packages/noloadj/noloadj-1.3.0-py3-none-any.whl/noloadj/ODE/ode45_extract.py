...
def _odeint45_extract(f,h0,tol,x0,T,*P):
    ...
    def body_fn(state):
        ...
        if type=='seuil':
            ...
            x_now,h_prev,t_now,y_now=cond(np.sign(output-seuil)!=np.sign(
                outputprev-seuil),lambda state:compute_new_point(x_now,x_prev,
                t_prev,t_now,y_prev,y_now,output-seuil,outputprev-seuil),
...
def _odeint45_extract_jvp(f,h0,tol, primals, tangents):
    ...
    if hasattr(f,'etat'):
        xf,yf,cstr,tf,dtf,dxf,dyf,dcstr,etats=odeint45_extract_etendu(f,
                        nPdP,h0, tol, x0,dx0,T,dT, *P, *dP)
        return (tf,xf,yf,cstr,etats),(dtf,dxf,dyf,dcstr,etats)
    else:
