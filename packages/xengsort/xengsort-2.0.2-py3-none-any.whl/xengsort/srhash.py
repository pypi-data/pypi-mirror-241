        ...
        for bucket in range(nbuckets):
            ...
            for slot in range(bucketsize):
                if ctrl_choice:
                    ...
                    if c != 0:
                        last = slot
                        v = min(get_value_at(table, subtable, bucket, slot), 1023)
                        valuehist[v] += 1
                elif ctrl_value:
                    v = min(get_value_at(table, subtable, bucket, slot), 1023)
                    valuehist[v] += 1
...
def print_statistics(h, level):
    show_statistics = (level != 'none')
    ...
    valuehist, fillhist, choicehist, shortcuthist = get_statistics(h)
    if level == "full":
