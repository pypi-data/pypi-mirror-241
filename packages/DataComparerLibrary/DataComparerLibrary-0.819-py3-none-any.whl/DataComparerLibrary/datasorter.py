...
import csv
import os
# from delimitertranslator import DelimiterTranslator
                ...
                if number_of_header_lines == 0 and number_of_trailer_lines == 0:
                    ...
                    match number_of_sort_columns:
                        ...
                        case 1:
                            ...
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a]))
                        case 2:
                            a = sort_on_columns_list[0]
                            b = sort_on_columns_list[1]
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a], int(column[b]) if isinstance(b, int) else column[int(b)]))
                        case 3:
                            a = sort_on_columns_list[0]
                            b = sort_on_columns_list[1]
                            c = sort_on_columns_list[2]
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a], int(column[b]) if isinstance(b, int) else column[int(b)], int(column[c]) if isinstance(c, int) else column[int(c)]))
                        case 4:
                            a = sort_on_columns_list[0]
                            b = sort_on_columns_list[1]
                            c = sort_on_columns_list[2]
                            d = sort_on_columns_list[3]
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a], int(column[b]) if isinstance(b, int) else column[int(b)], int(column[c]) if isinstance(c, int) else column[int(c)], int(column[d]) if isinstance(d, int) else column[int(d)]))
                        case 5:
                            a = sort_on_columns_list[0]
                            b = sort_on_columns_list[1]
                            c = sort_on_columns_list[2]
                            ...
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a], int(column[b]) if isinstance(b, int) else column[int(b)], int(column[c]) if isinstance(c, int) else column[int(c)], int(column[d]) if isinstance(d, int) else column[int(d)], int(column[e]) if isinstance(e, int) else column[e]))
                        case 6:
                            a = sort_on_columns_list[0]
                            b = sort_on_columns_list[1]
                            c = sort_on_columns_list[2]
                            ...
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a], int(column[b]) if isinstance(b, int) else column[int(b)], int(column[c]) if isinstance(c, int) else column[int(c)], int(column[d]) if isinstance(d, int) else column[int(d)], int(column[e]) if isinstance(e, int) else column[e], int(column[f]) if isinstance(f, int) else column[f]))
                        case 7:
                            a = sort_on_columns_list[0]
                            b = sort_on_columns_list[1]
                            c = sort_on_columns_list[2]
                            ...
                            sorted_data_text = sorted(data_text, key=lambda column: (int(column[a]) if isinstance(a, int) else column[a], int(column[b]) if isinstance(b, int) else column[int(b)], int(column[c]) if isinstance(c, int) else column[int(c)], int(column[d]) if isinstance(d, int) else column[int(d)], int(column[e]) if isinstance(e, int) else column[e], int(column[f]) if isinstance(f, int) else column[f], int(column[g]) if isinstance(g, int) else column[g]))
                        case _:
