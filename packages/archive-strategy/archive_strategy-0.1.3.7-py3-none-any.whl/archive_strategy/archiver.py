...
class Archiver:
    ...
    def list_files(
        ...
        Args:
            ...
            timestamp (bool, optional): Whether to show the timestamp of the file.
                Defaults to False.
            timeframe (bool, optional): Whether to show the timeframe of the file.
                Defaults to False.
            timeframe_key (bool, optional): Whether to show the timeframe key of the
    ...
    def archive(self) -> None:
        ...
        with Progress() as progress:
            move_task = progress.add_task("Moving to archive...", total=len(files_to_archive))
            owner_task = progress.add_task("Changing owner...", total=len(files_to_change_owner))
            prune_task = progress.add_task("Pruning files...", total=len(files_to_delete))

            for archive_file in files_to_archive:
                archive_file.move_to_archive()
                to_advance = 1 / len(files_to_archive) if len(files_to_archive) > 0 else 1
                progress.advance(move_task, advance=to_advance)

            for archive_file in files_to_change_owner:
                archive_file.change_owner()
                to_advance = 1 / len(files_to_change_owner) if len(files_to_change_owner) > 0 else 1
