...
def detect_barking(path_to_file):
    ...
    if bark == 1: ### TODO: resolve nan cases
        no_of_barks = random.sample(range(1,5),1)[0]
        timestamp_array = np.array(random.sample(range(0,100),no_of_barks))
        timestamp_array = timestamp_array/100
        timestamp_array = timestamp_array.tolist()
    else:
        no_of_barks = 0
        timestamp_array = [0]
    
    return_json = {'number of barks':no_of_barks, 'timestamps':timestamp_array}
