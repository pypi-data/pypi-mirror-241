...
class pynal():
	...
	def __init__(self):
		...
		self.var.update(self.var_common)
	...
	def change_any_text_time_to_dt_obj(self, input_text_time):
		...
		# Tuesday
		for one_week in self.var["dic_week_no"].keys():
			if one_week in dt_string:
				result["week"] = self.var["dic_week_no"][one_week]
		...
		if resql_result:
			result["mon"] = self.var["dic_mon_no"][resql_result[0][3][0]]
		...
		if resql_result:
			...
			for num in self.var["dic_no_mon"].keys():
				if bbb[0][0] in self.var["dic_no_mon"][num]:
					result["mon"] = num
	...
	def change_dt_obj_to_weekno_set(self, input_dt_obj):
		...
		result = [weekno, self.var["week"][weekno], self.var["요일"][weekno]]
		return result
	...
	def change_ymd_list_to_sec(self, input_list=[0, 0, 1]):
		...
		:return:
		"""
		total_sec = int(input_list[0]) * self.var["year_sec"] + int(input_list[1]) * self.var["month_sec"] + int(input_list[2]) * self.var["day_sec"]
		return total_sec
