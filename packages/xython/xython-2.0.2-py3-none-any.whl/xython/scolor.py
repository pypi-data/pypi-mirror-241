...
class scolor:
	...
	def change_excel56_to_rgb(self, input_no):
		...
		:return:
		"""
		result = self.var["excel56_rgb"][int(input_no)-1]
		return result
	...
	def change_hsl_to_rgb_with_pccs_style(self, input_hsl_list, color_style="파스텔", style_step=5):
		...
		step_2 = self.var["sl_big_step"][step]  # 스타일을 적용하는것
		step_1 = self.var["sl_no_vs_sl_small_step"][str(style_step)]  # 스타일을 얼마나 강하게 적용할것인가를 나타내는것
	...
	def change_scolor_to_hsl(self, input_scolor):
		...
		if type(input_scolor) == type([]):
			...
			if number_only != "":
				...
				r_no, g_no, b_no = self.var["excel56_rgb"][int(number_only)-1]
			else:
				# 색을 번호로 변경하는것
				...
				if color_name == "whi" or color_name == "bla" or color_name == "gra":
					...
					# 기본색 인경우
					h_code, s_code, l_code = self.var["color_vs_hsl"][color_name]
				else:
					# 기타 다른 경우
					h_code = self.var["color_vs_hsl"][color_name][0]
					s_code = 100
	...
	def change_scolor_to_rgb_with_pccs_style(self, input_scolor="red45", color_style="파스텔", style_step=5):
		...
		# 스타일을 적용하는것
		aaa = self.var["color_tone_12_names_vs_no"][color_style]
		step_2 = self.var["sl_no_vs_sl_small_step"][aaa]
		# 스타일을 얼마나 강하게 적용할것인가를 나타내는것
		step_1 = self.var["sl_big_step"][str(style_step)]
	...
	def check_change_mode(self, change_mode):
		...
		if type(change_mode) == type([]):
			...
			result = self.var["sl_big_step"][no]
		return result
	...
	def check_hsl_list(self, input_color):
		...
		try:
			...
			result = self.var["color_name_vs_hsl_list"][color_name]
		except:
			pass
		# 입력된 색이름을 찾을수 없을때
		return result
	...
	def check_input_hsl(self, input_color):
		...
		try:
			color_name = self.var["check_color_name"][input_color]
			result = self.var["color_vs_hsl"][color_name]
		except:
			pass
		# 입력된 색이름을 찾을수 없을때
		return result
	...
	def control_hsl_by_color_style(self, basic_hsl, color_style="파스텔", style_step=5):
		...
		step_1 = self.var["sl_no_vs_sl_small_step"][str(str(style_step))]
		h = int(basic_hsl[0])
		s = int(step_1[0]) + int(step_2[0])
		l = int(step_1[1]) + int(step_2[1])
	...
	def control_scolor_by_pccs_style(self, input_scolor="red45", color_style="파스텔", style_step=5):
		...
		basic_hsl = self.change_scolor_to_hsl(input_scolor)
		# 스타일을 적용하는것
		step_2 = self.var["sl_no_vs_sl_small_step"][color_style]
		# 스타일을 얼마나 강하게 적용할것인가를 나타내는것
		step_1 = self.var["sl_big_step"][str(style_step)]
	...
	def data_12rgb_for_pccs_style(self, input_hsl_list):
		...
		"""
		result = []
		for one_value in self.var["hsl12"]:
	...
	def data_basic_johannes_rgb_set(self, start_color=11, num_color=4, stongness=5):
		...
		"""
		h_list = self.var["hsl12"]
		sl_list = self.var["sl11"]
		hsl_johannes = self.var["hsl_by_johannes"]
	...
	def data_color_name_eng_for_13set(self):
		...
		:return:
		"""
		result = self.var["color_name_eng"]
		return result
	...
	def data_pccs_style_name_for_12set(self):
		...
		:return:
		"""
		result = list(self.var["color_tone_eng"].keys())
		return result
