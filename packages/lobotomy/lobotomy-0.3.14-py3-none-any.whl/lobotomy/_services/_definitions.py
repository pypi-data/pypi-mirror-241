...
def _read_potential_gzip(path: pathlib.Path) -> str:
    """
    Read the contents of the given path, which may be gzipped.
...
def _get_specification(service_name: str) -> dict:
    ...
    spec = json.loads(
        # As of botocore 1.32.1, the service definitions are gzipped.
        # This checks if the non-gzipped version exists and if not, tries
        # the gzipped version.
        _read_potential_gzip(directory.joinpath(folder, "service-2.json"))
    )
