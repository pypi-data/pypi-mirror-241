...
def find_read_msas(
    root_dir: str, max_return: int = None
...
def _gen_msa_from_alleles(
    chrom: str, pos: int, alleles: Iterable[str]
...
def _apply_annotate_msa(
    msa: MultipleSeqAlignment, **other_anns
) -> MultipleSeqAlignment:
...
def _apply_seq_name_to_anns_regex(
    seq: SeqRecord, rex: Pattern, fields: Iterable[Hashable] = None
) -> None:
...
def _apply_seq_name_to_anns_split(
    seq: SeqRecord, fields: Iterable[Hashable], sep: str = "|"
) -> None:
    ...
    """
    seq.annotations |= dict(
        zip(fields, (oed.parse_literal_eval(val) for val in seq.name.split(sep)))
    )


def _apply_seqs(
    msa: MultipleSeqAlignment, fn: Callable, *args, **kwargs
) -> MultipleSeqAlignment:
...
def parse_alignments(
    ...
    try:
        ...
        msas.apply(
            _apply_seqs, fn=_apply_seq_name_to_anns_split, fields=fields, sep=sep
        )
    else:
        msas.apply(
            _apply_seqs, fn=_apply_seq_name_to_anns_regex, rex=rex_name, fields=fields
        )
    ...
    if sort_by is not None:
        msas.apply(lambda msa: msa.sort(key=lambda seq: seq.annotations[sort_by]))

    oed.msg(oed.now(), "Adding MSA annotations and sorting...")

    msa_idx_cols = ["CHROM", "POS"]
    msas = (
        msas.apply(lambda msa: dict((k, msa.annotations[k]) for k in msa_idx_cols))
