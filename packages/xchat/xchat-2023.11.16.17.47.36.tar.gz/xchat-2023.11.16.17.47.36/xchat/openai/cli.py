...
import xchat.openai as openai
from xchat.openai.upload_progress import BufferReader
from xchat.openai.validators import (
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import xchat.openai.wandb_logger

        resp = xchat.openai.wandb_logger.WandbLogger.sync(
            id=args.id,
...
def wandb_register(parser):
    subparsers = parser.add_subparsers(
        title="wandb",
        help="Logging with Weights & Biases, see https://docs.wandb.ai/guides/integrations/openai for documentation"
    )
