        ...
        for machine in self.learner_names:
            ...
            if not final:
                ...
                for i in range(self.n_fold):
                    mod_fit = mod.fit(self.X_[self.cv_folds_ != i,:], self.y_[self.cv_folds_ != i])
                    temp[self.cv_folds_ == i] = mod_fit.predict(self.X_[self.cv_folds_ == i,:])
                if first:
                    self.Z_ = temp
                    first = False
                else:
    ...
    def train_meta_learners(self):
        ...
        if self.n_extra_learners > 0:
            for extra_learner in self.extra_learners:
                temp = np.zeros(shape=len(self.y_))
                for i in range(self.n_fold):
                    mod_fit = self.extra_learners[extra_learner].fit(self.X_[self.cv_folds_ != i,:], self.y_[self.cv_folds_ != i])
                    temp[self.cv_folds_ == i] = mod_fit.predict(self.X_[self.cv_folds_ == i,:])
                self.Z_ = np.column_stack([self.Z_, temp])
        ...
        def cv_eval(method):
            s = 0
            for i in range(self.n_fold):
                ft = method.fit(self.X_[self.cv_folds_ != i,:], self.y_[self.cv_folds_ != i])
                s += mean_squared_error(ft.predict(self.X_[self.cv_folds_ == i]), self.y_[self.cv_folds_ == i])
