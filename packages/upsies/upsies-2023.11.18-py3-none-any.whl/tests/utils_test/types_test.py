...
def test_ImageHost_allowed(mocker):
    ...
    assert isinstance(name, cls)
    assert isinstance(name, str)
    with pytest.raises(ValueError, match=r'^Not one of baz, foo: bar$'):
        cls('bar')

def test_ImageHost_disallowed(mocker):
    mocker.patch('upsies.utils.imghosts.imghost_names', return_value=('foo', 'bar', 'baz'))
    ...
    assert isinstance(name, cls)
    assert isinstance(name, str)
    with pytest.raises(ValueError, match=r'^Not one of bar, baz: foo$'):
        cls('foo')
