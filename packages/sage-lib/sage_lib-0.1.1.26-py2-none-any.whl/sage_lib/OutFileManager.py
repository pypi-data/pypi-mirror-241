...
class OutFileManager(FileManager):
    ...
    def export_configXYZ(self, file_location:str=None, save_to_file:str='w', filter_by_forces:bool=False, verbose:bool=False):
        file_location  = file_location if file_location else self.file_location+'_config.xyz'

        if filter_by_forces:
            for APM in self.AtomPositionManager:
                if np.any(APM.total_force > 1): 
                    print(APM.total_force)
                    return False
                else:
                    pass
