...
class Account:
    ...
    async def set_silence_mode(self, params: AccountSetSilenceModeParams):
        return await self.vk.execute('account', 'setSilenceMode', **params.model_dump(exclude_none=True))

    async def unban(self, params: AccountUnbanParams):
        return await self.vk.execute('account', 'unban', **params.model_dump(exclude_none=True))

    async def get_info(self, params: AccountGetInfoParams):
        return await self.vk.execute('account', 'getInfo', **params.model_dump(exclude_none=True))
