...
from ...utils import normalize_name, \
    double_quote_name, _retrieve_parameter_value, try_single_quote_value
...
class TableResource(ResourceBase):
    ...
    def _desc_table(self, deep: bool = False) -> Tuple[str, Dict[str, Any]]:
        sql_str = f"SHOW tables like '{self._table_name}' in schema {self._database}.{self._schema}"
        tables = self.snow_exec.execute(sql_str)
        if not tables:
            raise NotFound(f"Table {self._table_name} doesn't exist.")
        self._validate_table(tables, deep)
