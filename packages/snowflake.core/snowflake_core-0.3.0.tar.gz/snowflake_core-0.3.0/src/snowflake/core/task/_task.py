...
def _dict_to_task_info(info: Dict[str, Any]) -> "Task":
    _logger.debug(f"Load task: {info}")
    # schedule str -> timedelta | cron
    if info["schedule"]:  # Neither None nor ''
        schedule_str = info["schedule"].strip(" ").lower()
        if "minute" in schedule_str:  # <num> minute
            info["schedule"] = timedelta(minutes=int(schedule_str.split(" ")[0]))
        else:  # using cron * * * * * <time zone>
            split = schedule_str.split(" ")
            cron_str = f"{split[2]} {split[3]} {split[4]} {split[5]} {split[6]}"
            timezone = split[7]
            info["schedule"] = Cron(cron_str, timezone)
    # update allow_overlapping_execution, str -> bool
    info["allow_overlapping_execution"] = (
        info["allow_overlapping_execution"].lower() == "true"
    )

    # update predecessors, str -> list[str] '['db'.'sc'.'t1', 't2', 't3']'
    info["predecessors"] = json.loads(info["predecessors"])
    return Task(**info)


def _parameter_value_to_str(value: Union[str, int, float, bool]) -> str:
...
class Cron(NamedTuple):
    ...
    Example:
        >>> cron1 = Cron("0 0 10-20 * TUE,THU UTC", "America/Los_Angeles")
    """
