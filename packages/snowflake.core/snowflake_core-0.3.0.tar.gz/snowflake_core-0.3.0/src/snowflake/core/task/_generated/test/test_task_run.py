# coding: utf-8

"""
    Snowflake Task API

    The Snowflake Task API is a REST API that you can use to access, update, and perform certain actions on task resources in a Snowflake database.  # noqa: E501

    The version of the OpenAPI document: 0.0.1
    Contact: support@snowflake.com
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import snowflake.core.task._generated
from snowflake.core.task._generated.models.task_run import TaskRun  # noqa: E501
from snowflake.core.task._generated.rest import ApiException

class TestTaskRun(unittest.TestCase):
    """TaskRun unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test TaskRun
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `TaskRun`
        """
        model = snowflake.core.task._generated.models.task_run.TaskRun()  # noqa: E501
        if include_optional :
            return TaskRun(
                root_task_name = '', 
                database_name = '', 
                schema_name = '', 
                state = 'SCHEDULED', 
                first_error_task_name = '', 
                first_error_code = 56, 
                first_error_message = '', 
                scheduled_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                query_start_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                next_scheduled_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                completed_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                root_task_id = '', 
                graph_version = 56, 
                run_id = 56
            )
        else :
            return TaskRun(
                root_task_name = '',
                database_name = '',
                schema_name = '',
                state = 'SCHEDULED',
                scheduled_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                next_scheduled_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                root_task_id = '',
                graph_version = 56,
                run_id = 56,
        )
        """

    def testTaskRun(self):
        """Test TaskRun"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
