...
from snowflake.core.exceptions import ApiValueError
...
class DatabaseResource(ObjectReferenceMixin[DatabaseCollection]):
    ...
    def fetch(self) -> Database:
        return self.collection._api.fetch_database(
            self.name,
            async_req=False,
        )
    ...
    def create_or_update(
        ...
        self._api.create_or_update_database(
            database.name, database, async_req=False
        )
        return self
    ...
    @api_telemetry
    def primary_failover(self) -> None:
