...
from ...utils import normalize_name, parse_table_name, try_single_quote_value
...
class TaskResource(ResourceBase):
    ...
    def execute(self) -> Union[List[Dict[str, Any]], Dict[str, Any]]:
        ...
        if self._method == "PUT":
            ...
            if self._task_name != "" and self._parameter_key == "":
                _, ret = self._delete_task()
            elif self._parameter_key != "":
                _, ret = self._delete_parameter()
            else:
    ...
    def _create_or_alter_task(self) -> Tuple[str, Dict[str, Any]]:
        ...
        def _set_parameters(remote_param_list: Dict[Any, Any]) -> Any:
            ...
            for param in {*remote_param_list.keys(), *params_to_set}:
                # Only consider parameters that are not set to their default values
                # and which we have permission to change - Task level params
                if param == "USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE":
                    # even though this is originally set as a property it shows up
                    # in parameters. This property cannot be unset, so skipping it
                    # manually.
                    continue
    ...
    def _get_complete_graphs(self) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        if "resultLimit" in self._query_params:
            sql_str = "SELECT * from table(information_schema.complete_task_graphs(result_limit => {}, error_only=> {}))".format(
                self._query_params["resultLimit"], error_only
            )
        else:
    ...
    def _get_current_graphs(self) -> Tuple[str, List[Dict[str, Any]]]:
        if "resultLimit" in self._query_params:
            sql_str = "SELECT * from table(information_schema.current_task_graphs(result_limit => {}))".format(
                self._query_params["resultLimit"]
            )
        else:
    ...
    def _delete_task(self) -> Tuple[str, Dict[str, Any]]:
        if "ifExists" in self._query_params and self._query_params["ifExists"]:
            sql_str = f"DROP TASK IF EXISTS {self._task_name}"
        else:
            sql_str = f"DROP TASK {self._task_name}"
        return sql_str, self.snow_exec.execute(sql_str)[0]

    def _delete_parameter(self) -> Tuple[str, Dict[str, Any]]:
        if "ifExists" in self._query_params and self._query_params["ifExists"]:
            sql_str = (
                f"ALTER TASK IF EXISTS {self._task_name} UNSET {self._parameter_key}"
            )
        else:
            sql_str = f"ALTER TASK {self._task_name} UNSET {self._parameter_key}"
        return sql_str, self.snow_exec.execute(sql_str)[0]

    def _set_parameters(self) -> Tuple[str, Dict[str, Any]]:
        # The assumption here is the body only has parameters in a Dict[str, Any] format
        param_list = ""
        for param_name in self._prop:
            if param_list != "":
                param_list = param_list + ", "
            param_list = param_list + f"{param_name} = {self._prop[param_name]}"
        if "ifExists" in self._query_params and self._query_params["ifExists"]:
            sql_str = f"ALTER_TASK IF EXISTS {self._task_name} SET {param_list}"
        else:
            sql_str = f"ALTER_TASK {self._task_name} SET {param_list}"
        return sql_str, self.snow_exec.execute(sql_str)[0]

    def _suspend_task(self) -> Tuple[str, Dict[str, Any]]:
        if "ifExists" in self._query_params and self._query_params["ifExists"]:
            sql_str = f"ALTER_TASK IF EXISTS {self._task_name} SUSPEND "
        else:
    ...
    def _resume_task(self) -> Tuple[str, Dict[str, Any]]:
        ...
        if "ifExists" in self._query_params and self._query_params["ifExists"]:
            sql_str = f"ALTER_TASK IF EXISTS {self._task_name} RESUME "
        else:
    ...
    def _get_task_parameters(self) -> List[Dict[str, Any]]:
        return self.snow_exec.execute(f"SHOW PARAMETERS IN TASK {self._task_name}")
            ...
            if deep:
                params = self.snow_exec.execute(
                    "SHOW PARAMETERS in Task {}.{}.{} ".format(
                        t["database_name"], t["schema_name"], t["name"]
                    )
                )
