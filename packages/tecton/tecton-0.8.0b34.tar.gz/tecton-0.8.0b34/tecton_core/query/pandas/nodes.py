...
class PandasDataSourceScanNode(PandasExecNode):
    ...
    def _to_dataframe(self) -> pandas.DataFrame:
        ...
        if isinstance(batch_source, specs.PandasBatchSourceSpec):
            ...
            df = self._get_df_from_file_source(batch_source, self.ds.name)
        else:
            msg = "tecton-batch mode only supports Pandas and File batch data sources"
    ...
    def _get_df_from_file_source(self, batch_source: specs.FileSourceSpec, data_source_name: str) -> pandas.DataFrame:
        file_uri = batch_source.uri
        source = self.ds.batch_source
        timestamp_field = source.timestamp_field

        proto_format = batch_source.file_format
        if proto_format == batch_data_source_pb2.FILE_DATA_SOURCE_FORMAT_CSV:
            ...
            raise ValueError(batch_data_source_pb2.FileDataSourceFormat.Name(batch_source.file_format))
        ...
        try:
            ...
            # TODO(TEC-17401): Remove to_pandas()
            file_table = pyarrow.dataset.dataset(source=path, filesystem=fs, format=arrow_format).to_table().to_pandas()
            name = "file_table"
            duckdb_compute.register_temp_table_from_pandas(name, file_table)
            scan_sql = f"SELECT * FROM {name} "
            where_conditions = []
            if self.start_time:
            ...
            return duckdb_compute.run_sql(scan_sql, return_dataframe=True).to_pandas()
        except UserDefinedTransformationError as e:
            msg = f"reading Parquet data source '{data_source_name}'"
            raise e.with_context(msg)
        finally:
            duckdb_compute.cleanup_temp_tables()
...
class PandasFeatureViewPipelineNode(PandasExecNode):
    ...
    def _transformation_node_to_dataframe(self, transformation_node: TransformationNode) -> pandas.DataFrame:
        ...
        try:
            ...
            msg = (
                "Pandas pipeline function of feature view "
