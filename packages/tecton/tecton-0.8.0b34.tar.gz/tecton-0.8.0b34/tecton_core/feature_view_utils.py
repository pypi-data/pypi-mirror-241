...
def construct_aggregation_output_feature_name(
    ...
    column_name = (
        f"{column}_{aggregation_function_resolved_str}_{window_duration_name}_{aggregation_interval_name}".replace(
            " ", ""
        )
    )
    if offset_name:
        column_name = f"{column_name}_{offset_name}"
