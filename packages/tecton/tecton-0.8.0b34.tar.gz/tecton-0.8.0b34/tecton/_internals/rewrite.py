...
class SpineEntityPushdown(Rewrite):
    ...
    def pushdown_entities(self, tree: NodeRef, spine: NodeRef, join_cols: List[str]):
        ...
        if isinstance(node, can_be_pushed_down):
            ...
            entities_node = SelectDistinctNode(node.dialect, spine, join_cols).as_ref()
            tree.node = EntityFilterNode(node.dialect, node.as_ref(), entities_node, join_cols)
        ...
        if isinstance(node, can_be_pushed_down):
            ...
            if isinstance(node, FeatureViewPipelineNode):
                ...
                node = FeatureViewPipelineNode(
                    node.dialect, node.inputs_map, node.feature_definition_wrapper, feature_time_limits
                )
                tree.node = node
