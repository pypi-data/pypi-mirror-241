...
def _prepare_qt_executor(
    ...
    pipeline_monitor = monitor_for_stage(
        _DIALECT_TO_STAGE_TYPE[pipeline_dialect], f"Processing dataset from {_DIALECT_TO_UI_STRING[pipeline_dialect]}"
    )
...
def ray_main(materialization_task_params: MaterializationTaskParams) -> None:
    ...
    conf.set("TECTON_OFFLINE_RETRIEVAL_COMPUTE_MODE", "tecton-batch")
