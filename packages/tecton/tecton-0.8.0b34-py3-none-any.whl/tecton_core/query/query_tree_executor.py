...
class QueryTreeExecutor:
    ...
    def _measure_stage(self, step: str, node: NodeRef) -> Iterator[None]:
        ...
        if self.is_inside_interactive_shell and not self.is_debug:
            stop_clock()
    ...
    def exec_qt(self, qt_root: NodeRef) -> QueryTreeOutput:
        ...
        try:
            ...
            with self._measure_stage("Evaluating feature view pipelines", qt_root):
            ...
            with self._measure_stage("Computing aggregated features & joining results", qt_root):
            ...
            with self._measure_stage("Computing ODFV features", qt_root):
    ...
    def _execute_data_source_step(self, qt_root: NodeRef) -> QueryTreeOutput:
        ...
        for name, staging_node in staging_nodes_to_process.items():
            ...
            if ds_dialect == Dialect.PANDAS or ds_dialect == Dialect.DUCKDB:
                ...
                except UserDefinedTransformationError as exc:
                    msg = f"reading from Snowflake batch source '{data_source_node.ds.name}'"
                    raise exc.with_context(msg)
            else:
