...
class LifetimeWindow(object):
    ...
    def _to_proto(self):
        return feature_view_pb2.LifetimeWindow()
...
class Aggregation(object):
    ...
    def __init__(
        ...
        if isinstance(time_window, datetime.timedelta):
            self.time_window = RelativeTimeWindowSpec(
                window_start=-time_window, window_end=datetime.timedelta(seconds=0)
            )
        elif isinstance(time_window, TimeWindow):
            self.time_window = RelativeTimeWindowSpec.from_args_proto(time_window._to_proto())
        elif isinstance(time_window, LifetimeWindow):
            self.time_window = LifetimeWindowSpec()
        self.name = name
    ...
    def _to_proto(self, aggregation_interval: datetime.timedelta, is_continuous: bool):
        ...
        if isinstance(self.time_window, RelativeTimeWindowSpec):
            proto.time_window.CopyFrom(self.time_window.to_args_proto())
        elif isinstance(self.time_window, LifetimeWindowSpec):
            proto.lifetime_window.CopyFrom(self.time_window.to_proto())
        else:
        ...
        if self.name:
            ...
            proto.name = feature_view_utils.construct_aggregation_output_feature_name(
                ...
                is_continuous,
                self.time_window.window_duration_string(),
                self.time_window.offset_string(),
            )
        return proto
