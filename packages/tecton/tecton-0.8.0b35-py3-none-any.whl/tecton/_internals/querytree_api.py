...
def _time_range_get_historical_features_for_feature_definition(
    ...
    if fd.is_temporal or fd.is_feature_table:
        ...
        qt = nodes.RenameColsNode(dialect, compute_mode, qt, drop=[query_consts.anchor_time()]).as_ref()
    ...
    if entities is not None:
        ...
        entities_df = nodes.SelectDistinctNode(
            dialect, compute_mode, nodes.UserSpecifiedDataNode(dialect, compute_mode, entities).as_ref(), columns
        ).as_ref()
        qt = nodes.JoinNode(dialect, compute_mode, qt, entities_df, columns, how="right").as_ref()
