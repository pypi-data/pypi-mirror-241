...
class Explainer():
  ...
  def run_irt(self, datasetName):
    ...
    if os.path.isfile(self.path_content+self.path_irt+self.bar+'OutExecution'+self.bar+'theta_list.csv'):
        os.remove(self.path_content+self.path_irt+self.bar+'OutExecution'+self.bar+'theta_list.csv')
    if os.path.isfile(self.path_content+self.path_irt+self.bar+'OutExecution'+self.bar+'score_total.csv'):
        os.remove(self.path_content+self.path_irt+self.bar+'OutExecution'+self.bar+'score_total.csv')
    ...
    result_irt_new.to_csv(self.path_content+self.path_irt+self.bar+'irt_item_param_new.csv',index=False)

    try:
      !python decodIRT_analysis.py -dir $self.path_content_simple$self.path_irt -nameData OutExecution -respMatrix $self.path_content$self.path_irt$self.bar'tabela_base_para_executar_irt.csv' -IRTparam $self.path_content$self.path_irt$self.bar'irt_item_param_new.csv' -accur $self.path_content$self.path_irt$self.bar'tabela_base_para_executar_irt_accuracy.csv' -scoreAll -save $self.irt_method -missing
