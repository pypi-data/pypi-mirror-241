...
try:
    ...
    from src.profiles_local import ProfilesLocal
...
class ComprehensiveProfilesLocal:
    ...
    @staticmethod
    def insert(profile_json: str, lang_code: str) -> int:
        logger.start(object={"profle_json": profile_json})
        ...
        if "reaction" in keys:
            ...
            reaction_id = reaction_obj.insert(
                reaction_data, profile_id, lang_code)
        ...
        if "operational_hours" in keys:
            operational_hours = OperationalHours.create_hours_array(
                data["operational_hours"])
            operational_hours_obj = OperationalHours()
            operational_hours_obj.insert(
                profile_id, location_id, operational_hours)
