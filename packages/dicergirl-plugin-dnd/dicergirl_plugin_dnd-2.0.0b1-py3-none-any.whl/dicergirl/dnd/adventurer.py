...
class Adventurer(object):
    ...
    def rollcount(self) -> Tuple[int, int]:
        before = self.str[0] + self.dex[0] + self.con[0] + self.int[0] + self.fel[0] + self.chr[0]
        resized = sum(self.str) + sum(self.dex) + sum(self.con) + sum(self.int) + sum(self.fel) + sum(self.chr)
        return before, resized
