...
class SimpleGuiFunctionsManager:
    ...
    def get_function_name_case(self,function_name):
        if not self.is_in_function_names(function_name=function_name):
            if function_name !=  None:
                function_name = find_original_case(self.function_names, function_name)
        return function_name
    
    def get_closest_match(self,function_name,*args):
        if not self.is_in_function_names(function_name=function_name):
            names = [function_name,*args]
            name = get_closest_match_from_list(comp_str= names, total_list=self.function_names)
            print(f"{function_name} not found, changing to closest match: {name}")
            function_name=name
        return function_name
    
    def is_in_function_names(self,function_name:str)->bool:
        return (function_name in self.function_names)
    
    def add_function_name_to_dict(self,function_name,original_input=None):
        if function_name not in self.function_names_dict:
            all_params = self.get_all_parameters(function_name=function_name)
            self.function_names_dict[function_name]={"names":[],"all_params":all_params,"required_params":all_params["names"]}
        if original_input and original_input not in self.function_names_dict[function_name]["names"] and function_name != original_input:
            self.function_names_dict[function_name]["names"].append(original_input)
            
    def check_function_names_dict(self,function_name:str)->(str or None):
        original_input=function_name
        if self.is_in_function_names(function_name=function_name):
            self.add_function_name_to_dict(function_name=function_name,original_input=original_input)
            return function_name
        else:
    ...
    def get_function_name(self,function_name,*args):
        original_name = function_name
        self.function_name=function_name
        function_name_found=self.check_function_names_dict(function_name=function_name)
        if function_name_found == None:
            self.function_name = self.get_function_name_case(function_name=self.function_name)
            self.function_name = self.get_closest_match(self.function_name,*args)
            if self.function_name:
                self.add_function_name_to_dict(self.function_name,original_input=original_name)
    ...
    def get_gui_function(self,function_name: str = '', *args_2, args: dict = {}, **kwargs):
        ...
        Returns:
            callable: A callable object that invokes the PySimpleGUI function with the specified arguments when called.
        """
        function_name = self.get_function_name(function_name=function_name)
        # Map positional arguments to their respective required_params
        function_name_dict = self.function_names_dict[function_name]
        required_params = function_name_dict["required_params"]
        ...
        for param in required_params:
            if param not in args:
                ...
                if change is not None:
                    ...
                    args[param] = change
        if "layout" in function_name_dict["all_params"]:
    ...
    def make_component(self,function_name,*args,**kwargs):
        if 'args' in kwargs:
            ...
            del kwargs['args']
            kwargs.update(arges)
        return self.get_gui_function(function_name,*args,**kwargs)
