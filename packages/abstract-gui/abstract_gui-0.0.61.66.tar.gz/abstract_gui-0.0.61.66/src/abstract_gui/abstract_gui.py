...
class SimpleGuiFunctionsManager:
    ...
    def _resolve_function_name(self, function_name):
        ...
        if function_name not in self.function_names:
            ...
            print(f"Function name resolved to closest match: {function_name}")
...
def get_screen_dimensions(height:int,width:int,max_size:int=None)->(int,int):
    screen_width, screen_height = make_component("Window",title='screen_size',layout=[[]]).get_screen_size()
    def calculate_size(percentage:int, screen_measure:int)->int:
