...
class BaseLabelme(metaclass=SingletonMeta):
    ...
    def md5_value_find(self, md5):
        ...
        for dataset in self.datasets:
            if md5 == dataset.get('md5_value'):
                ...
                image_dir = self.make_up_dir(image_dir_parts, dataset.get('full_path'))
                labelme_dir = self.make_up_dir(labelme_dir_parts, dataset.get('full_path'))
                rebuild_json_path = os.path.join(dataset.get('output_dir'), labelme_dir, dataset.get('labelme_file'))
                if self.make_up_dir(image_dir_parts, dataset.get('full_path')) not in dir_paths:
                    dir_paths.append(self.make_up_dir(image_dir_parts, dataset.get('full_path')))
    ...
    @staticmethod
    def make_up_dir(dir_parts, full_path):
        """
        重组图像存储目录和json文件存储目录
