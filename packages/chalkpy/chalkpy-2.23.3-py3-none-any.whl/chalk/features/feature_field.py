...
from chalk.features._vector import get_annotation_override
...
class Feature(Generic[_TPrim, _TRich]):
    def __init__(
        ...
        embedding_input: Callable[[], Feature] | None = None,
        ...
        if last_for is not None:
            ...
            for f in last_for.features_cls.features:
                if f.name == last_for.name:
                    f.last = last_for
                    break
        self._embedding_input = embedding_input
    ...
    def converter(self) -> FeatureConverter:
        ...
        vector_typ = self.typ.as_vector(get_annotation_override(self.embedding_provider))
    ...
    @property
    def embedding_input(self) -> Optional[Feature]:
        if self._embedding_input is None:
            return None
        return self._embedding_input()
...
def embedding(
    ...
    """
    return Feature(embedding_input=input, embedding_provider=provider, embedding_model=model)
