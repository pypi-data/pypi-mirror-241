...
class KineticEmail:
    ...
    @staticmethod
    def get_mailbox_pdfs(connection, attachment_path='', mailbox="Inbox"):
        ...
        list = KineticEmail.get_mailbox_messages(connection,attachment_path,mailbox,None,None)
        processed = []
        if list['error_code'] == "0":
            email_list = list['data']
            for i in email_list:
                record = {
                    "error_code": "0",
                    "error_msg": "",
                    "data": {
                        "email_message_number": i['message_number'],
                        "email_from_address": i['from_address'],
                        "email_to_address": i['to_address'],
                        "email_subject": i['subject'],
                        "email_body": i['body']
                        }
                    }
                ...
                if len(i['attachments']) > 0:
                    for j in i['attachments']:
                        pdf = KineticPdf.get_pdf_form(j)
                        if pdf['error_code'] == "0":
                            blank = True
                            formdata = pdf['data']
                            for f in formdata:
                                record['data'][f] = formdata[f]
                                if record['data'][f] != "":
                                    blank = False
                            if blank:
                                record['error_code'] = "400"
                                record['error_msg'] = "PDF is blank."
                            info = KineticPdf.get_pdf_metadata(j)
                            if info['error_code'] == "0":
                                if info['data']['Author']:
                                    record['data']['pdf_author'] = info['data']['Author']
                                if info['data']['Title']:
                                    record['data']['pdf_title'] = info['data']['Title']

                            processed.append(record)
                        else:
                            record['error_code'] = pdf['error_code']
                            record['error_msg'] = pdf['error_msg']
                            processed.append(record)
                else:
                    record['error_code'] = "404"
                    record['error_msg'] = "Email has no attachments"
                    processed.append(record)

            return {"error_code": "0", "error_msg": "", "data": processed}
        else:
            return list
