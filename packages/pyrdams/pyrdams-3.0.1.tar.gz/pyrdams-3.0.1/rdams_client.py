...
def get_parser():
    ...
    parser.add_argument('-token', '-t',
                        type=str,
                        dest='token',
                        help='Token, required if -store_credentials option is activated',
...
def download(request_idx, out_dir='./'):
    ...
    if not ret:
        LOGGER.error('Get filelist returned and empty response!')
        return ret
    else:
        if 'data' in ret.keys():
            if len(ret['data']) == 0:
                LOGGER.warning('Returned data field is empty')
                return ret
            try:
                filelist = ret['data']['web_files']
            except KeyError:
                LOGGER.error('Cannot extract filelist. Response content:\n%s',
                             json.dumps(ret))
                return ret
        else:
            LOGGER.error('data field is not present in the response. Response content:\n%s',
                         json.dumps(ret))
    ...
    try:
        ...
        LOGGER.error('Cannot download files from the following filelist:\n%s',
                     filelist)
    return ret
