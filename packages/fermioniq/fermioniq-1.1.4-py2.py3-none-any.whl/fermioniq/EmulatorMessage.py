...
class EmulatorMessage(BaseModel):
    ...
    Attributes:
        message_type (Literal["event"]): The type of the message, always "event" for EmulatorMessage.
        event_type (Literal['LOG', 'STARTED', 'FINISHED']): The type of event associated with the message.
            - 'LOG': A log message from the job.
            - 'STARTED': The job has started.
            - 'FINISHED': The job has finished.
        ts (str): The timestamp of the event in UTC (ISO 8601 format).
        job_id (str): The unique ID of the EmulatorJob.
        job_output (str): The output associated with the event (e.g., log content).
        job_status_code (int): The status code of the job execution.
