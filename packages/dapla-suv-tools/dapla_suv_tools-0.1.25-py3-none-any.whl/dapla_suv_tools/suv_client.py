...
class SuvClient:
    ...
    def logs(self, treshhold: str | None = None) -> list:

        if not treshhold or treshhold not in constants.LOG_LEVELS:
            treshhold = constants.LOG_INFO

        log = self._filter_logs(treshhold=treshhold)
    ...
    def _filter_logs(self, treshhold: str) -> list:
        limit = constants.LOG_LEVELS.index(treshhold)
