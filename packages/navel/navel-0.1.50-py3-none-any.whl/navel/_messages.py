...
from navel._async import AsyncRunner, task
...
class MotionMessenger(AsyncRunner):
    ...
    async def move_base(
        ...
        v = int(math.copysign(speed, distance) * 10000)  # API wants µm / 100 s
    ...
    async def rotate_base(
        ...
        magic = 7  # FIXME: empirically determined conversion rate
        v = int(math.copysign(speed, angle) * magic)
    ...
    async def move_and_rotate_base(
        ...
        Args:
            distance (float): Motion magnitude and direction in meters.
            lspeed (float): Desired peak movement speed (deg/s).
            lacc (float): Desired peak acceleration (deg/s²).
            angle (float): Rotation magnitude and direction in degrees.
        ...
        t_c, t_a = _calc_motion_times(distance, speed, acceleration)
        magic = 240  # FIXME: empirically determined conversion rate
        vl = int(math.copysign(speed, distance) * magic)

        acc_time = t_a / 100
        const_time = t_c / 100
        spd = angle / (const_time + acc_time)  # should be the peak speed
        magic = 7 + 70 * ((speed / 0.25) ** 1.5)  # bigger when faster
        vr = int(math.copysign(spd, angle) * magic)
