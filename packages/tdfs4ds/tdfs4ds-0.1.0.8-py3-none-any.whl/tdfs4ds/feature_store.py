...
def get_feature_versions(entity_name, features, domain=None, latest_version_only=True, version_lag=0):
    ...
    for i, row in df.iterrows():
        ...
        FROM (
        CURRENT VALIDTIME
        SELECT '{row['FEATURE_NAME']}' AS FEATURE_NAME, FEATURE_VERSION FROM {row['FEATURE_DATABASE']}.{row['FEATURE_TABLE']}
    ...
    return df.set_index('FEATURE_NAME').to_dict()['FEATURE_VERSION']