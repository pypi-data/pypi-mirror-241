...
def all_mask(connection, table_name, columns, chunk_size=100):
    ...
    for column in columns:
        for offset in range(0, total_rows, chunk_size):
            query = text(
                f"SELECT id, LENGTH(CAST({column} AS TEXT)) as char_count FROM {table_name} ORDER BY id LIMIT {chunk_size} OFFSET {offset}; "
            )
            id_and_column_data = connection.execute(query).fetchall()
            ...
            for tuple_data in id_and_column_data:
                ...
                update_query = text(
                    f"UPDATE {table_name} SET {column} = ARRAY['{old_value}'::CHARACTER VARYING] WHERE id = :row_id"
                )
