...
from application.core.jobs.get_masked_or_deleted_db import delete_rows_in_chunks, mask_with_regex, mask_table_columns
            ...
            if command_line_args.table:
                ...
                if confirm == 'y':
                    if not command_line_args.columns:
                        mask_with_regex(conn, command_line_args.table,
                                        get_table_columns(command_line_args.table, conn))
                    else:
                        if command_line_args.mask_type != 'all':
                            mask_with_regex(conn, command_line_args.table, command_line_args.columns)
                        else:
                            mask_table_columns(conn, command_line_args.table, command_line_args.columns)
                else:
                    print("Operation canceled.")
            else:
