...
@dataclass
class LLMConfig:
    """LLM Configuration model class. It is also useful to configure a
    ...
    :param hf_model_id: The HuggingFace model.
    :param endpoint: The model endpoint to interact with.
    :param type: The model type (base/instruct/chat, custom)
    :param engine: The engine used by the deployments (hf/mpi)
    :param stream_config: Whether the deployment if configured for streaming.
    :param custom_attrs: Dict of custom attributes to send to the LLM. Defaults
                         to None.
    :param inference_access_path: Used to configure a custom LLM. It tells how
                                  access the json model response.
    :param payload_inference_format: Used to configure a custom LLM. It tells
                                     how to structure json input before
                                     sending it to the model container.
    """
    name: str
    hf_model_id: str
    endpoint: str
...
class LLM:
    """A client that connects to a proprietary inference server, facilitating
    ...
    def __init__(self,
        ...
        if not self.is_server_up(self.hostname):
    ...
    @__auto_reauthenticate__
    def read_config(self, config_name: str | None = None) \
            -> List[LLMConfig] | LLMConfig:
    ...
    def update_config(self,
                      ...
                      new_config: LLMConfig | None = None,
                      **kwargs):
    ...
    def justify_query(self,
        ...
        response = requests.post(req_uri,
                                 headers={"token": self.session_token},
                                 json=body)
        self.___validate__(response)
    ...
    def __stream_generator__(self, body: str, uri: str) \
