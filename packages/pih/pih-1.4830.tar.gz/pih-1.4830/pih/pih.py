...
class PIH:
    ...
    class RESULT(ResultTool):
        ...
        class NOTES:
            ...
            @staticmethod
            def files(name: str | None = None) -> Result[list[Note]]:
                return ResultTool.filter(
                    lambda item: True if n(name) else item.title.find(name) != -1,
                    PIH.RESULT.NOTES.by_label(CONST.NOTES.FILE_SECTION),
                )
    ...
    class CARD_REGISTRY:
        ...
        def set_folder_for_person(
            ...
            if set_by_polibase:
                ...
                PIH.ACTION.POLIBASE.update_person_change_date(person, registrator_person)
        ...
        def set_folder_for_person(
            ...
            result: bool = (
                e(value)
                or already_set
                or PIH.ACTION.POLIBASE.set_card_registry_folder(
                    value, person, registrator_person
                )
            )
            event_builder = PIH.EVENT.BUILDER.polibase_person_set_card_registry_folder
            if result:
                if e(value):
                    ...
                    if nn(event_ds):
                        ...
                        allow_add = already_set or card_registry_folder != value
                        if allow_add:
                            PIH.ACTION.EVENTS.remove(
                                *event_builder(card_registry_folder, person)
                            )
                        if already_set:
                            PIH.EVENT.send(
                                *event_builder(value, person, registrator_person)
    ...
    class ACTION:
        ...
        class EVENTS:
            ...
            def register(
                ...
                if remove_before:
                    parameters_for_remove: disct[str, Any] | None = parameters
                    ...
                    """if PIH.DATA.FORMAT.polibase_person_card_registry_folder(
                        ...
                        if ResultTool.is_empty(
                            ...
                            PIH.CARD_REGISTRY.set_folder_for_person(
                                value, person, registrator_person, True
                            )
                            return False
                        else:
                            return False"""
