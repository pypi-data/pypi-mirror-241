...
from .loops import StopLoopException, LoopException, TensorTuple, flatten_dict, default_loss, concat_dicts, to_numpy, ModelWrapper, LossWrapper
            ...
            for batch_idx, batch_data in enumerate(self.dataloader):
                ...
                if self.stage == 'train':
                    with torch.enable_grad():
                        m_value = step_method(batch_x, batch_y, metrics=metrics, **self.step_args)
                else:
                    m_value = step_method(batch_x, batch_y, metrics=metrics, **self.step_args)
...
class Trainer(TrainerBase):
    ...
    def evaluate_step(self,
        ...
        loss = self.loss(model_out, batch_y)
        results = {}
        if loss is not None:  # 如果task不需要计算损失，则loss为None
            results = {'loss': ValuePatch(loss.detach(), len(model_out))}
