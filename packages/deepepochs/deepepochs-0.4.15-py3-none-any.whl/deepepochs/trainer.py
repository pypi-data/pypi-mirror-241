...
from .loops import StopLoopException, LoopException, TensorTuple, flatten_dict, default_loss, concat_dicts, to_numpy, ModelWrapper
            ...
            for batch_idx, batch_data in enumerate(self.dataloader):
                ...
                if self.stage == 'train':
                    with torch.enable_grad():
                        m_value = step_method(batch_x, batch_y, metrics=metrics, do_loss=True, **self.step_args)
                else:
                    m_value = step_method(batch_x, batch_y, metrics=metrics, do_loss=self.do_loss, **self.step_args)
