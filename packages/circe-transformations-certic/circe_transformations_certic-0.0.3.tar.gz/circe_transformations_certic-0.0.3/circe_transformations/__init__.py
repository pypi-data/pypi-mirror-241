...
def pdf_to_png(working_dir: str, logger: logging.Logger, options: dict = None):
    nb_of_files_treated = 0
    logger.info("Looking for *.pdf files...")
    for file_path in _find_files("*.pdf", working_dir):
        logger.info("Convert {} to PNG".format(os.path.basename(file_path)))
        destination_file = "{}.png".format(file_path)
        return_code = call(["convert", file_path, destination_file])
        if return_code != 0:
        ...
        os.remove(file_path)
        nb_of_files_treated += 1
    logger.info(DONE_MESSAGE.format("PDF to PNG", nb_of_files_treated))


pdf_to_png.description = {
    "label": "PDF to PNG",
    "help": "Conversion image .pdf vers .png en utilisant convert (imagemagick)",
    "options": [],
}
