...
"""Module for BedGraphV2 data class.
...
cdef class bedGraphTrackII:
    ...
    Difference between bedGraphTrackII and bedGraphTrackI: 
       the data in bedGraphTrackI for each chromosome is saved in a list of two python arrays as [array, array]
       the data in II for each chromosome is saved in a 2D numpy array dtype=[ ( 'p', 'uint32' ), ( 'v', 'float32' ) ]

    Also, we limit the way to create the object -- we simply attach pv array from pileupV2.
    """
    cdef:
        dict __data
        float32_t maxvalue
    ...
    cpdef add_chrom_data( self, bytes chromosome, np.ndarray pv_array ):
        """Add a pv data to a chromosome. Replace the previous data.

        pv_array is a numpy ndarray with dtype=[ ( 'p', 'uint32' ), ( 'v', 'float32' ) ]
        """
        cdef:
            float32_t maxv, minv

        self.__data[ chromosome ] = pv_array
        maxv = pv_array['v'].max()
        minv = pv_array['v'].min()
    ...
    cpdef list get_data_by_chr (self, bytes chromosome):
        ...
        The return value is a pv array:
        """
        if chromosome in self.__data:
    ...
    cdef __get_next_func__ ( self, bytes chromosome ):
        return iter(self.__data[chromosome]).__next__

    cdef __adjust_len__ ( self, bytes chromosome, long l ):
        self.__data[chromosome].resize( l, refcheck = False )
    ...
    cpdef void write_bedGraph (self, fhd, str name, str description, bool trackline=True):
        ...
        chrs = self.get_chr_names()
        for chrom in chrs:
            get_next = self.__get_next_func__( chrom )
            pre = 0
    ...
    cdef merge_regions (self):
        ...
        for chrom in chrs:
            get_next = self.__get_next_func__( chrom )
            # new arrays
            new_pv_array = np.zeros( self.__data[chrom].shape[0], dtype= [ ( 'p', 'uint32' ), ( 'v', 'float32' ) ] )
            new_pv_l = 0
            new_pv_array[ new_pv_l ] = get_next()

            [ new_pre_pos, new_pre_value ] = new_pv_array[ 0 ]

            for i in range( 1, self.__data[chrom].shape[0] ):
                [ pos, value ] = get_next()
                if value == new_pre_value:
                    new_pos[ new_pv_l ][ 'p' ] = pos
                else:
                    # add new region
                    new_pv_l += 1
                    new_pv_array[ new_pv_l ] = [ pos, value ]
                    new_pre_pos = pos
                    new_pre_value = value
            new_pv_array.resize( new_pv_l+1, refcheck=False )
            self.__data[chrom] = new_pv_array
        return True
    ...
    cpdef bool filter_score (self, float32_t cutoff=0):
        ...
        for chrom in chrs:
            ...
            new_pre_pos = 0
            new_pre_value = 0
            for i in range( self.__data[chrom].shape[0] ):
                [ pos, value ] = get_next()
                ...
                if value < cutoff:
                    ...
                    if new_pre_value == self.baseline_value:
                        ...
                        new_pv_array[ new_pv_l ][ 'p' ] = pos
                    else:
                        # else add a new baseline region
                        new_pv_array[ new_pv_l ] = [ pos, self.baseline_value ]
                        new_pv_l += 1
                else:
