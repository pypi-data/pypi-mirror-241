...
def calc (
	...
	print ()
	print ('$ procedure: calc (the s2_tf1) essential ingredients')
	...
	for product in products:	
		...
		product_details ["product sequential"] = product_count;
	...
	for product in products:
		...
		product_details = product [0]
		packets_count = product [1]
	
		quantified_grove_aggregator.calc (
			recipe_struct_grove,
			product_details ["ingredients"]["quantified grove"],
			product_details,
			packets_count
		)
