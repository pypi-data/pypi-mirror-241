...
def convert_aq_timestamp(ts):
    ...
    if ts_sub[:4] == "0001":
        ...
        return None
    return pd.Timestamp(datetime.strptime(ts_sub, "%Y-%m-%d%H:%M:%S.%f"))
...
class DEWAquarius:
    ...
    def fetch_timeseries_points_only(self, ts_uid):
    ...
    def fetch_timeseries_metadata_only(self, ts_uid):
        ...
        response = self.ts.publish.get(
            "GetTimeSeriesCorrectedData",
            params={
                # "TimeSeriesUniqueId": "ad95f3430cb64dc5affcbb3ca34a8bf0",
                # "TimeSeriesUniqueId": "a46bb4818ad546e5893b5f7eeca3d940",
                "TimeSeriesUniqueId": ts_uid,
        ...
        for key, df in dfs.items():
            min_dt = df[["StartTime", "EndTime"]].min().min()
            max_dt = df[["StartTime", "EndTime"]].max().max()
    ...
    def fetch_timeseries_data(
        ...
        **kwargs,
        ...
        Args:
            ...
            param (str, optional): parameter, default "Depth to Water"
        ...
        Returns:
            pandas.DataFrame with columns "timestamp" (pd.Timestamp),
            "grade" (int), "qualifier" (str), and "approval" (str).
            The data itself is also included - the column heading is
        ...
        meta = self.fetch_timeseries_metadata_only(ts_uid)
        pts = self.fetch_timeseries_points_only(ts_uid)
        if len(pts) == 0:
            if "param" in meta:
                return pd.DataFrame(
                    columns=[
