...
class user():  
    ...
    def send_account_status_report(self):
        ...
        import numpy as np
        history_summary['account value'] = history_summary.apply(lambda x: round(x['account value'],2) if np.isnan(x['account value']) != True else np.nan, axis = 1)  
        history_summary['asset value'] = history_summary.apply(lambda x: round(x['account value'],2) if np.isnan(x['asset value']) != True else np.nan, axis = 1)  
        history_summary['cash position'] = history_summary.apply(lambda x: round(x['cash position'],2) if np.isnan(x['cash position']) != True else np.nan, axis = 1)  
        history_summary['margin position'] = history_summary.apply(lambda x: round(x['margin position'],2) if np.isnan(x['margin position']) != True else np.nan, axis = 1)  
        history_summary['Profit/Loss'] = history_summary.apply(lambda x: round(x['Profit/Loss'],2) if np.isnan(x['Profit/Loss']) != True else np.nan, axis = 1)  
...
if __name__ == '__main__':
    ...
    me = user(site=mysite, user_name='lishuan')
