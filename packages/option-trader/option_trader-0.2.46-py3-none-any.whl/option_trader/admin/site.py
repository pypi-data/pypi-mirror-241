    ...
    def send_site_user_account_report(self):
        ...
        for user_name in user_name_list:
            ...
            for account_name in account_name_list:
                ...
                unrealized_pl, realized_pl = a.get_pl()
                open_date = a.get_open_date()        

                o = a.get_position_summary()                
                tc = o.shape[0]        
                tc_win_pl = o[o['gain']>0]['pl'].sum() if tc > 0 else 0
                tc_loss_pl = o[o['gain']<0]['pl'].sum() if tc > 0 else 0        
                tc_win_rate = 100*o[o['gain']>0].shape[0]/tc if tc > 0 else 0       
                oo = o[o['status']=='opened']
                oc = oo.shape[0]
                oc_win_pl = oo[oo['gain']>0]['pl'].sum() if oc > 0 else 0
                oc_loss_pl = oo[oo['gain']<0]['pl'].sum() if oc > 0 else 0            
                oc_win_rate = 100*oo[oo['gain']>0].shape[0]/oc if oc > 0 else 0             
                oo['max risk'] = oo.apply(lambda x: x['max_loss'] * x['quantity'] * 100 if x['strategy'] != 'Unpaired' else x['open_price'], axis = 1)  
                oo_max_risk = oo['max risk'].sum()
                co = o[(o['status']=='closed') | (o['status']=='expired')]
                cc = co.shape[0]
                cc_win_pl = co[o['gain']>0]['pl'].sum() if cc > 0 else 0
                cc_loss_pl = co[o['gain']<0]['pl'].sum() if cc > 0 else 0            
                cc_win_rate = 100*co[co['gain']>0].shape[0]/cc if cc > 0 else 0      
                print('{:10s}\t{:10s}\t{:10s}\t{:10s}\t{:10s}\t{:10s}'.format( 'user', 'account', 'open date', 'init balance', 'max risk', 'risk ratio') )         
                print('{:10s}\t{:15s}\t{:10s}\t{:10s}\t{:10s}\t{:.2f}%\r\n'.format(u.user_name, a.account_name, str(open_date), str(round(initial_balance,2)), str(round(oo_max_risk,2)), 100*oo_max_risk/acct_value))    
                pl = realized_pl+unrealized_pl #acct_value-initial_balance
                ...
                tc_str = '[%d|%.2f%%|$%.0f|$%.0f]' % (tc, tc_win_rate, tc_win_pl, tc_loss_pl)
                oc_str = '[%d|%.2f%%|$%.0f|$%.0f]' % (oc, oc_win_rate, oc_win_pl, oc_loss_pl)
                cc_str = '[%d|%.2f%%|$%.0f|$%.0f]' % (cc, cc_win_rate, cc_win_pl, cc_loss_pl)                                                    
                print('{:30s}\t{:30s}\t{:30s}'.format('tatal|win%|w.pl|l.pl', 'opened|win%|w.pl|l.pl', 'closed|win%|w.pl|l.pl') )                
                print('{:30s}\t{:30s}\t{:30s}\r\n'.format(tc_str, oc_str, cc_str) )        

                print('{:10s}\t{:10s}'.format( 'Realized PL', 'Unealized PL') )         
                print('${:.2f}\t${:.2f}\r\n'.format(realized_pl, unrealized_pl))    
