...
def handle_help(self, arguments):
    ...
    utils.display_markdown_message("".join(full_message))


def handle_debug(self, arguments=None):
    if arguments == "" or arguments == "true":
        utils.display_markdown_message("> Entered debug mode")
        print(self.messages)
        self.debug_mode = True
    elif arguments == "false":
        utils.display_markdown_message("> Exited debug mode")
        self.debug_mode = False
    else:
        utils.display_markdown_message("> Unknown argument to debug command.")


def handle_reset(self, arguments):
    self.reset()
    utils.display_markdown_message("> Reset Done")


def default_handle(self, arguments):
    utils.display_markdown_message("> Unknown command")
...
def handle_save_message(self, json_path):
    ...
    utils.display_markdown_message(f"> messages json export to {os.path.abspath(json_path)}")
...
def handle_count_tokens(self, prompt):
    ...
    if len(self.messages) == 0:
        (conversation_tokens, conversation_cost) = utils.count_messages_tokens(
    ...
    if prompt:
        (prompt_tokens, prompt_cost) = utils.count_messages_tokens(
