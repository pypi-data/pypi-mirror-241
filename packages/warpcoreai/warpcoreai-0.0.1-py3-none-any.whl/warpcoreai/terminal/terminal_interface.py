...
from .blocks.message import Message
...
def terminal_interface(interpreter, message):
    ...
    if not interpreter.auto_run and not interpreter.local:
        ...
        if interpreter.safe_mode == "ask" or interpreter.safe_mode == "auto":
            if not utils.check_package("semgrep"):
    ...
    while True:
        ...
        if True:  ################## interpreter.vision:
            ...
            image_path = utils.find_image_path(message)
        ...
        try:
            for chunk in interpreter.chat(message, display=False, stream=True):
                ...
                if "executing" in chunk:
                    if not interpreter.auto_run:
                        ...
                        if should_scan_code:
                            ...
                            utils.scan_code(code, language, interpreter)
                ...
                if "image" in chunk or "html" in chunk or "javascript" in chunk:
                    ...
                    message_for_llm = utils.display_output(chunk)
                    if message_for_llm:
                ...
                if "output" in chunk:
                    ...
                    # Truncate output
                    active_block.output = utils.truncate_output(
