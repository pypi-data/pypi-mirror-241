...
from ..terminal.conversation_navigator import conversation_navigator
...
def cli(interpreter):
    ...
    for arg in arguments:
        ...
        if arg['type'] == bool:
            ...
            kwargs['choices'] = arg.get('choices')
            kwargs['default'] = arg.get('default')

        parser.add_argument(f'-{arg["nickname"]}', f'--{arg["name"]}', **kwargs)
    ...
    if args.config:
        if args.config_file:
            config_file = utils.get_config_path(args.config_file)
        else:
            config_file = utils.get_config_path()
...
"""
    ...
    for attr_name, attr_value in vars(args).items():
        ...
        if attr_value is not None and hasattr(interpreter, attr_name):
            ...
            if attr_name == "config_file":
                user_config = utils.get_config_path(attr_value)
    ...
    if args.version:
        version = utils.pkg_resources.get_distribution("open-interpreter").version
    ...
    if args.vision:
        ...
        utils.display_markdown_message("> `Vision` enabled **(experimental)**\n")
