...
class CheckAndCreateSuperUserConsumer(LifecycleConsumer):
    ...
    def on_event(self) -> None:
        ...
        if not (settings.ADMIN_USER_EMAIL and settings.ADMIN_USER_PASSWORD):
            logger.info('Email / password missing for super user - skipping')
            return

        user = (
            User.objects.filter(email=settings.ADMIN_USER_EMAIL, _address__object_version=Versions.LATEST)
            .get_or_none()
            .execute()
        )
        if user is not None:
...
class AuthenticateUserConsumer(LifecycleConsumer):
    def on_event(self, auth_header: str, authentication_info: Any) -> None:
        ...
        email: str | None = None
        try:
            jwt_payload = jwt.decode(auth_header, settings.AUTH_JWT_KEY, algorithms=['HS256'])  # type: ignore[arg-type]
