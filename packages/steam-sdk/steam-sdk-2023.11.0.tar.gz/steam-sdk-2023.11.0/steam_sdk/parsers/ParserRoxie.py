                ...
                for row in parameters:
                    if data.groups:
                        for group in data.groups:
                            if int(row.split()[0]) in data.groups[group].blocks:
                                block_symm = data.groups[group].symm
                                break
                    else:
                        block_symm = 1
                    layers[block_symm].append(float(row.split()[3 if int(row.split()[1]) == 1 else 4]))
                for symm in concentric_coils:
            ...
            for row in parameters:
                ...
                if key == 'group':
                    ...
                    if not data.groups:
                        ...
                        layer = layers[1].index(radius if block_type == 1 else phi) + 1
                        if imag == 1:
                                ...
                                if self.shift_flag <= 1 and len(concentric_coils) == 1:
                                    ...
                                    for trans in data.transs:
                                        if data.transs[trans].string == 'SHIFT2' and (data.transs[trans].act == 1 and int(group) in data.transs[trans].bcs or data.transs[trans].act == 3):
                                            ...
                                            break
                                layer = layers[data.groups[group].symm].index(radius if block_type == 1 else phi) + 1
