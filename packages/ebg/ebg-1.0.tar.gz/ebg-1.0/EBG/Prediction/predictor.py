            ...
            with warnings.catch_warnings():
                ...
                for index, row in features.iterrows():
                    ...
                    entropy_row_70 = entropy([row["prediction_bs_over_70"], 1 - row["prediction_bs_over_70"]],
                                             base=2)
                    features.loc[index, 'prediction_entropy_bs_over_80'] = entropy_row_80
                    features.loc[index, 'prediction_entropy_bs_over_85'] = entropy_row_85
                    features.loc[index, 'prediction_entropy_bs_over_75'] = entropy_row_75
                    features.loc[index, 'prediction_entropy_bs_over_70'] = entropy_row_70

        self.logger.info("Finished prediction!")

        features["dataset"] = self.output_prefix
        features.to_csv(os.path.join(self.current_directory, self.output_prefix, self.output_prefix + ".csv"))
        self.logger.info("Stored features and prediction at: " + self.output_prefix)

        self.prediction = features

    def map_results(self):
        classification_results = ["bs_over_70", "bs_over_75", "bs_over_80", "bs_over_85"]
            ...
            if value in regression_results:
                ...
                for node in current_tree.traverse():
                    if not node.is_leaf():
                        ...
                        if not matching_row.empty:
                            node.__setattr__("support", matching_row.iloc[0]['prediction_' + value])
                            node.add_feature("uncertainty", matching_row.iloc[0]['prediction_entropy_' + value])
                current_tree.write(outfile=os.path.join(self.current_directory, self.output_prefix,
                                                        self.output_prefix + "_" + value + "_support_prediction.newick"),
