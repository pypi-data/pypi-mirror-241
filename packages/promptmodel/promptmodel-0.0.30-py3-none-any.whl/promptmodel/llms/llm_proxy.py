...
class LLMProxy(LLM):
    ...
    def _wrap_chat(self, method: Callable[..., Any]) -> Callable[..., Any]:
        def wrapper(inputs: Dict[str, Any], **kwargs):
            chat_uuid = kwargs.get("chat_uuid", None)
            new_messages = kwargs.get("new_messages", [])
            function_list = kwargs.get("function_list", None)
            use_conv_log = kwargs.get("use_conv_log", True)
            ...
            if use_conv_log and not is_new_chat:
                run_async_in_sync(fetch_chat_model_conversation(chat_uuid))
            return
            # prompts, version_details = run_async_in_sync(fetch_prompts(self._name))
            # call_args = self._prepare_call_args(
            #     prompts, version_details, inputs, kwargs
