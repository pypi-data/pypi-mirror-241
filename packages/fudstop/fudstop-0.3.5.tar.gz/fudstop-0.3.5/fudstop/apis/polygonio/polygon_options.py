...
class PolygonOptions:
    def __init__(self, connection_string=None):
        ...
        self.user = os.environ.get('postgres')
        self.password = os.environ.get('fud')
    ...
    async def get_all_options(self,ticker):
        async with self.sema:
            options = await self.get_option_chain_all(ticker)
            df = pd.DataFrame(options.data_dict)
            print(type(options.data_dict))
            
            await self.batch_insert_dataframe(df, 'options_data', unique_columns='option_symbol')
    
        

    async def run_all_options(self):
        await self.connect()
        try:
            tasks = [self.get_all_options(i) for i in most_active_tickers]
