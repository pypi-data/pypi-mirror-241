...
class IUV:
    ...
    def abort(self, msg=None):
        ...
        DebugLogger.line('IUV: iuv.abort() called')
        DebugLogger.line(f'IUV: iuv.abort() msg: {msg}')
        DebugLogger.line('IUV: exiting testcase (ignore next "SKIPPED")')
    ...
    def _log(self, tag, val=None, total=None):
        ...
        if val is None:
            DebugLogger.line(tag)
        elif total is None:
            DebugLogger.line(f'   {tag: <25} {val: >5}')
        else:
            DebugLogger.line(f'   {tag: <25} {val: >5} {round(val / total * 100.0, 0):>5}%')
