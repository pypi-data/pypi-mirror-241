...
import pickle
from typing import Any, Callable, Dict, List, Optional, Set, Type, Iterator
...
class GeneratorMemoryObject(MemoryObject):
    strategy: str = "generator"
    items: List[PythonId]

    MAX_SIZE = 1_000

    def __init__(self, generator_object: object) -> None:
        self.items = []
        super().__init__(generator_object)

    def initialize(self) -> None:
        self.obj: typing.Generator
        serializer = PythonSerializer()
        ...
        comparable = all(
            serializer.get_by_id(elem_id).comparable
            for elem_id in self.items
        )

        super()._initialize(iter(items), comparable)
...
class GeneratorMemoryObjectProvider(MemoryObjectProvider):
    @staticmethod
    def get_serializer(obj: object) -> Optional[Type[MemoryObject]]:
        if inspect.isgenerator(obj) or type(obj) == map or isinstance(obj, Iterator):
            return GeneratorMemoryObject
        return None
