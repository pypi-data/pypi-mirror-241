...
class VanillaCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # (400, 161)
        return encoded
class VanillaCNN_DoG(nn.Module):
    def __init__(self,numoffea=48):
        super().__init__()
    ...
    def forward(self, x): # input x, (400, 2, 36, 64)
        ...
        return encoded
class VanillaCNN_one_DoG_fixOne(nn.Module):
    def __init__(self,numoffea=48):
        super().__init__()
        self.numoffea=numoffea
        # the ones-tensor
        ...
        tmp_gau = torch.zeros((1,1,5,5));tmp_gau[0,0] = cal_gau(5,1.0,0)
        self.gaussian_kernel_2d = tmp_gau#.to('cuda')
        self.inhib_alpha1 = nn.Parameter(torch.ones((self.numoffea,1)));self.inhib_alpha1.requires_grad = False
        # self.inhib_alpha2 = nn.Parameter(torch.ones((self.numoffea,1)))
        self.sizeoffea=22*50 # size of feature
        self.numofneuron= 161# test_loader[1].shape[1] # number of neurons
