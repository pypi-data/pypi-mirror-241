...
class AccessToken:
    def __init__(self, account_id, access_token, token_expiry, refresh_token):
...
"""
class Authenticate:
    def __init__(self, db_path=None, log_level='INFO', application_settings=None, account_id=None) -> object:

        if application_settings is None:
            raise AttributeError('application_settings is necessary for this to run.')
    ...
    def login(self, open_browser_if_available=True):
        token_data = self._start_authentication_flow(open_browser_if_available=open_browser_if_available)
        ...
        login_name = self._token_response_to_db_insert(token=token_data)
    ...
    def refresh_credentials_now(self) -> bool:
        ...
        new_token = self._refresh_token_using_refresh_token(refresh_token=token.refresh_token)
        self._del_data_by_account_id()
        self._token_response_to_db_insert(token=new_token)
        return True
    ...
    def _init_default_db(self, force=True):
        ...
        create_statement = "CREATE TABLE \"access_tokens\" (account_id TEXT PRIMARY KEY, access_token TEXT, token_expiry TIMESTAMP, refresh_token TEXT, id_token TEXT, debug_response TEXT)"
    ...
    def _get_db_token(self):
        select_statement = f"SELECT account_id, access_token, token_expiry, refresh_token from access_tokens where account_id = \'{self.account_id}\' COLLATE NOCASE"
    ...
    def _token_response_to_db_insert(self, token: dict) -> str:
        ...
        insert_statement = "INSERT INTO access_tokens (account_id, access_token, token_expiry, refresh_token, id_token, debug_response) values (?, ?, ?, ?, ?, ?)"
    ...
    def _start_authentication_flow(self, open_browser_if_available=True) -> str:
        ...
        REQUEST_SCOPES = self.application_settings.get('request_scopes')
    ...
    def _refresh_token_using_refresh_token(self, refresh_token: str) -> dict:
        CLIENT_ID = self.application_settings.get('client_id')
        AUTHORITY = self.application_settings.get('authority')
        REQUEST_SCOPES = self.application_settings.get('request_scopes')
        app_public = msal.PublicClientApplication(client_id=CLIENT_ID, authority=AUTHORITY)
