from constants import ERROR_MESSAGE, ERROR_CODE, PAYOS_BASE_URL
from utils import createSignatureFromObj, createSignatureOfPaymentRequest
from custom_error import PayOSError 
import requests
...
class PayOS:
    ...
    def createPaymentLink(self, paymentData):
        ...
        if None in [orderCode, amount, description, items, cancelUrl, returnUrl]:
            required_payment_data = {"orderCode": orderCode, "amount": amount, "returnUrl": returnUrl, "cancelUrl": cancelUrl, "description": description}
            required_keys = list(required_payment_data.keys())
            keys_error = [key for key in required_keys if required_payment_data[key] is None]
            msg_error = f"{ERROR_MESSAGE['INVALID_PARAMETER']} {', '.join(keys_error)} must not be None."
        ...
        paymentLinkRes = requests.post(url, json=paymentData, headers=headers)
