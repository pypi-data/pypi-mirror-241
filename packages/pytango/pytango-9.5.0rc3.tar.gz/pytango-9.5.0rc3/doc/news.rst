...
- Support Tango server debugging with PyCharm, PyDev and VS Code.  Breakpoints now work for command and attribute
  ...
  is based on `pydevd <https://pypi.org/project/pydevd>`_.  If necessary, the feature can be disabled by setting the
  environment variable ``PYTANGO_DISABLE_DEBUG_TRACE_PATCHING=1``.
