...
class RedisClient:
    ...
    def list_rpush(self, key, value, expiry=None):
        serialized_value = pickle.dumps(value)
        res = self.redis_client.rpush(key, serialized_value) #return the length of the list
    ...
    def sadd(self, key, value, expiry=None):
        serialized_value = pickle.dumps(value)
        res = self.redis_client.sadd(key, serialized_value) #return 1 if item was added, 0 is not
    ...
    def srem(self, key, value):
        serialized_value = pickle.dumps(value)
        return self.redis_client.srem(key, serialized_value) #return 1 if item was removed, 0 is not
