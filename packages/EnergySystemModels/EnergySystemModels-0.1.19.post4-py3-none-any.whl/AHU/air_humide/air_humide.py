...
def RH(w, P=101325.0, P_sat=None, T_db=None):
    if P_sat is None:
        if T_db is None:
...
def T_sat(HA_target):
	...
	T = -100
	Erreur = HA(P_sat(T), 100) - HA_target
...
#Temperature_Melange = T_Enthalpie_Ha((Enthalpie(T1, (HA(P_sat(T1), HR1))) * (m1 / (1 + (HA(P_sat(T1), HR1)) / 1000)) + (Enthalpie(T2, (HA(P_sat(T2), HR2)))) * (m2 / (1 + (HA(P_sat(T2), HR2)) / 1000))) / ((m2 / (1 + (HA(P_sat(T2), HR2)) / 1000)) + (m1 / (1 + (HA(P_sat(T1), HR1)) / 1000))), 1000 * (((m2 / (1 + (HA(P_sat(T2), HR2)) / 1000)) * ((HA(P_sat(T2), HR2)) / 1000)) + ((m1 / (1 + (HA(P_sat(T1), HR1)) / 1000)) * ((HA(P_sat(T1), HR1)) / 1000))) / ((m2 / (1 + (HA(P_sat(T2), HR2)) / 1000)) + (m1 / (1 + (HA(P_sat(T1), HR1)) / 1000))))
#return
#def HA_Melange(m1, T1, HR1, m2, T2, HR2)

#HA_Melange = 1000 * (((m2 / (1 + (HA(P_sat(T2), HR2)) / 1000)) * ((HA(P_sat(T2), HR2)) / 1000)) + ((m1 / (1 + (HA(P_sat(T1), HR1)) / 1000)) * ((HA(P_sat(T1), HR1)) / 1000))) / ((m2 / (1 + (HA(P_sat(T2), HR2)) / 1000)) + (m1 / (1 + (HA(P_sat(T1), HR1)) / 1000)))
#return
