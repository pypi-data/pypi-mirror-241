...
# Tk = absolute temperature, K = °C + 273.15
	...
	if T_db < 0 : #valable entre -100 et 0 °C
		...
		P_sat = exp(C1 / Tk + C2 + C3 * Tk + C4 * Tk ** 2 + C5 * Tk ** 3 + C6 * Tk ** 4 + C7 * log(Tk))
	else :  #valable entre 0 et 200 °C

		P_sat = exp(C8 / Tk + C9 + C10 * Tk + C11 * Tk ** 2 + C12 * Tk ** 3 + C13 * log(Tk))
...
def func_RH(w, P=101325.0, P_sat=None, T_db=None):
    if P_sat is None:
        if T_db is None:
            raise ValueError("Either P_sat or T_db must be provided")
        else:
            P_sat = func_P_sat(T_db)
...
def func_w( RH, P=101325.0,P_sat=None,T_db=None):
	if P_sat is None:
		if T_db is None:
			raise ValueError("Either P_sat or T_db must be provided")
		else:
	...
	if Pv<=P:
		...
		w =None
		raise ValueError("Oversaturated case")
...
def T_sat(HA_target):
	...
	T = -100
	Erreur = HA(func_P_sat(T), 100) - HA_target
...
#Temperature_Melange = T_Enthalpie_Ha((Enthalpie(T1, (HA(func_P_sat(T1), HR1))) * (m1 / (1 + (HA(func_P_sat(T1), HR1)) / 1000)) + (Enthalpie(T2, (HA(func_P_sat(T2), HR2)))) * (m2 / (1 + (HA(func_P_sat(T2), HR2)) / 1000))) / ((m2 / (1 + (HA(func_P_sat(T2), HR2)) / 1000)) + (m1 / (1 + (HA(func_P_sat(T1), HR1)) / 1000))), 1000 * (((m2 / (1 + (HA(func_P_sat(T2), HR2)) / 1000)) * ((HA(func_P_sat(T2), HR2)) / 1000)) + ((m1 / (1 + (HA(func_P_sat(T1), HR1)) / 1000)) * ((HA(func_P_sat(T1), HR1)) / 1000))) / ((m2 / (1 + (HA(func_P_sat(T2), HR2)) / 1000)) + (m1 / (1 + (HA(func_P_sat(T1), HR1)) / 1000))))
#return
#def HA_Melange(m1, T1, HR1, m2, T2, HR2)

#HA_Melange = 1000 * (((m2 / (1 + (HA(func_P_sat(T2), HR2)) / 1000)) * ((HA(func_P_sat(T2), HR2)) / 1000)) + ((m1 / (1 + (HA(func_P_sat(T1), HR1)) / 1000)) * ((HA(func_P_sat(T1), HR1)) / 1000))) / ((m2 / (1 + (HA(func_P_sat(T2), HR2)) / 1000)) + (m1 / (1 + (HA(func_P_sat(T1), HR1)) / 1000)))
#return
