...
' FUNCTION P_sat - Saturation vapor pressure [Pa] as a function of dry bulb temperature
...
def Air1_RH(T_db, Twb , P):
    ...
    Air1_RH = P / P_sat(T_db) * w / (0.62198 + w)
    return Air1_RH
"""
...
def Air1_Mv(T_db , Twb, P):
    ...
    Air1_Mv = Mv0 * 273.15 / (273.15 + T_db) * (P - (1 - 0.62198) * RH * P_sat(T_db)) / 101325
    return Air1_Mv
"""
...
def Air2_Mv(T_db, RH,  P):
    ...
    Mv0 = 1.2922
    """Moist air density [kg/m3]"""
    Air2_Mv = Mv0 * 273.15 / (273.15 + T_db) * (P - (1 - 0.62198) * RH * P_sat(T_db)) / 101325
    return Air2_Mv 
"""
...
def Air3_RH(w , P , H ):
    ...
    T_db = Air3_Tdb(w, P, H)
    """' Relative humidity [0 ; 1]"""
    Air3_RH = P / P_sat(T_db) * w / (0.62198 + w)
    return Air3_RH
"""
_____________________________________________________________________________________________
...
def Air3_Mv(w , P ,H ):
    ...
    Mv0 = 1.2922
    """' Moist air density [kg/m3]"""
    Air3_Mv = Mv0 * 273.15 / (273.15 + T_db) * (P - (1 - 0.62198) * RH * P_sat(T_db)) / 101325
    return Air3_Mv
...
def Air4_RH(T_db , P , w ):
    ...
    """' Relative humidity [0 ; 1]"""
    Air4_RH = P / P_sat(T_db) * w / (0.62198 + w)
    return Air4_RH
"""
_____________________________________________________________________________________________
...
def Air4_Mv(T_db , P , w ):
    ...
    Mv0 = 1.2922
    """' Moist air density [kg/m3]"""
    Air4_Mv = Mv0 * 273.15 / (273.15 + T_db) * (P - (1 - 0.62198) * RH * P_sat(T_db)) / 101325
    return Air4_Mv
"""
_____________________________________________________________________________________________
