...
class StorageInterface:
    ...
    def upload_to_storage(self, url, path, content_type, chunk_id, file_size, start_byte = 0, read_bytes = MULTI_PART_UPLOAD_CHUNK_SIZE):
        try:
            with open(path, 'rb') as object_file:
                ...
                content_range = f'bytes {start_byte}-{start_byte+read_bytes-1}/{file_size}'
                #url, chunk_id, chunk_data, content_type, content_range, read_bytes
                return self.storage_client.upload_chunk(url, chunk_id, file_data, content_type, content_range, read_bytes)
