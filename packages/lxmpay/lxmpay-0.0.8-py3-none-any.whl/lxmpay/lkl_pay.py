...
class LakalaPay(AsyncPayCore):
    ...
    async def pay(
        ...
        **kwargs,
        ...
        response = self._handler_response(response)
        if raw_response:
            return response
        return self.response_to_pay_result(response)
    ...
    async def refund(
        ...
        **kwargs,
        ...
        if raw_response:
            return response
        return self.response_to_cancel_result(response)

    def response_to_pay_result(self, response: httpx.Response) -> PayResult:
        data = response.json()["data"]
        return PayResult(
            code=0,
            msg="success",
            trade_no=data["out_order_id"],
            prepay_data=json.loads(data["wap_pay_request"]),
        )

    def response_to_cancel_result(self, response: httpx.Response) -> CancelResult:
        data = response.json()["data"]
        return CancelResult(
            code=0,
            msg="success",
    ...
    def _handler_response(self, response: httpx.Response) -> httpx.Response:
        try:
