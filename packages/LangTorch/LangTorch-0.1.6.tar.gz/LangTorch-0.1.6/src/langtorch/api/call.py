...
def chat(prompts, system_messages, model="gpt-3.5-turbo-0613", api_key=None, verbose = False, as_str = False, **kwargs):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(f"Asyncio get current event loop failed with {E1}. Tried to fall back on new event loop with asyncio.run(job) but failed with error: {E2}.\nThis indicates error with the API code or provider and could be hard to fix.")
...
class File:
    ...
    def to_df(self):
        return pd.DataFrame({"texts" if self.type == "embedding" else "prompt": self.input, "embedding" if self.type == "embedding" else "completion": self.completion})
