...
class SingletonMeta(type):
    _instance = contextvars.ContextVar('ctx', default={}) # Can i add that what contextvar is set depends on the object class so that i can use the same metaclass for various object?
...
class Session(metaclass=SingletonMeta):
            ...
            if os.path.exists(self._path) and not new_session_file: # apply overrides to _config
                ...
                with open(path, "w") as f:
                    with open(self.session_file_template, "r") as f2:
                        f.write(f2.read())
                        logging.warning(f"File not found at {path} when loading new session file. {'Session will not be saved' if not new_session_file else 'Overriding existing session file'}")
    ...
    def completions(self, job_id = -1, type = None, provider = None):
        ...
        for p,t,j in zip(provider, type, job):
            ...
            if t == "chat":
                from .texts import Chat
                from .tensors import ChatTensor
                result += [[[Chat.from_messages(m['message']) for m in resp[1]["choices"]] for resp in entry] for entry in j["responses"]]
            elif t=="embedding":
                import torch
                result+=[torch.tensor([resp[1]["data"][0]["embedding"] for resp in j["responses"][0]])]
        return result
    ...
    def extract_prompts(self, dict_list):
        ...
        return [(entry["input"] if "embedding" in entry else langtorch.Chat.from_messages(entry["messages"])) for entry in dict_list]
