...
class SingletonMeta(type):
    _instance = contextvars.ContextVar('session', default={}) # Can i add that what contextvar is set depends on the object class so that i can use the same metaclass for various object?
...
class Session(metaclass=SingletonMeta):
    ...
    current_session = contextvars.ContextVar('ctx', default=None)
    ...
    def __init__(self, session_file = None, override = False):
        self._async_lock = asyncio.Lock()
        self._thread_lock = threading.Lock()
        self._temp = dict()
        self._open = True
        self._override = override
        self.load(session_file, override)
    ...
    @classmethod
    def load(self, path = "$self._path", override = False):
        ...
        if self._path:
            if os.path.exists(self._path) and not override: # apply overrides to _config
                try:
                ...
                with open(path, "w") as f:
                    with open(self.session_file_template, "r") as f2:
                        f.write(f2.read())
                        logging.warning(f"File not found at {path} when loading new session file. {'Session will not be saved' if not override else 'Overriding existing session file'}")
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self._thread_lock_acquired:
            raise RuntimeError("The session was first entered without 'async' in 'with Session'. \nYou will have to close it before entering 'async with Session' or chose whether you will use only 'async with' or 'with' .")
    ...
    def get_job(self, job_id = -1, type = None, provider = None):
        ...
        if type is None and provider is not None:
            for _provider in provider:
                for _type in [t for t in _api[_provider].keys() if t!='limits']: # A provider could have different types of requests but limits is never one of them
