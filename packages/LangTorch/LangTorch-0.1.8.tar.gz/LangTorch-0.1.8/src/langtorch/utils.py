...
def zeros_like(other, **kwargs):
    return langtorch.TextTensor([langtorch.Text.identity]*other.size, **kwargs).reshape(other.shape)


def str_like(other, string, **kwargs):
    return  langtorch.TextTensor([string]*other.size, **kwargs).reshape(other.shape)
        ...
        for key in keys:
            ...
            try:
                ...
                if 0<len(not_nan)<len(values):
                    values = [value if value is not None else metadata_type(np.full_like(tensor.content, np.nan)) for tensor, value in zip(tensors, values)]
            except:
                continue
            new_metadata[key] = (text_func if metadata_type is np.ndarray else tensor_func)(values, *func_args, **func_kwargs)
        return new_metadata
