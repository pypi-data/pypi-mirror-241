class SurahInfo:
    ...
    def get_surah_info(self, input_type, query):
        if input_type == "nama":
            query = query.strip()
            for surah in self.surah_list:
                if surah["nama"].replace("-", "").lower() == query.lower():
                    return surah

        elif input_type == "nomor":
            query = int(query)
            for surah in self.surah_list:
                if surah["nomor"] == str(query):
                    return surah

        return None

    def print_surah_info(self, surah):
        if surah:
