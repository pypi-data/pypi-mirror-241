...
class QueueWorker(DatabaseQueue):
    ...
    def execute(self, action_ids: tuple, min_delay_after_execution: float, max_delay_after_execution: float,
                total_missions: int = sys.maxsize) -> None:
        """Execute tasks from the queue."""
        self.install_packages(action_ids)
        max_delay_after_execution = max(max_delay_after_execution, min_delay_after_execution)
        try:
...
def execute_queue_worker(action_ids: tuple, min_delay_after_execution: float, max_delay_after_execution: float,
    ...
    queue_worker.execute(action_ids, min_delay_after_execution, max_delay_after_execution, total_missions)
...
def main():
    ...
    parser.add_argument('-min_delay_after_execution', type=float, default=0.0)
    parser.add_argument('-max_delay_after_execution', type=float, default=0.0)
    ...
    try:
        for _ in range(args.processes):
            args = (tuple(args.action_ids), args.min_delay_after_execution,
