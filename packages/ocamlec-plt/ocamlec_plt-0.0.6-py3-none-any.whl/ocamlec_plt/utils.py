...
def ocamlFilesToPandas(filepaths):
    """
    Transforms several ocaml files into a pandas DataFrame 
    (assumes that the headers will have "t" as first variable name).
    """
    all_data = {}
...
def filterData(data, filter_dict, comparison="="):
    """
    ...
    if "<":
        ...
        raise ValueError("Unrecognized comparison operator "+comparison+" in comparison")
    return data
...
class stat:
    ...
    def add_file(self, path):
        new_data = ocamlFileToPandas(path)
        self.__data = pd.concat([new_data, self.__data], axis=0)
    
    def add_files(self, path_list):
        new_data = ocamlFilesToPandas(path_list)
        self.__data = pd.concat([new_data, self.__data], axis=0)
        
    def return_df_copy(self):
    ...
    def filter_columns(self, **kwargs):
        self.__data.filter(axis=1, **kwargs)
        
    def filter_rows(self, **kwargs):
        self.__data = filterData(data = self.__data, **kwargs)
        
    def rename_columns(self, columns):
        self.__data.rename(columns = columns, inplace = True)
...
def getRuntime(data, filter_dict):
    """
    TODO
