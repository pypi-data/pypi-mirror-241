...
def deploy_model(
    ...
    try:
        ...
        if helper_models is not None and packages_to_check is not None:
            logger.info(
                f"Checking for dependency mismatch between main model {c.model} and helper models {helper_models} for the following package(s): {packages_to_check}"
            )
            for helper_model in helper_models:
                requirements_file_target = os.path.join(
                    helper_model, "requirements.model"
                )
                # Find path to helper model requirements file - This might be superflous
