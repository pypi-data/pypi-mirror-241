...
def display_class_tree(obj, show_sys=False, depth=1):
    def print_members(cls):
        ...
        for property in properties:
            try:
                ...
                print(f"    {property}: {value_str}")
            except Exception as e:
                print(f"    {property}: <Error accessing value: {e}>")

    cls = obj.__class__
    print(f"Class Tree for {obj}:")

    # 根据depth的值确定类层次结构的遍历深度
    mro = cls.__mro__
    mro_slice = mro if depth <= 0 else mro[:depth]

    for c in mro_slice:
        print_members(c)


def capture_screens(save_to_file=False):
    screens = []
    with mss.mss() as sct:
        for monitor_number, monitor in enumerate(sct.monitors[1:], start=1):  # 跳过第一个条目
            screenshot = sct.grab(monitor)
            
            if save_to_file:
                mss.tools.to_png(screenshot.rgb, screenshot.size, output=f"monitor_{monitor_number}.png")
            
            # 将截图转换为 ARGB 格式的 NumPy 数组
