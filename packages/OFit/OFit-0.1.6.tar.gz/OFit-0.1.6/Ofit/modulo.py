...
class KernelFit:
    ...
    def comparacion(self):
        ...
        x,yd = self.Fecha, self.casos
        yfG = self.fit(self.kernelG)
        yfG = yfG*max(yd)/max(yfG)

        yfT = self.fit(self.kernelTC)
        yfT = yfT*max(yd)/max(yfT)

        yfE = self.fit(self.kernelEpa)
        yfE = yfE*max(yd)/max(yfE)

        plt.figure(figsize=(8,6))
        plt.plot(x,yd,label="Datos",color = "darkblue")
        plt.plot(x,yfG,label="Gauss",color = "darkred")
        plt.plot(x,yfE,label="Epanechnikov",color = "darkviolet")
        plt.plot(x,yfT,label="Tricube",color = "darkgreen")
        plt.xlabel("Dias")
        plt.ylabel("Casos")
        plt.xticks(rotation=45)
        plt.xticks(rotation=45)
        plt.title("Fit con Kernel")
        plt.legend()
