...
def modify_user(
    ...
    **kwargs,
    ...
    #
    # NOTE: As the number of users grows, we may need to implement a query
    #       that directly filters on id/email to avoid looping through all
    #       users.
    user_id = None
    user_email = None
    org_id = None
    users = api_get_users()
    for user in users:
        if user_identifier in (user.id, user.email):
            user_id = user.id
    ...
    if user_id is None:
        ...
        msg = f'{OK} Updated User ID to {user_id} ({user_email})'
        if org_id:
            conf.organization_id = org_id
            msg += f' and Organization to {org_id}'
        conf.save_config()
        logger.info(msg)
        return 0
