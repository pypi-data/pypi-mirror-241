import os
import traceback
import asyncio
import importlib
from datetime import datetime
from fluxo.fluxo import Fluxo
from fluxo.database.fluxo import Fluxo as ModelFluxo
from fluxo.database.task import Task as ModelTask
from fluxo.utils import utc_current_time_formatted
from fluxo.scheduler import scheduler


class Task:
    def __init__(
        self,
        name: str,
        fluxo: Fluxo,
        start_time: datetime = None,
        end_time: datetime = None
    ):
        self.task_info = {
            'name': name,
            'fluxo': fluxo,
            'start_time': start_time,
            'end_time': end_time,
        }

    def __call__(self, func):
        async def wrapper(*args, **kwargs):
            fluxo_register_db = ModelFluxo.get_by_name(
                self.task_info.get('fluxo').name)

            task = ModelTask(name=self.task_info.get(
                'name'), fluxo_id=fluxo_register_db.id)
            new_task = task.save()

            try:
                # Chama a função original
                new_task.start_time = utc_current_time_formatted()
                new_task.update(**new_task.__dict__)

                result = await func(*args, **kwargs)

                new_task.end_time = utc_current_time_formatted()
                new_task.execution_date = utc_current_time_formatted()
                new_task.update(**new_task.__dict__)

                return result
            except Exception as err:
                error = traceback.format_exc()
                new_task.error = f'[Erro na tarefa: {new_task.name}]' + \
                    '\n' + error
                new_task.end_time = utc_current_time_formatted()
                new_task.execution_date = utc_current_time_formatted()
                new_task.update(**new_task.__dict__)

        setattr(wrapper, 'task_info', self.task_info)
        return wrapper


def execute_tasks(path, file):
    '''Executa a task assíncrona'''
    try:
        # Remove a extensão .py para obter o nome do módulo
        name_module = file[:-3]

        # Importa o módulo dinamicamente
        dir_base = os.path.basename(path)
        module = importlib.import_module(f"{dir_base}.{name_module}")

        coroutines = []

        # Procura por funções assíncronas no módulo
        for name_attribute in dir(module):
            attribute = getattr(module, name_attribute)
            if asyncio.iscoroutinefunction(attribute):
                # Verifica se a função é decorada com @Task
                if hasattr(attribute, 'task_info'):

                    task_info = attribute.task_info
                    fluxo_info = task_info.get('fluxo')
                    fluxo_register_db = ModelFluxo.get_by_name(
                        fluxo_info.name)

                    if fluxo_register_db is None:  # Verifica se fluxo existe no banco
                        fluxo = ModelFluxo(
                            name=fluxo_info.name,
                            interval=fluxo_info.interval)
                        fluxo.save()

                    else:
                        if fluxo_register_db.interval != fluxo_info.interval:
                            fluxo_register_db.interval = fluxo_info.interval
                            fluxo_register_db.update(
                                **fluxo_register_db.__dict__)

                    coroutines.append((attribute, fluxo_info))

                else:
                    print("A função não está decorada com @Task")

        if coroutines:
            scheduler(coroutines)

    except Exception as e:
        print(f"Erro ao executar tarefa no módulo {module}: {str(e)}")
