...
def visplot(mask1, mask2, rangles=[0,90], fig_size=[5,5], res=400, save_path='./Results/', **params):
    ...
    for i in range(len(rangles)):
        fnrays=load_path+"Visamp_%s_a_%s_i_%s_dx0_%s_dx1_%s_dx2_%s_sk_%s_bphi_%s_br_%s_bvapp_%s_gfact_%s_mp_%s_gp_%s_sp_%s_fudge_%s_Ncut_%s.h5"%(rangles[i],spin_case,i_case, dx0, dx1, dx2, sub_kep, betaphi, betar,  bvapp, gfactor, np.round(mup, 3), gammap, sigmap, fudge, int(Ncut))
