...
def prepare(input_files, data_dir, input_format, round_sites, chunksize, resume):
    ...
    if not resume:
        ...
        echo(f"Resuming prepare run from .coo files...")
        chrom_sizes = _get_chrom_sizes(data_dir)
    echo(
...
def _get_chrom_sizes(data_dir):
    chrom_sizes = {}
    with open(
        "/mnt/A290/Luo2017_neuron_scMethylomes/LuoCH_data/chrom_sizes.tsv", "r"
    ) as cs:
        for line in cs:
            chrom, size = line.strip().split(",")
            if not os.path.isfile(os.path.join(data_dir, f"{chrom}.npz")):
                chrom_sizes[chrom] = int(size)
    return chrom_sizes


# def _get_chrom_sizes(data_dir):
#     coo_files = sorted(glob(os.path.join(data_dir, "*.coo")))
#     chromosomes = sorted({os.path.basename(s).split("_")[0] for s in coo_files})
#     chrom_sizes = {}
#     for chrom in chromosomes:
#         chrom_sizes[chrom] = 0
#         print(f"\ngetting size of {chrom=}")
#         last_chunk = sorted(glob(os.path.join(data_dir, f"{chrom}_*.coo")))[-1]
#         with open(last_chunk, "r") as coo:
#             for line in coo:
#                 pos = int(line.split(",")[0])
#                 if pos > chrom_sizes[chrom]:
#                     chrom_sizes[chrom] = pos
#         print(f"{last_chunk=}\t{chrom_sizes[chrom]=}")
#     return chrom_sizes
...
def _load_csr_from_coo(data_dir, chrom, chrom_size, n_cells):
    ...
    indptr = np.empty(chrom_size + 2, dtype=np.uint64)
    ...
    for chunk in _iter_chunks(data_dir, chrom):
        ...
        print("appending chunk data")
        data_chunks.append(chunk[sorting_idx, 2].astype(np.int8))
        print("appending chunk indices")
        indices_chunks.append(chunk[sorting_idx, 1].astype(np.uint16))
    print(f"adding last idxptr value: {indptr_counter=}")
    indptr[indptr_i] = indptr_counter  # last missing index pointer value
    print(f"concatenating {len(data_chunks)} data chunks...")
    data = np.concatenate(data_chunks)
    print(f"concatenating {len(indices_chunks)} indices chunks...")
    indices = np.concatenate(indices_chunks)
    print(
        f"making CSR matrix (max d={max(data)}), (max idx={max(indices)}), (max indptr={max(indptr)})"
    )
