...
def _calc_mean_shrunken_residuals(
    ...
    if np.unique(indices).size < min_cells:
        # not enough cells have any sequencing coverage in this window
        return shrunken_resid
    # slice index pointer
    indptr = indptr_chrom[start : end + 1] - indptr_chrom[start]
    if np.unique(indptr).size < min_sites:
        # not enough CpG sites have any sequencing coverage in this window
        return shrunken_resid
