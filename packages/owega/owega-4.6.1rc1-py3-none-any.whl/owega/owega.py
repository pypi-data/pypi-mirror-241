...
def genconfig(conf_path=""):
	...
	if is_blank:
		...
		return
	info_print(f"WARNING, YOU ALREADY HAVE A CONFIG FILE AT {conf_path}")
	info_print("DO YOU REALLY WANT TO OVERWRITE IT???")
...
def handle_quit(temp_file, messages, ps, given="", temp_is_temp=False):
	do_quit(
		success_msg(),
		temp_file=temp_file,
...
# prints help
def handle_help(temp_file, messages, ps, given="", temp_is_temp=False):
	print_help(handler_helps)
	return messages


# generates config file
def handle_genconf(temp_file, messages, ps, given="", temp_is_temp=False):
	genconfig()
	return messages


# shows chat history
def handle_history(temp_file, messages, ps, given="", temp_is_temp=False):
	print_history(messages)
	return messages


# enables/disables command execution
def handle_commands(temp_file, messages, ps, given="", temp_is_temp=False):
...
# enables/disables command execution
def handle_estimation(temp_file, messages, ps, given="", temp_is_temp=False):
	if given.lower() in ["on", "true", "enable", "enabled"]:
...
def handle_tokens(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	if given.isdigit():
...
def handle_context(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	if given:
...
def handle_save(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	try:
		if given:
			file_path = given
...
def handle_load(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	try:
		if given:
			file_path = given
...
def handle_model(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
...
def handle_finput(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	given = given.split(' ')[0]
	if given:
		file_path = given
...
# change temperature
def handle_temperature(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	try:
...
# change top_p value
def handle_top_p(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	try:
...
def handle_frequency(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	try:
...
def handle_presence(temp_file, messages, ps, given="", temp_is_temp=False):
	given = given.strip()
	try:
...
def single_ask(user_prompt, temp_file="", input_file="", temp_is_temp=False):
	if not temp_file:
		temp_is_temp = True
		temp_file = get_temp_file()
	messages = Conversation()
...
def main():
	...
	if (args.ask):
		single_ask(args.ask, temp_file, input_history, temp_is_temp)
	else:
		try:
			user_interaction_loop(temp_file, input_history, temp_is_temp)
		except EOFError:
			do_quit(
				success_msg(),
				temp_file=temp_file,
