...
class SaveResultsToDatabase(BaseProtocolHook):
    ...
    def on_run_end(
        ...
        if context == TaskContext.WORKER:
            ...
            db_con: Optional[Connection] = kwargs["db_con"]
            table: Optional[str] = kwargs["table"]
            results: Any = kwargs["results"]

            # TODO: [BIT-3416] SaveResultsToDatabase not registered when empty context
            if any(v is None for v in [worker, connector, db_con, table]):
                logger.warning(
                    "Unable to save results to pod database. "
                    + "Either the worker, connector, db_con or table is None."
                )
                return
