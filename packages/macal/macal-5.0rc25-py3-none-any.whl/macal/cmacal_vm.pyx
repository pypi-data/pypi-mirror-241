    ...
    def Execute(self):
        while True:
            ...
            if   opCode[0] == 9 : # PUSH
                ...
                reg: BytecodeRegister = self.opcode_to_register_map[opCode[1]]
                while len(self.stack) > self.rsp.value and len(self.stack) > 0: self.stack.pop()
                if len(self.stack) == 0:
                    self._Error(f"POP: Stack underflow. (RIP: {self.rip.value-1})")
                ...
                if reg2.value + offset < 0 or reg2.value + offset >= len(self.stack):
                    self._Error(f"LOADR: Stack offset out of range: {reg2.value + offset} (0..{len(self.stack)-1}) (RIP: {self.rip.value-1})")
            ...
            # Need to run a sensus in more applications to optimize the order.
                ...
                if stack_offset < 0 or stack_offset >= len(self.stack):
                    self._Error(f"LOAD: Stack offset out of range: {stack_offset} (0..{len(self.stack)-1}) (RIP: {self.rip.value-1})")
                ...
                if stack_offset < 0 or stack_offset >= len(self.stack):
                    self._Error(f"STOR: Stack offset out of range: {stack_offset} (0..{len(self.stack)-1}) (RIP: {self.rip.value-1})")
                ...
                if reg1.value < 0 or reg1.value >= len(self.stack):
                    self._Error(f"STORIR: Stack offset out of range: {reg1.value} (0..{len(self.stack)-1}) (RIP: {self.rip.value-1})")
                ...
                if reg2.value < 0 or reg2.value >= len(self.stack):
                    self._Error(f"LOADIR: Stack offset out of range: {reg2.value} (0..{len(self.stack)-1}) (RIP: {self.rip.value-1})")
                ...
                if self.rax.value_type == AstNodetype.RECORD:
                    ...
                    if self.rbx.value < 0 or self.rbx.value >= lng:
                        self._Error(f"FEINDEX: Record key index out of range: {self.rbx.value} (0..{lng-1}) (RIP: {self.rip.value-1})")
                    self.rax.value = list(self.rax.value.keys())[self.rbx.value]
                else:
                    if self.rbx.value_type != AstNodetype.INTEGER:
                        self._Error(f"FEINDEX: Type error, array index must be an integer. (RIP: {self.rip.value-1})")
                    if self.rbx.value < 0 or self.rbx.value >= len(self.rax.value):
                        self._Error(f"FEINDEX: Array index out of range: {self.rbx.value} (0..{len(self.rax.value)-1}) (RIP: {self.rip.value-1})")
                ...
                if self.rax.value_type == AstNodetype.RECORD:
                    ...
                    self._Error(f"INDEX: Type error, array index must be an integer. (RIP: {self.rip.value-1})")
                elif self.rbx.value < 0 or self.rbx.value >= len(self.rax.value):
                    self._Error(f"INDEX: Array index out of range: {self.rbx.value} (0..{len(self.rax.value)-1}) (RIP: {self.rip.value-1})")
                else:
                    self.rax.value = self.rax.value[self.rbx.value]
                self.rax.value_type = typeFromValue(self.rax.value)
                ...
                if self.rax.value_type == AstNodetype.RECORD:
                    ...
                    if self.rbx.value_type != AstNodetype.INTEGER:
                        self._Error(f"INDEXR: Type error, array index must be an integer. (RIP: {self.rip.value-1})")
                    if self.rbx.value < 0 or self.rbx.value >= len(self.rax.value):
                ...
                reg: BytecodeRegister = self.opcode_to_register_map[opCode[1]]
                if reg.value_type != AstNodetype.ARRAY and reg.value_type != AstNodetype.RECORD and reg.value_type == AstNodetype.STRING:
                    self._Error(f"LEN: Type error, can't get length of a {reg.value_type.name.lower()}. (RIP: {self.rip.value-1})")                    
                ...
                if reg2.value_type != AstNodetype.ARRAY and reg2.value_type != AstNodetype.RECORD and reg2.value_type == AstNodetype.STRING:
                    self._Error(f"LEN: Type error, can't get length of a {reg2.value_type.name.lower()}. (RIP: {self.rip.value-1})")                    
                ...
                if reg2.value_type != reg3.value_type != AstNodetype.ARRAY:
                    self._Error(f"MERGE: Type error, both parameters must be arrays. (RIP: {self.rip.value-1})")
