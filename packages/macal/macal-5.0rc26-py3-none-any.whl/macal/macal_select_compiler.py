...
class MacalSelectCompiler:
    ...
    def compile_select(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        select_fields_lbl = scope.root.get_new_label("continue")

        # check if the result is empty.
        self.parent.emit_new_label("check_result_length", scope)
        self.emitter.MOVSR(self.cs.rax)  # rax = stack[rsp-1]
        self.emitter.CMPRTI(self.cs.rax, AstNodetype.NIL)  # if rax != ARRAY, jump to into_lbl
        self.emitter.ERRORZ(f"Expected an array, record or string in the FROM clause of the SELECT statement got nil. ({self.cs.rip.value - 1})")
        self.emitter.CMPRTI(self.cs.rax, AstNodetype.BOOLEAN)  # if rax != ARRAY, jump to into_lbl
        self.emitter.ERRORZ(f"Expected an array, record or string in the FROM clause of the SELECT statement got bool. ({self.cs.rip.value - 1})")
        self.emitter.CMPRTI(self.cs.rax, AstNodetype.INTEGER)  # if rax != ARRAY, jump to into_lbl
        self.emitter.ERRORZ(f"Expected an array, record or string in the FROM clause of the SELECT statement got integer. ({self.cs.rax.value - 1})")
        self.emitter.CMPRTI(self.cs.rax, AstNodetype.FLOAT)  # if rax != ARRAY, jump to into_lbl
        self.emitter.ERRORZ(f"Expected an array, record or string in the FROM clause of the SELECT statement got float. ({self.cs.rax.value - 1})")
        self.emitter.LEN(self.cs.rax)  # array_register = len(rax)
        self.emitter.CMPRI(self.cs.rax, AstNodetype.INTEGER, 0)  # if len == 0, jump to into_lbl
        self.parent.emit_jump(Opcode.JMPZ, select_fields_lbl, scope)
        ...
        scope.root.set_label_address(
            lbl, self.cs.rip.value
        )  # we jump here if it's an array or fall through to it if we have converted it to an array.
        ...
        # If there was no record to start with, we skipped to this.
        # If there was data to begin with we fall through to here via from and where.
        scope.root.set_label_address(select_fields_lbl, self.cs.rip.value)  # into

        # Compile the select fields
        if not (len(select.Fields) == 1 and select.Fields[0].fieldname == "*"):
            self.emitter.POP(self.cs.rax)  # rax = value
            self.compile_select_fields(select, scope)
            self.emitter.PUSH(self.cs.rax)  # save value
    ...
    def compile_where(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        self.emitter.CMPRI(
            self.cs.rax, AstNodetype.BOOLEAN, True
        )  # if rax == True we need to include the record in the result array.
    ...
    def compile_select_fields(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        select_fields_end_label = scope.root.get_new_label("select_fields_end")
        select_null_record_label = scope.root.get_new_label("select_null_record")
        ...
        # skip this if there's no records.
        self.emitter.LENR(array_register, self.cs.rax)  # array_register = len(rax)
        self.emitter.CMPRI(array_register, AstNodetype.INTEGER, 0)  # if len == 0, jump to select_fields_continue
        self.parent.emit_jump(Opcode.JMPZ, select_null_record_label, scope)  # jump to select_null_record_label
        ...
        self.emitter.MOVRR(
            current_record_register, self.cs.rax
        )  # current_record_register is now a "copy" of the record in rax.
        ...
        # skip to the end.
        self.parent.emit_jump(Opcode.JMP, select_fields_end_label, scope)  # jump to select_fields_end

        scope.root.set_label_address(select_null_record_label, self.cs.rip.value)
        # the record is empty but we do know the names of the fields that we need
        # so we create an empty record with those fields.
        self.emitter.MOVRI(self.cs.rax, AstNodetype.RECORD, {})
        self.emitter.MOVRI(self.cs.rcx, AstNodetype.NIL, AstNodetype.NIL)
        for field in select.Fields:
            self.emitter.MOVRI(self.cs.rbx, AstNodetype.STRING, field.altfieldname)
            self.emitter.INDEXR(self.cs.rcx)

        # this is the end of the select fields.
        scope.root.set_label_address(select_fields_end_label, self.cs.rip.value)

    def compile_field(
        self,
        field: AstNodeSelectField,
        record_register: BytecodeRegister,
        result_record_register: BytecodeRegister,
        field_value_register: BytecodeRegister,
        scope: CompilerScope,
    ) -> None:
        ...
        )  # # rax[rbx] = field_value_register so we add the current new field to the resulting record.
        self.emitter.MOVRR(
            result_record_register, self.cs.rax
        )  # result_record_register is now a "copy" of the result record in rax.
    ...
    def compile_into(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        self.emitter.MOVRI(reg, AstNodetype.BOOLEAN, select.merge)
        self.emitter.CMPRI(reg, AstNodetype.BOOLEAN, True)
        self.parent.emit_jump(Opcode.JMPZ, lbl2, scope)  # if merge, skip to lbl2 because we already did do the merge
        # and we don't want to reduce the record to a single value if we're merging.
    ...
    def walk_into_index(self, expr: AstNodeExpression, scope: CompilerScope) -> None:
        reg = self.cs.GetFreeRegister()
    ...
    def compile_where_binary_expression(
        self, where: AstNodeBinaryExpression, record_register: BytecodeRegister, scope: CompilerScope
    ) -> None:
    ...
    def compile_where_literal_expression(self, expression: AstNodeLiteralExpression, scope: CompilerScope) -> None:
    ...
    def compile_where_unary_expression(
        self, expression: AstNodeUnaryExpression, record_register: BytecodeRegister, scope: CompilerScope
    ) -> None:
    ...
    def compile_where_variable_expression(
        self, expression: AstNodeVariableExpression, record_register: BytecodeRegister, scope: CompilerScope
    ) -> None:
    ...
    def compile_where_indexed_variable_expression(
        self, expression: AstNodeIndexedVariableExpression, record_register: BytecodeRegister, scope: CompilerScope
    ) -> None:
    ...
    def walk_where_index(self, expression: AstNodeExpression, record_register: BytecodeRegister, scope: CompilerScope) -> None:
