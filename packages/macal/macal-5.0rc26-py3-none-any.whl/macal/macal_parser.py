...
class Parser:
    ...
    def expression_array_element(self) -> Any:
        ...
        if node.expr_type != AstNodetype.LITERAL_EXPRESSION:
            self.error(
                f"Invalid array element type {node.expr_type} (expected literal, array or record) ({node.token.lineno}, {node.token.offset})"
            )
        vt = node.value_type
        if vt == AstNodetype.STRING_INTERPOLATION_STRING_PART or vt == AstNodetype.STRING_INTERPOLATION_END:
            self.error(
                f"Invalid array element type {vt} (expected literal, array or record) ({node.token.lineno}, {node.token.offset})"
            )
