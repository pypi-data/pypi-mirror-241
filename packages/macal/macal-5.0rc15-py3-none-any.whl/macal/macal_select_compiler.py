...
class MacalSelectCompiler:
    ...
    def compile_select(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        into_lbl = scope.root.get_new_label("into")
        ...
        self.parent.emit_jump(Opcode.JMPZ, into_lbl, scope)
        ...
        scope.root.set_label_address(into_lbl, self.cs.rip.value) # into
        # Compile the into clause
        
        self.compile_into(select, scope)
        
        if (select.From.expr_type != AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION 
            and select.From.expr_type != AstNodetype.FUNCTION_CALL_EXPRESSION):
            self.cs.rsp.value -= 1 # compensation for missed pops.
    ...
    def compile_select_fields(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        select_fields_continue = scope.root.get_new_label("select_fields_continue")
        ...
        self.emitter.CMPRI(array_register, AstNodetype.INTEGER, 0) # if array_register == 0, jump to select_fields_continue
        self.parent.emit_jump(Opcode.JMPNZ, select_fields_continue, scope) 
        self.emitter.PUSH(self.cs.rax)
        self.parent.emit_jump(Opcode.JMP, select_fields_end_label, scope) # jump to select_fields_end
        
        scope.root.set_label_address(select_fields_continue, self.cs.rip.value)
    ...
    def compile_into(self, select: AstNodeSelectStatement, scope: CompilerScope) -> None:
        ...
        if into is None:
            ...
            #print("define variable: ", select.Into.name)
            #print("stack offset: ", self.cs.rsp.value)
            offset = self.cs.rsp.value - 1
            if (select.From.expr_type == AstNodetype.VARIABLE_FUNCTION_CALL_EXPRESSION 
                or select.From.expr_type == AstNodetype.FUNCTION_CALL_EXPRESSION):
                offset += 1
            into = scope.define_variable(select.Into.name, offset)
