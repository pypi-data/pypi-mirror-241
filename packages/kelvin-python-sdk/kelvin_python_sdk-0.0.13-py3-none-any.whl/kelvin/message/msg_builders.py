...
from kelvin.message import KRN, KRNAsset, KRNAssetParameter, Message
...
from kelvin.message.primitives import BooleanParameter, NumberParameter, StringParameter
...
@dataclass
class AssetParameter(MessageBuilder):
    """Asset Parameter Builder.
    ...
    def to_message(self) -> Message:
        msg_type: Any = StringParameter
        if isinstance(self.value, bool):
            msg_type = BooleanParameter
        elif isinstance(self.value, float) or isinstance(self.value, int):
            msg_type = NumberParameter

        return msg_type(resource=self.resource, payload=self.value)


@dataclass
class Parameters(MessageBuilder):
    """Parameters Builder. Set application parameters in bulk.

    Args:
        resource (KRN): Kelvin Resource name for the target App Version (usually KRNAppVersion)
