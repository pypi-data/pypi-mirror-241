...
from tsakul.settings import parameters
from tsakul.supply_data import default_buffer_dist
...
def add_centroids(g, X, Y, k=1, method="turn", euclidean=False, on_top=False, **kwargs):
    """
    ...
    on_top : bool, optional
        set to True if extra nodes are made on top of each other
    **kwargs : iterable, optional
    ...
    The route choice algorithms for DTA within tsakul rely on iterative computations on
...
def add_connectors(x, y, u, k, g, new_g, euclidean):
    ...
    can be changed in tsakul.settings.
...
def _get_nearest_node(G, point, method="haversine", return_dist=False):
    ...
    coords = ((n, d["x_coord"], d["y_coord"]) for n, d in G.nodes(data=True))  # if 'centroid' in d.keys() and d['centroid'] == True)
