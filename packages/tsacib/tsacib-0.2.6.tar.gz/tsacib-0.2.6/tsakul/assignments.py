...
"""
    ...
    order to use the assignments in tsakul.
    ...
    All of these requirements are met if tsakul's functions for extracting the network
    from OpenStreetMap are used.

    See Also
    --------

    tsakul.supply_data.road_network_from_place

    tsakul.demand_data.add_centroids

    tsakul.demand_data.add_connectors

    tsakul.supply_data.relabel_graph
...
import tsakul._context
from tsakul.demand import (
    InternalDynamicDemand,
    build_internal_dynamic_demand,
    build_internal_static_demand,
)
from tsakul.demand_data import _check_centroid_connectivity
from tsakul.dta.i_ltm_aon import i_ltm_aon
from tsakul.dta.incremental_assignment import incremental
from tsakul.results import DynamicResult, StaticResult, get_skim
from tsakul.sta.dial_b import dial_b
from tsakul.sta.dial_stochastic_assignment import dial_sue
from tsakul.sta.msa import msa_flow_averaging
from tsakul.sta.uncongested_dial import sun
from tsakul.sta.dun import dun
from tsakul.supply_data import build_network
from tsakul.utilities import log
...
class DynamicAssignment:
    ...
    def __init__(
        ...
        dynamic_demand : tsakul.demand.DynamicDemand
        simulation_time: tsakul.time.SimulationTime
        """
    ...
    def run(self, method: str = "i_ltm_aon"):
        ...
        """
        tsakul._context.running_assignment = (
...
class StaticAssignment:
    ...
    def run(self, method, store_iterations=False, **kwargs):
        ...
        tsakul.results.StaticResult
        list of tsakul.results.StaticResult, optional
        ...
        if method == "dial_b":
            ...
            assert not store_iterations  # does not make sense for dun
            costs, flows = dun(
                network=self.internal_network, demand=self.internal_demand
            )
            result = StaticResult(
                costs,
                flows,
            ...
            costs, flows, origin_flows = sun(
                self.internal_network, self.internal_demand, **kwargs
            )
            result = StaticResult(
                costs,
                flows,
        ...
        if not store_iterations:
            ...
            return result, tsakul._context.iteration_states
