...
after initializing either a `tsakul.assignments.StaticAssignment` or
`tsakul.assignments.DynamicAssignment` we have access to a compiled
`tsakul.supply.Network` object.
Alternatively, this can be build using `tsakul.supply_data.build_network`

>>> network = tsakul.supply_data.build_network(g)
...
from tsakul.csr import UI32CSRMatrix, f32csr_type, ui32csr_type
...
class Links(object):
    ...
    free_speed: numpy.ndarray
        float, 1D
    out_turns: tsakul.csr.UI32CSRMatrix
    in_turns: tsakul.csr.UI32CSRMatrix
    lanes: numpy.ndarray
        int, 1D
    ...
    should not be initialized by the user, use tsakul.supply_data.build_network
...
class Nodes(object):
    ...
    Parameters
    ----------
    out_links: tsakul.csr.UI32CSRMatrix
    in_links: tsakul.csr.UI32CSRMatrix
...
class Network(object):
    ...
    should be initialized with tsakul.supply_data.build_network

    See Also
    --------
    tsakul.supply_data.build_network
    tsakul.supply.Links
    tsakul.supply.Nodes
    tsakul.supply.Turns
