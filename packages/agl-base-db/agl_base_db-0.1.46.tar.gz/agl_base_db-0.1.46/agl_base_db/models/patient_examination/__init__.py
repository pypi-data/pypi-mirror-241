...
class PatientExamination(models.Model):
    ...
    def find_matching_video_from_patient(self):
        ...
        """
        videos = self.patient.videos.filter(date=self.report_file.date, patient_examination__isnull=True)
        if videos:
            if len(videos) > 1:
                print(f"Warning: Found more than one video for patient {self.patient} on date {self.report_file.date}. Choosing the first one.")
            return videos[0]
        else:
            videos = self.patient.videos.filter(patient_examination__isnull=True)
            if len(videos)==1:
                return videos[0]
