...
def fetch_from_ipfs(cid, filename, file_type="File"):
    primary_source = "http://silver-absent-kite-292.mypinata.cloud/ipfs/"
...
def get_multisig_address(address):
    ...
    """
    url = f"https://subscription-library.spectral.finance/getMultiSigAddress/{address}"
...
def fetch_training_data(challenge_id):
    ...
    if challenge_id == '0xFDC1BE05aD924e6Fc4Ab2c6443279fF7C0AB5544':
        ipfsTrainingDataset = "QmTqZdaVrHmh9kQFhEjeobk1p4FkhDB2uwZMUHsh6fgMcB"
...
def submit_inferences(config_manager, challenge_id, submission_file):
    ...
    try:
        ...
        print(
            f"You can check status of that transaction under: https://arbiscan.io/tx/{str(tx_hash)}")
    except Exception as e:
        config_manager = ConfigManager(CONFIG_PATH)
        wallet_address = config_manager.get('global', 'wallet_address')
