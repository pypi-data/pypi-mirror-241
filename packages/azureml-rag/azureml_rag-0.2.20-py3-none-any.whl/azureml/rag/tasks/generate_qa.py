...
def get_model_config(llm_config: Dict[str, str], openai_api_type: str, openai_api_version: str, activity_logger: Logger):
    ...
    if connection_id:
        ...
        if connection_metadata.get(
            'ApiType', connection_metadata.get('apiType', '')
            ).lower() == "azure" or connection_metadata.get('azureml.flow.connection_type', '').lower() == "azureopenai":
