...
def ask(messages, llm, vecdb_id):
    ...
    if vecdb_id == "single":
        ...
        instruction = "GNP ofrece los siguientes seguros. Responde la pregunta del final usando la información. Si no sabes la respuesta responde 'No lo sé.'"
        system_message = messages[0]['content']
        question = messages[-1]["content"]
        # Prompt
        temp_stop_conditions=["no se menciona", "no se especifica"]
        ...
        if answer == "" or "No lo sé." == answer or \
                "basándome en" in answer.lower() or \
