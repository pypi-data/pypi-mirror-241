...
def main():
    ...
    detector = cv2.FaceDetectorYN.create("./yunet_model/face_detection_yunet_2023mar.onnx",
                                         "",                                     
                                         [int(coord * scale) for coord in clip.size],
                                         score, #score threshold
                                         nmsthld, #nmsthreshold not sure what this does??
                                         n_kboundboxes #top k bounding boxes
                                         )
    detector_detect_lambda = lambda frame,model: model.detect(frame)
