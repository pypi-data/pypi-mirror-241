...
min_version = semver.Version.parse('1.12.790')
max_version = semver.Version.parse(max_version_str)
...
def get_latest_local_semver() -> str:
    pth = Path.home() / '.lumipy'
    sem_vers = sorted([semver.Version.parse(d.parts[-1].replace('_', '.')) for d in pth.glob('*')])
