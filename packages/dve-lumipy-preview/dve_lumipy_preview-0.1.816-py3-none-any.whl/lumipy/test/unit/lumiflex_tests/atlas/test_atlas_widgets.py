...
class TestAtlasWidgets(SqlTestCase):
    ...
    def test_atlas_widget(self):
        ...
        node = factory.build()
        self.assertEqual('<b style="color:#415464;font-size:22px;">Atlas</b>', node.name)
    ...
    def test_filtered_atlas_widget(self):
        ...
        self.assertEqual('<b style="color:#415464;font-size:22px;">Atlas</b> <code>filter = \'*instrument*\'</code>', node.name)
        self.assertEqual('globe', node.icon)
        self.assertEqual(2, len(node.nodes))
