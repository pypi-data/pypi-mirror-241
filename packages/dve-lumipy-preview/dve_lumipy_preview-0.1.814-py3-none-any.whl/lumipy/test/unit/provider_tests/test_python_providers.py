...
class TestLocalProviderInfra(unittest.TestCase):
    ...
    def test_manager_creation(self):
        ...
        manager = lp.ProviderManager(*providers, _sdk_version='1.1.1')
    ...
    def test_manager_operation_bernoulli(self):
        ...
        host = 'localhost'
        port = 5004
        manager = lp.ProviderManager(*providers, dry_run=True, host=host, port=port, _sdk_version='1.1.1')
    ...
    def test_manager_input_validation(self):
        ...
        with self.assertRaises(ValueError) as ve:
            lp.ProviderManager(dry_run=True, host='${being mean}', port=5004, _sdk_version='1.1.1')

        with self.assertRaises(ValueError) as ve:
            lp.ProviderManager(prov, dry_run=True, host='${being mean}', port=5004, _sdk_version='1.1.1')

        with self.assertRaises(ValueError) as ve:
            lp.ProviderManager(prov, dry_run=True, host='localhost', port='${being mean}', _sdk_version='1.1.1')

        with self.assertRaises(ValueError) as ve:
            lp.ProviderManager(prov, dry_run=True, host='localhost', port=5004, domain='${being mean}', _sdk_version='1.1.1')

        with self.assertRaises(ValueError) as ve:
            lp.ProviderManager(prov, dry_run=True, host='localhost', port=5004, user='${being mean}', _sdk_version='1.1.1')
    ...
    def test_runtime_error_handlingr(self):
        ...
        host = 'localhost'
        port = 5006
        manager = lp.ProviderManager(p, dry_run=True, host=host, port=port, _sdk_version='1.1.1')
