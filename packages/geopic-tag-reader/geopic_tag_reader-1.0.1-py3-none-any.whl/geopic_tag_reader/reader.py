...
def decodeDateTimeOriginal(data: dict, datetimeField: str, msField: str) -> Tuple[Optional[datetime.datetime], List[str]]:
    ...
    if d is None and isExifTagUsable(data, datetimeField):
        try:
            ...
            if "." in timeRaw[2]:
                secondsParts = timeRaw[2].split(".")
                secondsRaw = int(secondsParts[0])
                msRaw = int(secondsParts[1][:6].ljust(6, "0"))
            else:
                secondsRaw = int(timeRaw[2])
                msRaw = data[msField] if isExifTagUsable(data, msField, float) else "0"
                msRaw = int(str(msRaw)[:6].ljust(6, "0"))
