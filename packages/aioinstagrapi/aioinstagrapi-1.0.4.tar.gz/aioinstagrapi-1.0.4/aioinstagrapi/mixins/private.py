...
class PrivateRequestMixin:
    ...
    @retry(retry=retry_if_exception_type(httpx.TimeoutException, httpx.), stop=stop_after_attempt(10), wait=wait_random(1,3))
    async def _send_private_request(
        ...
        try:
            ...
            if "don't have permission to post a GIF" in text:
                raise ClientNoGifPermissions(e, response=e.repsonse, **last_json)
            ...
            if "https://www.instagram.com/accounts/suspended/" in text:
                raise ClientSuspended(e, response=e.repsonse, **last_json)
            if e.response.status_code == 500:
                raise InternalServerError(e, response=e.repsonse, **last_json)
            if e.response.status_code == 403:
                ...
                if "Couldn't Post Your Comment" in text: 
