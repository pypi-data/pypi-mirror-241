...
class WinBotMain(socketserver.BaseRequestHandler, metaclass=_protect("handle", "execute")):
    ...
    def init_ocr_server(self, ip: str, port: int = 9752, use_angle_model: bool = False, enable_GPU: bool = False, enable_tensorrt: bool = False) -> bool:
        """
        初始化 OCR 服务,内置OCR需要到群文件下载对应版本的cuda依赖包，暂不支持win7系统

        :param ip: ocr服务器IP。当参数值为 "127.0.0.1"时，则使用内置的ocr识别，不必打开AiboteWindowsOcr.exe服务端
        :param port: ocr服务器端口，默认9528
        :param use_angle_model: 支持图像旋转。 默认false
        :param enable_GPU: 启动GPU 模式。默认false 。GPU模式需要电脑安装NVIDIA驱动，并且到群文件下载对应cuda版本
        :param enable_tensorrt: 启动加速，仅 enableGPU = true 时有效，默认false 。图片太大可能会导致GPU内存不足
        :return:
        """
        return self.__send_data("initOcr", ip, port, use_angle_model, enable_GPU, enable_tensorrt) == "true"
