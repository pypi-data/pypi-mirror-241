...
class WebBotMain(socketserver.BaseRequestHandler, metaclass=_protect("handle", "execute")):
    ...
    def touch_start(self, x: float, y: float) -> bool:
        ...
        :param x: 非Windows坐标，页面左上角为起始坐标，可以通过getElementRect 获取相关坐标
        :param y: 非Windows坐标，页面左上角为起始坐标，可以通过getElementRect 获取相关坐标
        :return:
        """
    ...
    def touch_end(self, x: float, y: float) -> bool:
        ...
        :param x: 一般同最后一个触屏事件的坐标一致
        :param y: 一般同最后一个触屏事件的坐标一致
        :return:
        """
