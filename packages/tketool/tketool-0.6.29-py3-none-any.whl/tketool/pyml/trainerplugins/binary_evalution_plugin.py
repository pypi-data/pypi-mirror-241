...
class binary_evaluate_plugin(trainer_plugin_base):
    ...
    def Invoke(self, base_wall, epoch_wall, batch_wall):
        ...
        # 将模型的输出通过sigmoid函数转换到0-1之间，然后以0.5为阈值转化为类别标签
        predictions = (torch.from_numpy(all_output) > 0.5).numpy()

        accuracy = accuracy_score(all_label, predictions)
        precision = precision_score(all_label, predictions, zero_division=0)
        recall = recall_score(all_label, predictions)
        ...
        epoch_wall['evaluations']['precision'] = precision
        epoch_wall['evaluations']['recall'] = recall
        epoch_wall['evaluations']['f1'] = f1
        # epoch_wall['evaluations']['roc_auc'] = roc_auc

        base_wall['logs_stack'].append(
            f"Evalution: acc:{accuracy:.4f}, pre:{precision:.4f}, recall:{recall:.4f}, f1:{f1:.4f}")
        model.train()
