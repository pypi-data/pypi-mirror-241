...
class HighlightRenderer(HTMLRenderer):
    ...
    def block_code(self, code, info=None):
        ...
        for index in range(len(code)):
            ...
            whitespaces_before_line: str = " " * (len(code[index]) - len(code[index].lstrip()))
            
            if code[index].startswith("//"): whitespaces_before_line += "//"
            
            _range: int = 92 if len(code[index]) > 92 else len(code[index]) - 1
            
            if " " not in code[index]:
                # Find the first ( and replace it with a newline:
                
                for _index in range(_range, 0, -1):
