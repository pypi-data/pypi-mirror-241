...
def split_line_if_too_long(line: str) -> str:
    if len(line) <= 80: 
        return line
    ...
    if "//" in line: 
        ...
        if len(_line.split("\n")) > 80: 
            _line = _line.split("\n")[0] + split_line_if_too_long(_line)
        
        return _line
        
    # Find last white space in the line before the 80th character:
    if not "(" in line and not " " in line:
        return wrap(line, width=80, placeholder="...")
    
    if not "(" in line:
        for index in range(last_line_index, 0, -1):
            if line[index] != " ": continue
            return new_line(index, index, indentation + 4)
    
    for index in range(last_line_index):
        if line[index] != "(": continue
        return new_line(index + 1, index + 1, indentation + 4)
