...
from skyatc.cluster.server import Server, ServerState, ServerState, ServerType, key_root
from skyatc.cluster.config import aws_worker_default_image, aws_controller_default_image
...
class AWSServer(Server):
    ...
    def get_ssh_client_impl(self):
        ...
        try:
            print(f"ssh -i {str(self.local_keyfile)} {self.login_name}@{self.public_ip()}")
            time.sleep(30)
            client.connect(
                self.public_ip(),
                # username="ec2-user",
