...
// Based on https://github.com/jupyterlab/jupyterlab/blob/3.4.x/packages/services/src/terminal/default.ts
// Changes: None
// Included here because TerminalConnection isn't exported
// from @jupyterlab/services and is needed by ClusterTerminalManger
...
import { shutdownTerminal, TERMINAL_SERVICE_URL } from './clusterTerminalAPI';
...
export class TerminalConnection implements Terminal.ITerminalConnection {
  ...
  private _createSocket = () => {
    ...
    let url = URLExt.join(settings.wsUrl, 'terminals', 'websocket', encodeURIComponent(name));
  ...
  private _onWSClose = (event: Event) => {
    console.warn(`Terminal websocket closed: ${(event as CloseEvent).code}`);
    if (!this.isDisposed) {
      this._reconnect();
    }
  };
   ...
   */
  private _updateConnectionStatus(connectionStatus: Terminal.ConnectionStatus): void {
