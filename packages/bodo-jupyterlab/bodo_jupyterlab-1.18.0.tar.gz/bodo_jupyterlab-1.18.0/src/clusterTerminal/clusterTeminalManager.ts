...
// Based on https://github.com/jupyterlab/jupyterlab/blob/3.4.x/packages/services/src/terminal/manager.ts
// Changes:  add startNewClusterTerminal

import { IIterator, iter } from '@lumino/algorithm';
import { Poll } from '@lumino/polling';
import { ISignal, Signal } from '@lumino/signaling';
import { Terminal, ServerConnection, TerminalManager } from '@jupyterlab/services';
import { TerminalConnection } from './clusterTerminalConnection';
import { BaseManager } from './baseManager';

import { listRunning, shutdownTerminal, startNew } from './clusterTerminalAPI';

/**
 * A terminal session manager.
 */
export class ClusterTerminalManager extends BaseManager implements Terminal.IManager {
  ...
  dispose(): void {
    ...
    this._connections.forEach((x) => x.dispose());
   ...
   * options.
   */
  async startNew(options?: Terminal.ITerminal.IOptions): Promise<Terminal.ITerminalConnection> {
    const model = await startNew(this.serverSettings, options?.name, options?.cwd);
    await this.refreshRunning();
    return this.connectTo({ model });
  }

  async startNewClusterTeminal(
    options?: Terminal.ITerminal.IOptions,
    cluster?: string
  ): Promise<Terminal.ITerminalConnection> {
    const model = await startNew(this.serverSettings, options?.name, options?.cwd, cluster);
    await this.refreshRunning();
    return this.connectTo({ model });
  }
  ...
  async shutdownAll(): Promise<void> {
    ...
    await Promise.all(this._names.map((name) => shutdownTerminal(name, this.serverSettings)));
  ...
  private _onStarted(terminalConnection: Terminal.ITerminalConnection): void {
    this._connections.add(terminalConnection);
  ...
  private _onDisposed(terminalConnection: Terminal.ITerminalConnection): void {
    this._connections.delete(terminalConnection);
  ...
  private _connections = new Set<Terminal.ITerminalConnection>();
...
/**
 * The namespace for ClusterTerminalManager statics.
 */
export namespace ClusterTerminalManager {
  /**
   * A no-op terminal manager to be used when starting terminals is not supported.
   */
  export class NoopManager extends ClusterTerminalManager {
    /**
