...
// Based on https://github.com/jupyterlab/jupyterlab/blob/3.4.x/packages/services/src/terminal/restapi.ts
// Changes: Modify startNew to take cluster as a parameter and forward to extension backend
 ...
 *
 * @param cluster - The ICluster to connect to converted to a json string
 *
...
export async function startNew(
  ...
  const init = {
    method: 'POST',
    body: JSON.stringify({ name, cwd, cluster }),
  };
...
export async function shutdownTerminal(
  ...
  if (response.status === 404) {
    const data = await response.json();
    const msg = data.message ?? `The terminal session "${name}"" does not exist on the server`;
    console.warn(msg);
