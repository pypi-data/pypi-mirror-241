...
class App():
    ...
    async def download_req(self, request):
        ...
        if FILE_SERVER_PROVIDER=='FS':
            FILE_SERVER_BASE_URL = f'https://{request.url.host}' # request.url.host, request.client.host
            url = fs_download_signed_url(self.fs_secret, FILE_SERVER_BASE_URL, args['bucket'], key, timeout=timeout)
        else:
    ...
    async def upload_req(self, request):
        ...
        return rest_ok({'url':url, 'info':{'request_url_host':request.url.host, 'request.client.host':request.client.host}})
