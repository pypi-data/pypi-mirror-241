...
def install():
    """Install the terminalgpt-pro openai api key and create app directories."""
    while True:
        ...
        restart = input("\nDo you want to restart the installation process? (yes/no): ")
...
@click.command(help="Change the default model.")
@click.pass_context
@click.option(
    "--model",
    "-m",
...
def change_model(ctx, model):
    """Change the default model."""
    ctx.invoke(cli, model=model)
    click.echo(f"Changed the default model to: {model}")
