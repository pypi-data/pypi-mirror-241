...
from terminalgptpro.reset import run_reset
...
def install():
    ...
    while True:
        ...
        if model in models:
            ...
            printer.printt(f"\n{Style.BRIGHT}{Fore.RED}Invalid model! Please choose from the provided list.{Style.RESET_ALL}\n")
            continue
    ...
    while True:
        ...
        if printing_style in printing_styles:
            break
        else:
            printer.printt(f"\n{Style.BRIGHT}{Fore.RED}Invalid printing style! Please choose from the provided list.{Style.RESET_ALL}\n")
            continue
...
def reset():
    """Command to reset all configuration files."""
    run_reset()  # Вызов функции run_reset из reset.py
