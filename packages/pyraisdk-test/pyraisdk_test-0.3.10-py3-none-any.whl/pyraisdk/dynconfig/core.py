...
class Evaluator(ABC):
    ''' Evaluator is for the updating scenario of "read-then-write".
    '''
    @abstractmethod
    def eval(self, dataset: Dict[str, Any]) -> ChangeSet:
        ''' Specify how you would like to modify it for a given remote full dataset.
            ...
            Attention: Do not make changes to input `dataset` directly.
            
            Args:
                dataset: Remote full dataset.
...
class _PatchOperationEvaluator(Evaluator):
    ''' Internal updating evaluator for patch operation.
    '''
...
class DynamicConfigWriter:
    ...
    def patch(self, delta: ChangeSet, ext_seq: Optional[int] = None) -> ChangeSet:
        ''' Update remote dataset with input `delta`.
            ...
            Returns:
                The changeset updated to remote in new commit.
        '''
        return self.update(_PatchOperationEvaluator(delta), ext_seq)


    def update(self, evaluator: Evaluator, ext_seq: Optional[int] = None) -> ChangeSet:
        ''' Update remote dataset in read-then-write approach.
            Success of `update` guarantee at that moment the remote full dataset
            completely match with the `evaluator`, which means `evaluator` cannot
            make real changes to the dataset anymore.
            Update operation is atomic.
            
            Args:
                evaluator: Updating logic to remote full dataset.
                ext_seq: An extra external sequence. For provided values, require
                        them to be incremental in commit chain.
        ...
        '''
        with self._lock:
            return self._update(evaluator, ext_seq)
    ...
    def _reconcile(self, dataset: Dict[str, Any], ext_seq: Optional[int] = None) -> ChangeSet:
        ...
        for trial in range(MAX_WRITE_CONFLICT_RETRY_COUNT):
            ...
            if trial == 0:
                ...
                revision = ChangeSet.make_from_datasets(self._full_dataset, dataset)
                ...
                # 1. in scope of `pulled_data`
                # 2. exclude Keys already in `commit_changeset`
                revision = ChangeSet.make_from_datasets(
                    {k: v for k, v in self._full_dataset.items() if k in pulled_data and k not in commit_changeset}, 
    ...
    def _update(self, evaluator: Evaluator, ext_seq: Optional[int] = None) -> ChangeSet:
        # ext_seq
        if ext_seq is None:
            ext_seq = -1
        else:
        ...
        for trial in range(MAX_WRITE_CONFLICT_RETRY_COUNT):
            ...
            if trial == 0:
                # evaluate
                val = evaluator.eval(self._full_dataset)
