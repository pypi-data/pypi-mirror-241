...
def test_format_files(tmp_path: pathlib.Path):
    ...
    context.load()
    assert isinstance(context.models["memory.this.model"], SqlModel)
    assert isinstance(context.models["memory.other.model"], SqlModel)
    assert context.models["memory.this.model"].query.sql() == 'SELECT 1 AS "CaseSensitive"'
    assert context.models["memory.other.model"].query.sql() == "SELECT 2 AS another_column"
