...
async def column_lineage(
    ...
    for i, node in enumerate(node.walk()):
        if i > 0:
            ...
            node_name = (
                context._model_fqn_to_snapshot[node_name].name
                if node_name in context._model_fqn_to_snapshot
                else node_name
            )
        ...
        graph[node_name] = {
            column_name: LineageColumn(
                ...
                models=_process_downstream(node.downstream, context._model_fqn_to_snapshot),
            )
        }
    return graph
...
async def model_lineage(
    ...
    return {
        context._model_fqn_to_snapshot[fqn].name: {
            context._model_fqn_to_snapshot[dep].name for dep in deps
        }
