...
class HrAttendanceMitxelena(models.Model):
    ...
    def process_attendance_overtime(self, force_recompute=False):
        ...
        if (
            ...
            self.message_post(body=f"Updated entry: Inheriting values as previous attendance record was already overtime: {previous_attendance.id}")
            self.message_post(body=f"Updated entry: {self.id} Original values for entry: Extra time: {original_entry['extra_time']} \n - Extra time with factor: {original_entry['extra_time_with_factor']}")
            return
