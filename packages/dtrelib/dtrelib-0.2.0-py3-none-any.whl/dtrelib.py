...
class Max(Filter):
    ...
    def Flush(self):
        if self.outfieldIdx != -1:
            self.nextProc.Execute(self.extraValue)
        else:
...
class BestUMTS(Filter):
    def __init__(self):
        ...
        self.lastUsedLog = None
        self.channelList= []
        self.last_ch_sc = None
        self.bandTriggerAllowed = False
        self.zero_list_allow_all = False

    def pocketDump(self):
        if self.getBest:  # если нас просят считать Best,
            self.maxValue = -float("inf")
            self.maxValueIndex = (None, None)
            # перебираем весь карман в поисках максимума
            for key in sorted(list(self.pocket.keys())):
                if (len(self.channelList) == 0 and self.zero_list_allow_all) or (key[0] in self.channelList):
                    rscp = self.pocket[key][2]
                    if rscp > self.maxValue:
                        self.maxValue = rscp
                        self.maxValueIndex = key

            if len(self.pocket.keys()) != 0 and self.maxValueIndex != (None, None):
                return [self.pocket[self.maxValueIndex][0],
                        self.pocket[self.maxValueIndex][1],
                        self.maxValueIndex[0],
                        self.maxValueIndex[1],
                        self.pocket[self.maxValueIndex][2],
                        self.pocket[self.maxValueIndex][3],
                        self.pocket[self.maxValueIndex][4]]

        #TODO: проверить эту ветку
        else:  # иначе просто построчно сбрасываем в буфер (с учетом списка допустимых каналов, если он непуст)
            for key in sorted(list(self.pocket.keys())):
                if (len(self.channelList) == 0) or (key[0] in self.channelList):
                    return [self.pocket[key][0],
                            self.keyDate,
                            key[0],
                            key[1],
    ...
    def Configure(self, config):
        ...
        shortChannelList = '' if 'ch_list' not in config else config['ch_list']
        self.channelList = self.channelList if 'ch_list' not in config else channelListParser(shortChannelList)
        self.bandTriggerAllowed = self.bandTriggerAllowed if 'bandTriggerAllowed' not in config else config['bandTriggerAllowed']
        self.zero_list_allow_all = self.zero_list_allow_all if 'zero_list_allow_all' not in config else config['zero_list_allow_all']
        AbstractProcessor.Configure(self, config)
        # вычисляем положение ключевых колонок
        self.cols = [self.FindColumn('filename'),
                     self.FindColumn('date_time'),
    ...
    def Execute(self, record):
        if not record:
            ...
            if self.ch and self.sc != None and self.ch in self.channelList:
                umtsBands = {
                    range(10562, 10838): '2100',
                    range(2937, 3088): '900'
                }
                ...
                # при этом, если номер канала остался прежним от предудущей итерации, ничего не предпринимаем -
                # это текущее измерение, а не заход на новый цикл
                if ((self.ch, self.sc) in self.pocket and (self.ch, self.sc) != self.last_ch_sc) or \
                        (self.bandTriggerAllowed and (currentBand != self.lastBand)) or \
                    (self.filename != self.lastUsedLog):
                    self.lastUsedLog = self.filename
                    # прежде чем сбросить "карман" в ноль, выводим его накопленное содержимое
                    # (при первом заходе ничего не будет выведено, что нормально)
                    best_result = self.pocketDump()
                    if self.nextnode:
                        self.nextnode.Execute(best_result)
                    self.keyDate = self.date
                    self.last_ch_sc = self.ch, self.sc
                    self.lastBand = currentBand
                    self.pocket.clear()
                    self.pocket[(self.ch, self.sc)] = (self.filename, self.date, self.rscp, self.lon, self.lat)
                else:
                    self.pocket[(self.ch, self.sc)] = (self.filename, self.date, self.rscp, self.lon, self.lat)
...
class BestLTE(Filter):
    def __init__(self):
        ...
        self.lastUsedLog = None
        self.channelList= []
        self.last_ch_pci = None
        self.bandTriggerAllowed = True
        self.zero_list_allow_all = False
        ...
        if self.getBest:  # если нас просят считать Best,
            ...
            if len(self.pocket.keys()) != 0 and self.maxValueIndex != (None, None):
                return [self.pocket[self.maxValueIndex][0],
                        ...
                        self.pocket[self.maxValueIndex][3],
                        self.pocket[self.maxValueIndex][4],
                        self.pocket[self.maxValueIndex][5]]
    ...
    def Execute(self, record):
        if not record:
            ...
            if self.ch and self.pci != None and self.rsrp != None and self.ch in self.channelList:
                lteBands = {
                    range(6150, 6450):  '800',
                    range(3450, 3800):  '900',
                    range(1200, 1950):  '1800',
                ...
                # при этом, если номер канала остался прежним от предудущей итерации, ничего не предпринимаем -
                # это текущее измерение, а не заход на новый цикл
                if ((self.ch, self.pci) in self.pocket and (self.ch, self.pci) != self.last_ch_pci) or \
                        (self.bandTriggerAllowed and (currentBand != self.lastBand)) or \
                    (self.filename != self.lastUsedLog):
                    self.lastUsedLog = self.filename
                    # прежде чем сбросить "карман" в ноль, выводим его накопленное содержимое
                    ...
                    if self.nextnode:
                        self.nextnode.Execute(best_result)
                    self.keyDate = self.date
                    self.last_ch_pci = self.ch, self.pci
                    self.lastBand = currentBand
                    self.pocket.clear()
