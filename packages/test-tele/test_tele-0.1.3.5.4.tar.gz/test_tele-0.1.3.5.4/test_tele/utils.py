...
async def download_from_link(recipient: EntityLike, tm: "TgcfMessage", link: str, count: Union[int, str]=10):
    ...
    command1 = f'gallery-dl --no-part --range 1-{count} {link} --config-ignore -c "$HOME/test_tele/config/config.json"'
    ...
    try:
        ...
        while True:
            ...
            dir_path = os.path.join(dir_path, 'gallery-dl')
            logging.warning(f"{os.getcwd()} {dir_path} {folder_name}")
