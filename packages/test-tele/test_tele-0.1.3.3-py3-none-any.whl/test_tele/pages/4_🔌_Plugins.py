...
from plugin_models import FileType, Replace, Style, Language, SendAs
from password import check_password
from utils_webui import get_list, get_string, hide_st
...
if check_password(st):
    ...
    with st.expander("Special Rizumiya"):
        ...
        CONFIG.plugins.special.download = st.checkbox(
            "Download images from url", value=CONFIG.plugins.special.download
        )
        ...
        CONFIG.plugins.special.send_as = st.selectbox(
            "Choose message delivery format* [BETA]", send_as_Type, index=send_as_Type.index(CONFIG.plugins.special.send_as)
        )
        st.caption("*) specifically, 'send as album' can only be done on picture messages")
