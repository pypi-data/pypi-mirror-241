...
from ..config import KEYWORDS, DATA_CRAWL
...
def get_street_for_district(province_name: str, district_name: str):
    idx_dict = json.load(open("src/Index.json"))
    ...
    if idx_dict.get(province_name) == None:
        return []
    path_json = os.path.join("src/", idx_dict[province_name]["file_path"][2:])
    province_dict = json.load(open(path_json))
    ...
    for province_info in data:
        ...
        for district in districts:
            ...
            district_dict["street"] = get_street_for_district(
                province_name=province_dict["name"], district_name=district_dict["name"]
            )
