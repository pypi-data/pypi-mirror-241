...
def get_args():
    ...
    parser.add_argument("-m", "--sleep-max", required=False, type=float)
    ...
    if kwargs.get("target"):
        target_status_split = kwargs["target"].split(",")
        target_status = [int(s.strip()) for s in target_status_split]
        kwargs["target_status"] = target_status
        del kwargs["target"]
