...
class Operation(object):
    def __init__(self, table: str, pool: Pool = None):
        self.__table = table
        self.__pool = pool
        self.__cols = None
        self.__vals = None
    ...
    def delete(self):
        self.__vals = []
        self.__sql = f'''DELETE FROM {self.__table}'''
        return self
    ...
    def insert(self, **pairs) -> Self:
        ...
        self.__sql = f'''INSERT INTO {self.__table} ({', '.join(keys)}) VALUES ({placeholders})'''
        return self

    def join(self, **conditions):
        for key, value in conditions.items():
            table, column = key.split('__')
            self.__sql = f'{self.__sql} JOIN {table} ON {table}.{column}={value}'
        return self
    ...
    def select(self, *columns):
        ...
        self.__asking = True
        columns = ', '.join(columns) if columns else '*'
        self.__sql = f'''SELECT {columns} FROM {self.__table}'''
        return self
    ...
    def update(self, **pairs):
        self.__vals = []
        self.__sql = f'UPDATE {self.__table} SET'
        start = len(self.__vals)
        equations = []
