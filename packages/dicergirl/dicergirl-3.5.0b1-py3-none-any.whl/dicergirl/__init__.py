...
if initalized:
    ...
    async def bothandler(
        bot: V11Bot, matcher: Matcher, event: Event, args: list = []
    ):
        """机器人管理指令"""
    ...
    @setcommand.handle()
    async def sethandler(bot: V11Bot, matcher: Matcher, event: Event):
        """角色卡设置指令"""
        if not get_status(event) and not event.to_me:
            return
    ...
    @rolecommand.handle()
    async def rolehandler(bot: V11Bot, matcher: Matcher, event: Event):
        """身份组认证"""
        if not get_status(event) and not event.to_me:
            return
    ...
    async def roleobhandler(bot: V11Bot, matcher: Matcher, event: Event):
        """OB 身份组认证"""
        if not get_status(event) and not event.to_me:
            return
