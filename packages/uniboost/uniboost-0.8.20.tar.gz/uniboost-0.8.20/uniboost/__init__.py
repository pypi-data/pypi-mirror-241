...
def print_caller_name():
    ...
    try:
        for frame_info in stack:
            # Skipping the frame of the current function call
            if frame_info[3] != 'print_caller_name':
                # The frame_info tuple format: (frame, filename, lineno, function, code_context, index)
                # frame_info[1] is the filename, frame_info[3] is the function name
                if frame_info[3] == '<module>':
                    print("Imported by:", frame_info[1])
                    return
    finally:
        del stack

# Call this function within the module being imported
print_caller_name()
...
def get_caller_info():
    ...
    # Initialize the variable to hold the filename
    caller_filename = None

    # Find the caller's filename, skipping internal Python frames
    try:
        for frame_info in stack:
            # frame_info is a tuple, and the filename is at index 1
            filename = frame_info[1]  # Get the filename from the tuple
        ...
        # Clean up the stack frames to avoid reference cycles
        del stack
    ...
    return caller_filename, aa_value, sys.argv[1:]

caller_filename, aa_value, arguments = get_caller_info()
caller_filename = print_caller_name
