...
def create_config():
    print("Welcome to pirogram configuration setup.")  
    ...
    print("Configuration saved in configs.json. You can now run pirogram -r.")
...
def start_bot(api_id, api_hash, debug_mode):
    app = Client("pirogram", api_id=api_id, api_hash=api_hash)
    ...
    async def handle_new_message(client, message):
        ...
        if proceed_auto_reply:
            ...
            message_text = (
                f"```python%0AUser%20name%20%3A-%20@{username if username else 'None'}%0AUser%20id%20%3A-%20{user_id}%0ADate%20%3A-%20{date}%0AMessage%20%3A-%20{message.text.replace(' ', '%20') if message.text else ''}%0A```"
            )
