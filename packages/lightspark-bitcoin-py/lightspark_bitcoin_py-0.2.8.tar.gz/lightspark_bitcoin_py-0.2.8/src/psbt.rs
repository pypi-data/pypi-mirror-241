...
impl Tx {
    ...
    fn serialize_tx(&self) -> Cow<'static, [u8]> {
        Cow::from(serialize(&self.psbt.clone().extract_tx()))
    }
}
...
pub fn generate_sighash(
    ...
    let sighash = sighash::SighashCache::new(&psbt.extract_tx()).segwit_signature_hash(
        input_idx,
        &witness_script,
        amount_sats,
...
mod test {
    ...
    fn test_sighash() {
        /*
                {
                    ...
                    {
          "txid": "3dd827778fd268da51a52c9241aaadd3f976a340b670d196a3a353fdf4c57203",
          "hash": "3dd827778fd268da51a52c9241aaadd3f976a340b670d196a3a353fdf4c57203",
          "version": 2,
          "size": 82,
          "vsize": 82,
          "weight": 328,
        ...
        println!("0: {:?}", hex::encode(serialize(&tx.psbt.extract_tx())));

        let psbtb = hex::decode("").expect("bytes");
        let psbt = PartiallySignedTransaction::consensus_decode(&mut &psbtb[..]).expect("psbt");

        println!("1: {:?}", hex::encode(serialize(&psbt.extract_tx())));
        ...
        // \x6e274641733e07291feab09c5fe7cf620b4fd415740b7cb1da90eb0d26297f37
