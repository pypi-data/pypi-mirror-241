...
from .models import (
    ...
    IGPostMetric, IGCarouselMetric, IGReelMetric, IGStoriesMetric, Company, SocialLinks, ProStelaExpert, ProStelaUsage
...
from .forms import (
    ...
    SimpleContentForm, SetFaqForm, ImageContentForm, TextContentForm, AboutContentForm
    )
...
def home(request):
    ...
    try:
        aboutstela=Content.objects.filter(parent__section="Section Stela About", lang=lang)
    except:
        aboutstela = None
    try:
        appstela=Content.objects.filter(parent__section="Section Stela Apps", lang=lang)
    except:
        appstela = None
    try:
        pathstela=Content.objects.filter(parent__section="Section Stela Path", lang=lang)
    except:
        pathstela = None
    try:
        media=Content.objects.filter(parent__section="Section Stela Media", lang=lang)
        ...
        video7 = None
    mediacount=media.count()
    appscount=appstela.count()
    aboutcount=aboutstela.count()
    pathcount=pathstela.count() 
    services=Inventory.objects.filter(type="Service", status="Active")

    context = {
        'aboutstela': aboutstela,
    ...
    return render(request, 'stela_control/site/index.html', context)
...
@login_required
def stelaDocs(request):
    lang=request.LANGUAGE_CODE
    steladocs = SitePolicy.objects.filter(parent__section="Stela Legal", lang=lang)
    context = {
       'steladocs': steladocs,
     }
    return render(request, 'stela_control/site/sections/steladocs.html', context)
...
#developer 
@user_passes_test(lambda u: u.is_superuser, login_url='/')
def homeDeveloper(request):
    lang=request.LANGUAGE_CODE
    sections=TemplateSections.objects.filter(lang=lang)
    styles=Content.objects.all()
    categ=Category.objects.all()

    tempsecform=formset_factory(TempSecForm, formset=RequiredFormSet, extra=1)
    stylesform=formset_factory(StylesForm, formset=RequiredFormSet, extra=1)
    categform=formset_factory(categForm, formset=RequiredFormSet, extra=1)
    formset=tempsecform(prefix='tempsec')
    formset3=stylesform(prefix='styles')
    formset4=categform(prefix='categ')

    if request.method == 'POST':
        form_id = request.POST.get('form-id')
        print(form_id)
        if form_id == 'tempsec':
            formset=tempsecform(request.POST, prefix='tempsec')
            if formset.is_valid():

                for loop in formset:
                    obj=loop.save(commit=False)
                    obj.lang = lang
                    obj.save()
        ...
        elif form_id == 'stylesForm':
                formset=stylesform(request.POST, request.FILES, prefix='styles')
                if formset.is_valid():
    ...
    return render(request, 'stela_control/developer/home.html', context)
...
def updateData(request):
    ...
    if request.POST.get('action') == "tempsecDelete":
        objects_ids = request.POST.getlist('id[]')
        for id in objects_ids:
            object = TemplateSections.objects.get(pk=id)
            object.delete()
        response = JsonResponse({'success': 'return something'})
        return response
    
    if request.POST.get('action') == "colorsDelete":
        objects_ids = request.POST.getlist('id[]')
        print(objects_ids)
        for id in objects_ids:
            object = StelaColors.objects.get(pk=id)
            object.delete()
        response = JsonResponse({'success': 'return something'})
        return response
    
    if request.POST.get('action') == "stylesDelete":
        objects_ids = request.POST.getlist('id[]')
        print(objects_ids)
        for id in objects_ids:
            object = Content.objects.get(pk=id)
            object.delete()
        response = JsonResponse({'success': 'return something'})
        return response

    if request.POST.get('action') == "categDelete":
        objects_ids = request.POST.getlist('id[]')
        print(objects_ids)
        for id in objects_ids:
    ...
    if request.POST.get('action') == "dataSections":
        temp_id=request.POST.get('tempid')
        obj=TemplateSections.objects.get(pk=temp_id)
        form=TempSecForm(instance=obj)

        obj_data = render_to_string('stela_control/developer/data-update/modal1.html', {
                        'form': form,    
                        'obj': obj  
            })
    ...
    if request.POST.get('action') == "dataColors":
        temp_id=request.POST.get('tempid')
        obj=StelaColors.objects.get(pk=temp_id)
        form=ColorsForm(instance=obj)

        obj_data = render_to_string('stela_control/developer/data-update/modal2.html', {
                        'form': form,   
                        'obj': obj   
            })
    ...
    if request.POST.get('action') == "dataStyles":
        temp_id=request.POST.get('tempid')
        obj=Content.objects.get(pk=temp_id)
        form=StylesForm(instance=obj)
        obj_data = render_to_string('stela_control/developer/data-update/modal3.html', {
                        'form': form,   
                        'obj': obj
            })          
    ...
    if request.POST.get('action') == "dataCateg":
        temp_id=request.POST.get('tempid')
        obj=Category.objects.get(pk=temp_id)
        form=categForm(instance=obj)

        obj_data = render_to_string('stela_control/developer/data-update/modal4.html', {
                        'form': form,   
                        'obj': obj   
            })
    ...
    if request.method == 'POST':
        ...
        if form_id == 'tempsec':
            data=TemplateSections.objects.get(pk=obj_id)
            form=TempSecForm(request.POST, instance=data)
            if form.is_valid():
            ...
            data=Content.objects.get(pk=obj_id)
            form=StylesForm(request.POST, request.FILES, instance=data)
            if form.is_valid():
                form.save()
...
def stelaMain(request):
    ...
    get_formset2 = formset_factory(StelaAboutForm, formset=RequiredFormSet, extra=3)
    formset2=get_formset2(prefix='about')
    form2 = AppstelaForm()
    get_formset3 = inlineformset_factory(
        Content, DynamicBullets, 
        form=BulletDescripForm,
        extra=3, can_delete=False,
        )
    formset3=get_formset3(prefix='app')

    get_formset4 = formset_factory(PathForm, formset=RequiredFormSet, extra=2)
    formset4=get_formset4(prefix='path')

    get_formset5 = inlineformset_factory(
            TemplateSections, Content, 
            form=MediaForm,
            extra=1, can_delete=True,
            )
    formset5=get_formset5(prefix='media')

    form3 = FooterContentForm(prefix="footer")
    ...
    if request.method == 'POST':
        ...
        if form_id == "newAbout":
            formset2 = get_formset2(request.POST, request.FILES, prefix='about')
            templates=TemplateSections.objects.all()
            print(templates)
            if formset2.is_valid():
                for form in formset2:
                    child = form.save(commit=False)
                    child.owner = request.user 
                    child.parent = TemplateSections.objects.get(section='Section Stela About')
                    child.lang = lang
                    child.save()

                messages.success(request, _("Changes made successfully"))
                return redirect('/dynamic-content/stela/main')
        ...
        if action == "dataAbout":
            ...
            obj=TemplateSections.objects.get(section="Section Stela About", lang=lang)
            get_formset = inlineformset_factory(
            TemplateSections, Content, 
            form=StelaAboutForm,
            extra=0, can_delete=True,
            )
            formset=get_formset(instance=obj, prefix='updateabout')

            obj_data = render_to_string('stela_control/load-data/aboutstela/modal.html', { 
                            'formset': formset,   
                            'obj': obj  
                })
        ...
        if form_id == "updateAbout":
            ...
            formset = set_formset(request.POST, request.FILES, prefix='updateabout', instance=obj)  
            ...
            if formset.is_valid():
                ...
                for instance in instances:
                    ...
                    instance.parent = TemplateSections.objects.get(section='Section Stela About')
                    instance.lang = lang
                    instance.save()

                messages.success(request, _("Changes made successfully"))
                return redirect('/dynamic-content/stela')
            else:
                print(formset.errors, form.errors)
        
        if form_id == "newApp":
            form2 = AppstelaForm(request.POST, request.FILES)
            formset3 = get_formset3(request.POST, prefix='app')
            ...
            if all([form2.is_valid(), 
                    ...
                    parent.owner = request.user 
                    parent.parent = TemplateSections.objects.get(section='Section Stela Apps', lang=lang)
                    parent.lang = lang
                    parent.save()
        ...
        if action == "dataApps":        
            obj=request.POST.get('appid')
            app=Content.objects.get(pk=obj)
            form2=AppstelaForm(instance=app, prefix='app')
            get_formset3 = inlineformset_factory(
            Content, DynamicBullets, 
            form=BulletDescripForm,
            extra=0, can_delete=True,
            )
            formset3=get_formset3(instance=app, prefix='updateapp')

            obj_data = render_to_string('stela_control/load-data/appstela/modal.html', { 
                            'form2': form2,
                            'formset3': formset3, 
                            'obj': obj   
                })
        ...
        if form_id == "updateApp":
            ...
            form2 = AppstelaForm(request.POST, request.FILES, instance=obj, prefix='app')
            formset3 = set_formset(request.POST, prefix='updateapp', instance=obj)  
        ...
        if form_id == "newPath":
            formset = get_formset4(request.POST, prefix='path')
            
            if formset.is_valid():
                for form in formset:
                    child = form.save(commit=False)
                    child.owner = request.user 
                    child.parent = TemplateSections.objects.get(section='Section Stela Path', lang=lang)
                    child.lang = lang
                    child.save()
        ...
        if action == "dataPath":
            ...
            obj=TemplateSections.objects.get(section="Section Stela Path", lang=lang)
            get_formset = inlineformset_factory(
            TemplateSections, Content, 
            form=PathForm,
            extra=0, can_delete=True,
            )
            formset=get_formset(instance=obj, prefix='updatepath')

            obj_data = render_to_string('stela_control/load-data/stelapath/modal.html', { 
                            'formset4': formset,   
                })
        ...
        if form_id == "updatePath":
            ...
            formset = set_formset(request.POST, prefix='updatepath', instance=obj)  
            ...
            if formset.is_valid():
                ...
                for instance in instances:
                    instance.owner = request.user 
                    instance.parent = TemplateSections.objects.get(section='Section Stela Path')
                    instance.lang = lang
                    instance.save()
        ...
        if form_id == "newMedia":
            formset = get_formset5(request.POST, request.FILES, prefix='media')
            
            if formset.is_valid():
                for form in formset:
                    child = form.save(commit=False)
                    child.owner = request.user 
                    child.parent = TemplateSections.objects.get(section='Section Stela Media', lang=lang)
                    child.lang = lang
                    child.save()
        ...
        if action == "dataMedia":
            ...
            obj=TemplateSections.objects.get(section="Section Stela Media", lang=lang)
            set_formset5 = inlineformset_factory(
            TemplateSections, Content, 
            form=MediaForm,
            extra=0, can_delete=True,
            )
            formset5=set_formset5(instance=obj, prefix='updatemedia')

            obj_data = render_to_string('stela_control/load-data/stelamedia/modal.html', { 
                            'formset5': formset5,   
        ...
        if form_id == "updateMedia":
            ...
            formset = set_formset5(request.POST, request.FILES, prefix='updatemedia', instance=obj)  
            ...
            if formset.is_valid():
                ...
                for instance in instances:
                    instance.owner = request.user 
                    instance.parent = TemplateSections.objects.get(section='Section Stela Media')
                    instance.lang = lang
                    instance.save()
    ...
    return render(request, 'stela_control/content/stela/main.html', {
            'aboutcount': aboutcount,
            'aboutstela': aboutstela,
            'appstela': appstela,
            'appscount': appscount,
...
def stelaDocsContent(request):
    lang=request.LANGUAGE_CODE
    stelacontent=TemplateSections.objects.filter(section="Section Stela Apps", lang=lang)
    appcontent=Content.objects.filter(parent__section="Section Stela Apps", lang=lang)
    integration=Inventory.objects.filter(type="Service", lang=lang).exclude(yearly=True)
    steladocs = SitePolicy.objects.filter(parent__section="Stela Legal", lang=lang)
    ...
    if request.method == 'POST':
        ...
        if form_id == "newDocs":
            ...
            formset = get_formset(request.POST, prefix='form')
            
            if all([form.is_valid(), 
                    formset.is_valid(),
                    ]):
                    parent = form.save(commit=False)
                    parent.parent = TemplateSections.objects.get(section='Stela Legal', lang=lang)
                    parent.policy = _('Legal provision')
                    parent.lang = lang
                    parent.save()
                    ...
                    messages.success(request, _("Changes made successfully"))
                    return redirect('/dynamic-content/stela/docs')
        ...
        if action == "stelaDocs":
            obj_id=request.POST.get('parentid')
            obj=SitePolicy.objects.get(pk=obj_id)
            ...
            formset=get_formset(instance=obj, prefix='updateterms')

            obj_data = render_to_string('stela_control/load-data/stela-docs/formset-docs.html', {
                            'form': form, 
                            'formset': formset,   
                            'obj': obj  
                })
        ...
        if form_id == "updateDocs":
            pk=request.POST.get('obj-id')
            policy=SitePolicy.objects.get(pk=pk)
            set_formset = inlineformset_factory(
                SitePolicy, LegalProvision, 
                form=LegalProvitionForm,
                extra=0, can_delete=True,
            ...
            formset = set_formset(request.POST, prefix='updateterms', instance=policy)  
        
            if all([form.is_valid(),
                    formset.is_valid(),
                    ]):
                    parent = form.save(commit=False)
                    parent.parent = TemplateSections.objects.filter(section='Stela Legal', lang=lang)
                    parent.lang = lang
                    parent.save()
        ...
        if action == 'deleteDoc':
            id = request.POST.get('id')
            obj = SitePolicy.objects.get(pk=id)
            obj.delete()
        
            response = JsonResponse({'success': 'return something'})
            return response
    
    return render(request, 'stela_control/content/stela/steladocs.html', {
                'form': form,   
                'formset': formset,
                'stelacontent': stelacontent,
...
#     if q: 
#         comment = Comment.objects.filter(product__title__icontains=q).order_by('-id')

#     if date:
#         date_min = datetime.datetime.combine(datetime.date.today() - datetime.timedelta(days=int(date)), datetime.time.min)
#         today = timezone.now()
#         comment = Comment.objects.filter(created_at__range=[date_min, today]).order_by('-id')
...
#     return render(request, 'stela_control/reviews/reviews_list.html', context)
...
#     return render(request, 'stela_control/reviews/update_review.html', context)
...
# def autocompleteReview(request):
#   if 'term' in request.GET:
#       q = Comment.objects.filter(product__title__icontains=request.GET.get('term'))
#       titles = list()
#       for search in q:
#           titles.append(search.product.title)
#       return JsonResponse(titles, safe=False)
...
# def autocompleteOrder(request):
#   if 'term' in request.GET:
#       q = Order.objects.filter(transaction_id__icontains=request.GET.get('term'))
#       titles = list()
#       for product in q:
#           titles.append(product.transaction_id)
#       return JsonResponse(titles, safe=False)
