...
def clean_markdown_code_blocks(text):
    start_pattern = r"^```[^\n]*\n"
    end_pattern = r"\n```$"
    text = re.sub(start_pattern, "", text, flags=re.MULTILINE)
    text = re.sub(end_pattern, "", text, flags=re.MULTILINE)
    return text
...
class TvorozhokFSM:
    ...
    def highlight_text(self, text, language="text"):
        try:
            lexer = get_lexer_by_name(language, stripall=True)
            formatter = TerminalFormatter()
            highlighted_text = highlight(text, lexer, formatter)
        except ClassNotFound:
            print(f"Warning: Language '{language}' not found, defaulting to text.")
    ...
    def pulling_repo(self):
        repo_name = input("Enter the name of the repository to pull changes: ")
        repo_path = os.path.join(GIT_REPOS_DIR, repo_name)
    ...
    def removing_repo(self):
        repo_name = input("Enter the name of the repository to remove: ")
        repo_path = os.path.join(GIT_REPOS_DIR, repo_name)
    ...
    def continue_dialogue(self):
        ...
        # Offer to continue a saved dialogue or start a new one
        ...
        if selected_option == "Start a new dialogue":
            self.get_system_context()  # Make sure this method allows for quitting as well
            self.transition(State.IN_DIALOGUE)
        else:
            # Load the saved dialogue and transition to the IN_DIALOGUE state
        ...
        for root, _, files in os.walk(repo_path):
            for file in files:
                if (file.endswith(".py") and file in file_names) or (
                    file_names is None or file_names == "all"
                ):
    ...
    def dialogue_interaction(self):
        ...
        language = (
            input(
                "If the response is code, specify the language (or 'text' if it is not code): "
            )
            .strip()
            .lower()
        )
        ...
        while True:
            ...
            if user_message.lower() in ["quit", "q"]:
                if self.context["messages"]:
                    if input("Do you want to save this dialogue? [Y/n] ").lower() in [
                        ...
                        dialogue_name = input("Enter a name for this dialogue: ")
            ...
            try:
                ...
                chatgpt_message = clean_markdown_code_blocks(
                    response["choices"][0]["message"]["content"]
                )

                # Use the highlight_text method to print the response with syntax highlighting
                highlighted_message = self.highlight_text(
                    chatgpt_message, language=language
                )
                print(f"[ChatGPT]: {highlighted_message}")
    ...
    def load_saved_dialogue(self, dialogue_name):
        ...
        try:
            with open(dialogue_path, "r") as dialogue_file:
                self.context["messages"] = json.load(dialogue_file)
            print(f"Dialogue '{dialogue_name}' has been loaded as system context.")
        except IOError as e:
            print(f"An error occurred while loading the dialogue: {e}")
