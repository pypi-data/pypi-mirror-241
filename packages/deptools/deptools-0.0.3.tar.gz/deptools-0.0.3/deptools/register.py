...
def get_french_name(packageid):
    ...
    if packageid.key == 'fwupdater-cli':
        return "Utilitaire pour mise à jour du micrologiciel (CLI)"
    if packageid.key == 'fwupdater-gui':
        return "Utilitaire pour mise à jour du micrologiciel (GUI)"
    if packageid.key == 'fwupdater':
        return "Utilitaire Windows pour mise à jour du micrologiciel"
    if packageid.key in ['ATS-GPU-BASE', 'ATS-GPU-OCT', 'ATS-GPU-NUFFT', 'ATS-SDK']:
    ...
    for packageid in installers:
        packages.append(
            Package(os=packageid.os,
                    ...
                    name_french=get_french_name(packageid).replace("\u00e0", "à")))
        ...
        if tb_key != '':
            ...
            packages.append(
                Package(os=tb_packageid.os,
                        ...
                        name_french=get_french_name(tb_packageid).replace("\u00e0", "à")))
