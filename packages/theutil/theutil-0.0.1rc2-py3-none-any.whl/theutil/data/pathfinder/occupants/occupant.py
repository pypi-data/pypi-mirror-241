...
from theutil.data.pathfinder import Trigger, Target
...
class Occupant(ADataObject):
    ...
    Attributes:
        ...
        congestion_time_total (float):Total time the occupant spent moving at less than the congestion Velocity as specified in the simulation parameters (see `Section 14.1.2<https://support.thunderheadeng.com/docs/pathfinder/latest/user-manual/#_output_parameters>`_).
        ...
        last_goal_started (float): The time at which the occupant started their last behavior action.
            The default value is infinity, representing that the occupant did not report starting their last behavior action.
        plus_time (float): The time at which an occupant with a Goto Refuge Rooms action reached their refuge room or an occupant with a Wait Until Simulation End action begain waiting. 
            See `Section 5.3.3<https://support.thunderheadeng.com/docs/pathfinder/latest/user-manual/#_behavior_action_types>`_ for more information.
            The default value is infinity, representing that the Occupant did not report this value.
        safe_time (float): The total amount of time the occupant spent in a refuge room.
            The default value is infinity, representing that the Occupant did not report this value.
    ...
    def _load_tags_data(self) -> None:
        ...
        try:
            for key, data in self._tags.items():
                self._tags[key] = Tag(name=key, data=data)
            self.tags = TagCollection(self._tags)
        except:
            self.tags = TagCollection([])
            self._log.warn(f'There was an error parsing tags for Occupant "{self.name}"')
...
class OccParams(ADataObject):
    ...
    Args:
        params_data (Optional[Dict]): Dict of raw param data for the Occupant.
    ...
    Attributes:
        ...
        shape (Dict): Dict of shape info assigned to this Occupant.
...
class SocialDistanceFrame(ADataObject):
    ...
    Attributes:
        group_id (Optional[int]): Unique ID for occupant group, if present
        closest_occupant (Optional[Occupant]): Closest Occupant to parent Occupant during the time step
    ...
    def __init__(self, *, frame_data: Dict, parent: Occupant):
        ...
        self.group_id: Optional[int] = None
        self.closest_occupant: Optional[Occupant] = None
    ...
    def _update_occupant_references(self):
        if self._closest != None:
            self.closest_occupant = self.parent.parent.occupants.get_by_id(int(self._closest["id"]))
