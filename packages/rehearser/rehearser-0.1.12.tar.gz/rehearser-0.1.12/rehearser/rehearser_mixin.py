...
class RehearserMixin(object):
    def get_interactions(self) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        """
        return {
            GUMMIES_INTERACTIONS_FILE_TYPE: self._rehearser_type,
    ...
    def get_filename_path(self) -> str:
        ...
        component_name = (
            ...
            if self._rehearser_type == SupportedInteractionsType.PYTHON_INSTANCE.name
            else self._obj.__name__
        )
        filenamepath += f"{component_name}/"
    ...
    def write_interactions_to_s3(self, bucket_name: Optional[str]=None, s3_key: Optional[str]=None):
        ...
        doc_str = json.dumps(self.get_interactions_serializable_json(), indent=2)
        print(
            f"Uploading interactions to S3 bucket: {bucket_name}, key: {s3_key}, with {len(doc_str)} chars"
        )
        s3.put_object(Body=doc_str, Bucket=bucket_name, Key=s3_key)
