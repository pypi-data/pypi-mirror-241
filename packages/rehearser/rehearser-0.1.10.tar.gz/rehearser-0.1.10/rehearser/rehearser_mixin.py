...
class RehearserMixin(object):
    ...
    def get_interactions_file_path(self) -> str:
        ...
        if not self._interactions_file_path:
            ...
            directory += f"{self._scenario_name}/" if self._scenario_name else ""
            component_name = (
                self._obj.__class__.__name__
                if self._rehearser_type
                == SupportedInteractionsType.PYTHON_INSTANCE.name
                else self._obj.__name__
            )
            directory += f"{component_name}/"
            directory += f"{self._entity_id}/" if self._entity_id else ""
            if self._use_timestamp:
                filename = (
                    f"{datetime.datetime.now().strftime('%Y%m%d_%H%M%S%f')}_interactions.json"
                )
            else:
