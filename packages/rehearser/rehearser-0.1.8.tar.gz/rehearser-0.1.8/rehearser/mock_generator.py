...
from rehearser.constants import GUMMIES_INTERACTIONS_FILE_TYPE, INTERACTIONS_KEY, InteractionType, SupportedInteractionsType
...
class MockGenerator:
    def __init__(
        ...
        if isinstance(interactions_src, dict):
            self._interactions_src_type = interactions_src.get(GUMMIES_INTERACTIONS_FILE_TYPE, SupportedInteractionsType.PYTHON_INSTANCE.name)
            print(f"[MockGenerator]_interactions_src_type: {self._interactions_src_type}")
            if self._interactions_src_type in (SupportedInteractionsType.PYTHON_INSTANCE.name, SupportedInteractionsType.PYTHON_CALLABLE.name):
                interactions_src = interactions_src[INTERACTIONS_KEY]
            else:
                raise NotImplementedError(f"Unsupported interactions type: {interactions_src[GUMMIES_INTERACTIONS_FILE_TYPE]}")
        ...
        if not self._interactions_src_type:
            raise NotImplementedError(f"Unsupported interactions type: {type(interactions_src)}")

        # interactions_src is supposed to be a list
        interactions = from_serializable_interactions(interactions_src)
        # interactions_src is supposed to be a list
        for interaction in interactions:
            if interaction["type"] in (InteractionType.INSTANCE_METHOD_CALL.name, InteractionType.METHOD_CALL.name):
    ...
    def create_mock(self)->Union[MagicMock, Mock, Callable]:
        ...
        if self._interactions_src_type == SupportedInteractionsType.PYTHON_INSTANCE.name:
            ...
            return mock
        elif self._interactions_src_type == SupportedInteractionsType.PYTHON_CALLABLE.name:
