...
class SeleniumManagerBase:
            ...
            for _action in self.actions:
                ...
                if _callback := _action.get('callback'):
                    ...
                    if _callback_function := self.external_functions_mapping.get(_callback_function_name):
                        _callback_args = _callback.get('args', [])
                        _callback_function(*_callback_args)
                    else:
    ...
    def prepare_action(self, action: dict):
        return {self.__get_env__(_key): self.__get_env__(_value) for _key, _value in action.items()}
    ...
    def __get_env__(self, search_value):
        _search_value = str(search_value)
        if not _search_value.startswith('{{') and not _search_value.endswith('}}'):
            return search_value
        _search = _search_value.removeprefix('{{').removesuffix("}}").strip()
...
class SeleniumManager(SeleniumManagerBase):
    ...
    def _external_function(self, action: dict):
        ...
        if _mapped_function := self.external_functions_mapping.get(_function_mapping_name):
            _mapped_function.get('function')(*_function_args)
            return
