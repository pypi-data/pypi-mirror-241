...
def load(filename):
    ...
    if (
        ...
        or filename.endswith(".pml")
        or filename.endswith(".prom")
    ):
        dst = os.path.basename(filename) + ".spins"
        if not os.path.exists(dst) or (
            os.path.getmtime(dst) < os.path.getmtime(filename)
        ):
            p = subprocess.run(
                ["spins", filename],
...
class model:
    def kripke(
        self, ap_set, dict=spot._bdd_dict, dead=spot.formula_ap("dead"), compress=2
    ):
    ...
    def info(self):
        ...
        res["state_size"] = ss
        res["variables"] = [
            (self.state_variable_name(i), self.state_variable_type(i))
            for i in range(ss)
        ]
        tc = self.type_count()
        res["types"] = [
            (
                self.type_name(i),
                [self.type_value_name(i, j) for j in range(self.type_value_count(i))],
            )
            for i in range(tc)
        ]
        return res
        ...
        if tool == "divine":
            ...
            try:
                out = subprocess.check_output(
                    ["divine", "compile", "--help"], stderr=subprocess.STDOUT
                )
            except subprocess.CalledProcessError:
                print("divine does not understand 'compile --help'", file=sys.stderr)
                sys.exit(77)
            if b"LTSmin" not in out:
                print("divine available but no support for LTSmin", file=sys.stderr)
                sys.exit(77)
        elif tool == "spins":
