        ...
        def operation(param, param_locator, value):
            ...
            if param_locator.get('type') in ('text', 'fill'):
                ...
                if isinstance(value, list):  # value按照table_tr()的传参方式传参,
                    self.table_tr(new_table, param_locator.get('locator'), value, self.locale, param,
                                  agg_function=self.agg_in)
                else:
                    logging.error(f'value {value} should be a list, not {type(value)}')
            elif param_locator.get('type') == 'grid':
                new_grid = []
                for one in param_locator.get('grid'):
                    one[1].update({'locator': self._get_locator(one[1].get('locator'))})
                    new_grid.append(one)
                if isinstance(value, list):  # value按照table_tr()的传参方式传参,
                    self.grid(new_grid, param_locator.get('locator'), value, self.locale, param,
                                  agg_function=self.agg_in)
                else:
                    logging.error(f'value {value} should be a list, not {type(value)}')
