...
def kill_windows_port(ip_, port: int or list) -> None:
    ...
    def kill_one_port(one_port: int):
        result = subprocess.run(['netstat', '-ano', '|', 'findstr', str(one_port)], stdout=subprocess.PIPE)
        ...
        for host in host_or_ip:
            ...
            if assert_result:
                ...
                if lost_packets:  # 判断需要丢包
                    send = re.findall(r'已发送|Sent = (.*?)，', result, re.S)[0]
                    accept = re.findall(r'已接收|Received = (.*?)，', result, re.S)[0]
                    if send == accept:
                        if ('无法访问' not in result) and ('unreachable' not in result):
                            logging.info(f'PC {src} ping {host} does not loss packet')
                            tag_result = False
                else:  # 判断不丢包
                    if not (('丢失 = 0' in result) or ('Lost = 0' in result)):
                        logging.info(f'PC {src} ping {host} loss packet')
                        tag_result = False
                    else:
                        if ('无法访问' in result) or ('unreachable' in result):
                            logging.info(f'PC {src} ping {host} loss packet')
                            tag_result = False
            else:
                os.popen(command)
