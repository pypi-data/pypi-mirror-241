...
class Protocol(BasePage, IgLocators):
    ...
    def __turn_page(self, name: str, type_='add'):
        ...
        page_number = 1
        if type_ == 'add':
            locator = list(filter(lambda x: x[0] == 'name', self.edge_locators.protocol_modbus_mapping_table(name)))[0][
                1].get('locator')
            page_l = self.edge_locators.page_father.get('measure')
        else:
            locator = \
                list(filter(lambda x: x[0] == 'name', self.edge_locators.protocol_modbus_mapping_batch_table(name)))[0][
                    1].get('locator')
    ...
    def config(self, type_='modbus_tcp_slave', **kwargs):
        ...
        self.agg_in(self.edge_locators.protocol_modbus_tcp_locator, kwargs)
        if kwargs.get('mapping'):
            for ac in kwargs.get('mapping'):
                if ac[0] == 'add':
                    self.agg_in(self.edge_locators.protocol_modbus_tcp_operation, {ac[0]: True})
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table_add, ac[1])
                elif ac[0] == 'batch_add':
                    ...
                    if type_ == 'opcua_server':
                        self.agg_in(self.edge_locators.protocol_modbus_tcp_operation, {'add': True})  # opcua 批量添加的按鈕就是 add 按鈕
                    else:
                        self.agg_in(self.edge_locators.protocol_modbus_tcp_operation, {ac[0]: True})
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table_batch_add, ac[1])
                    if mapping:
                        for m in mapping:
                            self.__turn_page(m[1], 'batch')
                            self.agg_in(self.edge_locators.protocol_modbus_mapping_batch_table(m[1]), {m[0]: m[2]})
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table_batch_add,
                                {"confirm": confirm, "tip_messages": tip, "text_messages": text, "cancel": cancel})
                elif ac[0] == 'edit':
                    self.__turn_page(ac[1])
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table(ac[1]), {ac[0]: True})
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table_add, ac[2])
                elif ac[0] == 'search':
                    self.agg_in(self.edge_locators.protocol_modbus_tcp_operation, dict_merge(ac[1], {'search': True}))
                elif ac[0] in ('import', 'export', 'page_number', 'check_all'):
                    self.agg_in(self.edge_locators.protocol_modbus_tcp_operation, {ac[0]: ac[1]})
                elif ac[0] == 'check':
                    self.__turn_page(ac[1])
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table(ac[1]), {'check': ac[2]})
                elif ac[0] == 'delete_bulk':
                    self.agg_in(self.edge_locators.protocol_modbus_tcp_operation, {'check_all': False})  # 先取消选择所有的
                    all_measure = [ac[1]] if isinstance(ac[1], str) else ac[1]
                    self.config(type_, modbus_mapping=[('check', x, True) for x in all_measure])
                    self.agg_in(self.edge_locators.protocol_modbus_tcp_operation,
                    ...
                    self.__turn_page(ac[1])
                    self.agg_in(self.edge_locators.protocol_modbus_mapping_table(ac[1]),
