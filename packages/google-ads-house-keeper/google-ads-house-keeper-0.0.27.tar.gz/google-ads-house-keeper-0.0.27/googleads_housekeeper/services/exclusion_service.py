...
class NegativePlacementsList(BaseQuery):
    ...
    def __init__(self, name):
        self.query_text = f"""
            ...
            AND shared_set.type = 'NEGATIVE_PLACEMENTS'
...
class PlacementExcluder:
    ...
    def _create_placement_operation(
            ...
            exclusion_level: ExclusionLevelEnum) -> tuple[str, Any, bool]:
    ...
    # TODO: Working
    def _create_shared_set(self, customer_id, campaign_id) -> str:
        # TODO: Name should be customized
        name = f"API Negative placements list - Campaign: {campaign_id}"
        # TODO: Init only once
        report_fetcher = AdsReportFetcher(self._client)
        # TODO: fetch report once for all negative placements and when just get data from it
        resource_name = report_fetcher.fetch(NegativePlacementsList(name),
                                             customer_id)
        if resource_name:
    ...
    def _get_share_set(self, name: str, level: str):
        # TODO: Use report_fetcher to resource_name for a particular share set
        ...

    # TODO: Does not work for VIDEO campaigns
