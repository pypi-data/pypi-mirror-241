...
from morphonet.tools import isfile, rmrf, cp_dir, printv, convert_vtk_file_in_obj, RemoveLastTokensFromPath, get_version
...
class Plot:  # Main function to initalize the plot mode
    ...
    def __init__(self, log=True, start_browser=False, port_send=9875, port_recieve=9876, memory=20,temp_folder=".TEMP",full_path=False,verbose=1,start_servers=True,check_version=True):
        ...
        self.show_raw = False #Is user ask for showing raw
    ...
    def curate(self,load_default_plugins=True):  # START UPLOAD AND WAIT FOR ANNOTATION
        ...
        self.plot_raw(self.dataset.begin) #Activate the first time step only of no meshs
    ...
    def compute_mesh(self, t, data=None):
        ...
        obj = convert_to_OBJ(data, t, background=self.dataset.background, factor=self.factor, center=self.dataset.get_center(),VoxelSize=self.dataset.get_voxel_size(t),
                             cells_updated=self.dataset.cells_updated[t],path_write=obj_path,write_vtk=True,force_recompute=self.recompute,
