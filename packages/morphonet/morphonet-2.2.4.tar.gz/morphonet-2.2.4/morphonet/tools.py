...
class calcul_regionprops_thread(Thread):
    ...
    def run(self):  # START Compute regions
        if self.t in self.dataset.seg_datas:
            ...
            self.sr=save_properties_thread(self.dataset,self.t,self.filename,self.regionprops_name,self.send_properties)
            self.sr.start()
...
class save_properties_thread(Thread):
    ...
    def run(self):  # START Save properties
        ...
        for name in self.regionprops_name:
            self.dataset.regionprops[name].computed_times[self.t] = True #Set the region fully computed at this time point
