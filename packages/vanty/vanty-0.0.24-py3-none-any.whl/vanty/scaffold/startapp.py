...
def startapp(app_name: str, destination: str = "apps", override: bool = False):
    ...
    try:
        ...
        temp_template_url = 'https://cdn.advantch.com/template/app_template.zip'
        
        # check no dir exists with the same name
        if Path(app_name).exists():
            typer.echo(f"Error: Directory with name '{app_name}' already exists. please choose a different name.")
            raise typer.Exit(code=1)
