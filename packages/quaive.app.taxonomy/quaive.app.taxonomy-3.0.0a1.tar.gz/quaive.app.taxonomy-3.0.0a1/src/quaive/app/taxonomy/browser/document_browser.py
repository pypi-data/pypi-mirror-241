...
class DocumentBrowserView(Sidebar):
    ...
    title = _("label_title_app_document_browser", default="Document browser")
    ...
    def items(self):
        ...
        if self.vocabulary_index not in search_util.facet_fields:
            log.error(
                "There is no search facet for the vocabulary index named: {}".format(
        ...
        if self.vocabulary_name:
            if self.search_query:
                searchable_text = "{}*".format(search_query)
                ...
                for token in term_tokens:
                    if token.startswith(search_query):
                        term_tokens = [
                            i for i in term_tokens if i.startswith(search_query)
                        ]
        ...
        results = search_util.query(searchable_text, filters=filters, step=9999)
        ...
        for result in results:
            ...
            cls_desc = "has-description" if result.description else "has-no-description"
            ctype = proto_view.friendly_type2type_class(result.friendly_type_name)
            ...
            item["mime-type"] = ""
            item["url"] = result.url
            item["id"] = result.getId() if callable(result.getId) else result.getId
    ...
    @property
    def breadcrumb(self):
        app_obj_url = self.context.absolute_url()
        view_name = self.__name__
        app_url = f"{app_obj_url}/@@{view_name}"
        if not self.term_id:
            return None
        else:
            ...
            if len(term_path) < 2:
                # A top level term
                return {"url": app_url, "title": _("Document browser")}
            else:
    ...
    def search_placeholder(self):
        ...
        if term:
            return _(
                "search_within_placeholder",
                default="Search within the new '${sterm}'",
                mapping={"sterm": format(term.value)},
            )
...
class TaxonomySearchTile(GroupedSearchTile):
    ...
    def search_results(self):
        ...
        if self.vocabulary_index not in search_util.facet_fields:
            log.error(
                "There is no search facet for the vocabulary named: {}".format(
    ...
    def results_by_author(self):
        ...
        for result in self.search_results():
            user_id = result.get("Creator", _("Unknown"))
        ...
        for user_profile in user_profiles:
            by_user_name[user_profile.title] = by_user_profile_paths[user_profile.path]
