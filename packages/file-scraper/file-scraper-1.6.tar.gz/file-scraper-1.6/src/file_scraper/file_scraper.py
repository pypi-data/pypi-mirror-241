...
def validate_template_decode(obj):
	...
	for key in obj:
		if subkey in obj[key]:
			if not isinstance(obj[key][subkey], str) or len(obj[key][subkey]) < 1 or obj[key][subkey].lower() not in ["base64", "url", "hex"]:
				obj[key].pop(subkey, None)
			else:
			...
			if "decode" in subtemplate:
				try:
					if subtemplate["decode"] == "base64":
						decoded = self.__replace(base64.b64decode(match), binary = True)
						if decoded:
							tmp["decoded"].append(decoded)
							tmp["matched"].append(match)
					elif subtemplate["decode"] == "url":
						decoded = self.__replace(urllib.parse.unquote(match, encoding = self.__url_enc), binary = False)
						if decoded:
							tmp["decoded"].append(decoded)
							tmp["matched"].append(match)
					elif subtemplate["decode"] == "hex":
						decoded = self.__replace(bytes.fromhex(match.replace("0x", "").replace("\\", "").replace("x", "")), binary = True)
						if decoded:
							match = match.replace("\\\\", "\\")
							tmp["decoded"].append(decoded)
							tmp["matched"].append(match)
...
class Validate:
	...
	def __basic(self):
		...
		print("File Scraper v1.6 ( github.com/ivan-sincek/file-scraper )")
		print("")
