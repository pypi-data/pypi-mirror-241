...
class DIC_Grid:
     ...
     def draw_marker_img(self,analysis_folder=None):
          """Draw marker image"""
          name = self.prepare_saved_file(prefix ='marker', extension='png', analysis_folder=analysis_folder)
          draw_opencv_v2(self.image, point=self.correlated_point, l_color=(0,0,255), p_color=(255,255,0),
     ...
     def obtain_strains(self, func=None)->dict:
        ...
        def portion_function(fraction:float = 0):
            """factury function that generates a function that takes a portion of the elemetns of an array 
...
class GridSize():
    ...
    def create_DIC_Grid(self) -> 'DIC_Grid':
        ...
        self.__newgrid = DIC_Grid(
             grid_x=self.grid_x, 
             grid_y=self.grid_y, 
             size_x=int(self.xnum), 
             size_y=int(self.ynum))
