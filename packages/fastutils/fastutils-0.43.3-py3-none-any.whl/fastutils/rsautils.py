...
def load_private_key(text, passphrase=None):
    ...
    ```text
        ...
        -----END RSA PRIVATE KEY-----
    ```
    但在有的程序中把“BEGIN RSA PRIVATE KEY”和“END RSA PRIVATE KEY”等封装格式移除掉，
    以形如“MIIEpAIBAAKCAQEAr7NZ....Bme5cOcZQ==”样的字符串格式保存。

    这类非标准格式的私钥默认是无法加载的。
    这里进行的特殊处理，支持标准和非标准格式的私钥加载。
    ...
    try:
        ...
        text = base64utils.decodebytes(sixutils.BYTES(text))
        sk = RSA.import_key(text, passphrase=passphrase)
    return sk
