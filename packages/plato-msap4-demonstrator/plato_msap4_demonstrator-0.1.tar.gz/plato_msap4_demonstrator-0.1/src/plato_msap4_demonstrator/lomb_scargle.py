...
def compute_uncertainty_smoothing (periods, power, filename=None) :
  ...
  smoothed = msap4.apply_smoothing (power, sizebox, win_type='triang')
  hmax = smoothed[index]
...
def compute_uncertainty_lomb_scargle_pymc (t, s, prot, ps_object,
                                           draw=100, tune=50, chains=2,
                                           cores=2) :
  '''
  Compute uncertainty on selected period in the Lomb-Scargle
  periodogram.
  
  The framework is Bayesian and is designed as follows: 
  ``astropy`` Lomb-Scargle periodogram provide parameters
  for the best sine-wave fit, then the distribution of these
  parameters is sampled thank to a PyMC3 framework exploring
  the sine-wave model on the light curve.
  '''
  f = 1 / (prot*86400)
  theta = ps_object.ls.model_parameters(f)
  model = pm.Model () 
  with model :
    # variables
    offset = pm.Normal("offset", mu=0, sigma=1)
    a1 = pm.Normal("a1", mu=theta[0], sigma=1)
    a2 = pm.Normal("a2", mu=theta[1], sigma=1)
    omega = pm.Normal ("omega", mu=2*np.pi*f, sigma=1)
    # sine-wave model
    sine_wave = offset + a1*np.sin (omega*t*86400) + a2*np.cos (omega*t*86400)
    # likelihood
    y_obs = pm.Normal("y_obs", mu=sine_wave, sigma=1, observed=s)
    # sampling the chain
    idata = pm.sample (draw, tune=tune, chains=chains, 
                       cores=cores, init='adapt_diag')
    #TODO 
    # I let this function as work in progress as it does
    # not seem computationnally efficient enough.
  return
...
def plot_ls (periods, ls, ax=None, lw=1,
  ...
  if param_profile is not None :
    ...
    model = np.ones (periods.size)
    prot = 1e6 / (param_profile[0,1]*86400)
    cond = (periods>max(0.1, 1/2*prot))&(periods<3*prot)
    model = model[cond] 
    x = 1e6 / (periods[cond]*86400)
    for ii in range (n_profile) :
      model = model * compute_model (param_profile[ii,:], x) 
...
def compute_model (param, x, profile=1, back=None) :
  if back is None :
    ...
    trend = 0
  if profile==0 :
    model = msap4.gauss (x, param[0], param[1], param[2])*back + trend
  elif profile==1 :
    model = msap4.lor (x, param[0], param[1], param[2])*back  + trend
  else :
...
def fit_gaussian_lomb_scargle (x, ps, x_init, back=None) :
  ...
  result = minimize (log_likelihood, np.log (param0),
                     args=(x, ps, back), bounds=np.log (bounds))

  fitted = np.exp (result.x)
  model = compute_model (fitted, x, back=back)
...
def compute_prot_err_gaussian_fit_chi2_distribution (periods, ps,
  ...
  cond = (periods>max(0.1, 1/2*p_init))&(periods<3*p_init)
  ps = ps[cond]
  aux_ps = np.copy (ps)
  ...
  while ii < n_profile and np.amax (ps) > threshold*max_init :
    ...
    ps = ps / model
    if back is not None :
      # setting the background again for next
      # iteration
      ps = ps * back
    x_init = 1e6/(periods[np.argmax (ps)]*86400)
...
def prepare_idp_fourier (ps_object, param, h_ps,
