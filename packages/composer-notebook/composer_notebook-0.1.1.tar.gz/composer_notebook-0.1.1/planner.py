...
def run_planner(profile="./test_profile.yaml"):
    user_profile = parse_user_profile(profile)
    planner_sys = f"""
    ...
    convo_stream = stream(
        ...
        log_last_msg_pretty("ASSISTANT", "yellow"),
        add_user_input(),
        log_last_msg_pretty("USER", "green"),
        condition(
            func=satisfy_msg(),
    ...
    if convo.error:
        ...
        prettify_log("SYSTEM", f"structured requirements set: \n {getattr(convo.memory.state, 'requirements', '')}", "cyan")
