...
class JdBaseModel():
    ...
    def apply_repeal_save(self, access_token, vender_id, transaction_id, card_num_list, sku_id, is_log=False, is_test=False):
        ...
        for cert in card_num_list:
            cert = CryptoHelper.aes_decrypt(cert, "17f72a775ec0a96ad52b662206c7bceb")
            ...
            if "jingdong_certList_query_responce" in resp:
                if resp['jingdong_certList_query_responce']['ydResponse']['success'] == False:
                    ...
                    if int(result['code']) != 200:
                        ...
                        data = result["data"]
                        if data["list"]:
                            password1 = "07afbc8bca08bc5139d36b3239ab3408"
                            password2 = "17f72a775ec0a96ad52b662206c7bceb"
                            for item in data["list"]:
                                item["cardNum"] = CryptoHelper.aes_decrypt(item["cardNum"], password1)
                                item["cardNum"] = CryptoHelper.aes_encrypt(item["cardNum"], password2)
                                item["cardPwd"] = CryptoHelper.aes_decrypt(item["cardPwd"], password1)
                                item["cardPwd"] = CryptoHelper.aes_encrypt(item["cardPwd"], password2)
