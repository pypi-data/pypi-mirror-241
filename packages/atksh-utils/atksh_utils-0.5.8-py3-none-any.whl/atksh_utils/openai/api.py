...
class OpenAI:
    ...
    def _create_text_message(self, role: str, content: str) -> ChatCompletionMessage:
        if role == "user":
    ...
    def call(
        ...
        if messages is None:
            messages = []
            messages.append(self._create_text_message("system", self.system_prompt))
            messages.append(self._create_text_message("user", user_prompt))
            if is_question:
        ...
        if (tool_calls := message.tool_calls) is not None:
            ...
            with TPE(max_workers=mp.cpu_count()) as exe:
                ...
                for tool_call in tool_calls:
                    if tool_call.type == "function":
                        ...
                        try:
                            ...
                            dist = [
                                hamming_distance(function_name, name)
                                + (1000 if function_name not in name else 0)
                        ...
                        if msg is None:
                            ...
                            if isinstance(function_call_args, str):
                                ...
                                msg += f"Please check the arguments: {json.dumps(func.info['parameters']['properties'])}.\n"
                            else:
                            ...
                            futures.append(
                                {"tool_call_id": tool_call.id, "role": "tool", "content": future}
                            )
                        else:
                            futures.append(
                ...
                for future in futures:
                    ...
                    print(content)
                    future["content"] = json.dumps(content)
                    print(future["content"])
                    tool_response_messages.append(future)
    ...
    def set_run_python_code_function(self):
        self.set_function(run_python_code)
