...
def iterations(test_cases: List[Dict], method: str, old_prompts_and_rating: List[str], new_number_of_prompts: int, functions: Dict=None, model_test: str='gpt-3.5-turbo', model_test_temperature: float=1.2, model_test_max_tokens: int=1000, model_generation: str='gpt-4-turbo', model_generation_temperature: float=0.6, model_generation_max_tokens: int=500, function_call: str='auto', description: str=None, best_prompts: int=2, model_embeddings: str=None, timeout: int=10, n_retries: int=5):
...
def concat_and_sort_lists(list1, list2, n):
    combined_list = list1 + list2

    sorted_list = sorted(combined_list, key=lambda x: x['rating'], reverse=True)
