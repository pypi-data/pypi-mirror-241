...
def tile_burst_to_xspectra(burst, geolocation_annotation, orbit, calibration, noise_range, noise_azimuth, tile_width, tile_overlap,
    ...
    for ii  in range(nbtiles):
        ...
        sigma0_BT, nesz_BT = compute_mean_sigma0_interp(DN, calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
        if not np.isfinite(sigma0_BT): # should only append in IW mode. Case when line are badly indexed in noise-range LUT
            sigma0_BT, nesz_BT = compute_mean_sigma0_closest(DN, burst['linesPerBurst'], calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
        ...
        if water_only: # Bright target mask is applied only on water 
            ...
            # ---------------- part of the variables to be added to the final dataset ----------------------
            variables_list+=[sigma0.to_dataset(), nesz.to_dataset(), nv.to_dataset()]
