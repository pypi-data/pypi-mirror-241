...
def tile_bursts_overlap_to_xspectra(burst0, burst1, geolocation_annotation, calibration, noise_range, noise_azimuth, tile_width, tile_overlap,
    ...
    # for i in xndindex(tiles_sizes):
        ...
        if water_only: # Bright target mask is applied only on water 
            ...
            # ------------- mean sigma0 and nesz ------------
            sigma0_f, nesz_f = compute_mean_sigma0_interp(DN0, calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
            ...
            if not np.isfinite(sigma0_f): # should only append in IW mode. Case when line are badly indexed in noise-range LUT
                sigma0_f, nesz_f = compute_mean_sigma0_closest(DN0, burst['linesPerBurst'], calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
            ...
            nesz_f.attrs.update({'long_name':nesz_f.long_name+' with BT correction'})
            nv_f.attrs.update({'long_name':nv_f.long_name+' with BT correction'})

            # ---------------- part of the variables to be added to the final dataset ----------------------
            variables_list+=[sigma0_f.to_dataset(name=sigma0_f.name+'_filt'), nesz_f.to_dataset(name=nesz_f.name+'_filt'), nv_f.to_dataset(name=nv_f.name+'_filt'), sigma0_f_overlap.to_dataset(name=sigma0_f_overlap.name+'_filt')]
