...
class CustomRecast(BaseTextRecast):
    """Recast text data by removing special custom characters.
    ...
    --------
    >>> # process='remove'
    >>> from swachhdata.text import CustomRecast
    >>> text = 'You can have a look at our   catalogue  at \www.samplewebsite.com in the services tab'
    >>> rec = CustomRecast(characters={'  ': ' ', '   ': ' ', '\\': ''}, regex={r'\b\d+(\.\d+)?%': ''})
    >>> rec.setup(text)
    >>> rec.recast()
    'You can have a look at our catalogue at www.samplewebsite.com in the services tab, 50% off'
    >>> # OR
    >>> rec.setup_recast(text)
    'You can have a look at our catalogue at www.samplewebsite.com in the services tab, off'
    >>> 
    """

    def __init__(self, characters=None, regex=None, verbose=0):

        super().__init__(verbose=verbose)
        self.characters = characters
        self.regex = regex
        self._name = 'CustomRecast'
    ...
    def __base_recast(self, text):
        ...
        if self.regex is not None:
            for r, replace in self.regex.items():
                regex_pattern = re.compile(r)
                text = re.sub(regex_pattern, replace, text)
        return text
