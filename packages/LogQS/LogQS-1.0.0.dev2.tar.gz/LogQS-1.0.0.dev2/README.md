...
The project uses Python 3.10, which may require some dependencies to be required.  In one go,

    sudo apt install python3.10 python3.10-dev python3.10-distutils python3.10-venv

Note that you may need to add the deadsnakes PPA to your system before being able to install Python 3.10:
...
6. Optionally, run the dev API in standalone mode (no DSM):

        lqs.api run --dev --enable-dsm=False
