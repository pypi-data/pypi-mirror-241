...
def heatmap(X:np.ndarray, Y:np.ndarray, Q:np.ndarray, combine=None, combine_method='sum', imgpath=None, resolution=512,
            offset=0.1, levels=None, cache=None)->str:
    ...
    #                     extend='both')
    if levels is None:
        info = plt.contourf(gridx, gridy, z, levels=np.percentile(z.flatten(), np.linspace(100/8, 700/8, num=8)), cmap='rainbow',
                            extend='both')
    else:
        info = plt.contourf(gridx, gridy, z, levels=levels, cmap='rainbow', extend='both')
    # plt.colorbar(info)
    # plt.show()
    plt.axis('off')
    if imgpath is None:
    ...
    plt.close()
    return imgpath, [xstart, ystart], [xend, yend], info.levels.tolist()