...
class FriendlyCaptchaClient:
    ...
    def _create_friendly_response_with_error(raw_response, default_error_detail):
        ...
        error_detail = raw_response.get('error', {}).get('details', str(default_error_detail))
    ...
    def _is_client_error(error: Union[Error, None]):
        return error is not None and error.error_code in NON_STRICT_ERROR_CODES # TODO: this could be O(1)
    ...
    def _get_current_version():
        ...
        try:
            ...
            my_version = pkg_resources.get_distribution('friendly-captcha-client').version
        except Exception:
            pass
        return my_version
    ...
    def _process_response(self, response) -> (FriendlyCaptchaResponse, int):
        ...
        try:
            ...
            friendly_response = self._create_friendly_response_with_error(response.json(), e)
    ...
    def _is_loose_verification_available(self, status_code: int, error: Union[Error, None]):
        ...
        """
        return error is None or (not self.strict and self._is_error_loose(error, status_code))

    def _is_error_loose(self, error, status_code):
        # known error where we allow loose verification
        if (any(error.error_code == _error.value for _error in self._non_strict_error_code) or \
                # unknown errors where we allow loose verification
                all(error.error_code != _error.value for _error in DefaultErrorCodes) and status_code in [200, 500]):
            return True
        return False
    ...
    def _handle_api_response(self, response: requests.request) -> FriendlyCaptchaResult:
        ...
        if was_able_to_verify and friendly_response.error is not None:
            if friendly_response.error.error_code == DECODE_RESPONSE_FAILED_INTERNAL_ERROR_CODE:
        ...
        friendly_result = FriendlyCaptchaResult(
            should_accept=self._is_loose_verification_available(status_code, friendly_response.error),
    ...
    def verify_captcha_response(self, captcha_response: str, timeout: int = 10) -> FriendlyCaptchaResult:
