...
from vpnetbox.types_ import LStr, LDAny, DDDLInt, LValue, LParam, T2Str
from vpnetbox.types_ import LTInt2, DAny, TValues, TLists, T3Str
...
def slice_params_d(url: str, max_len: int, key: str, params_d: DAny) -> LDAny:
...
def slice_params_ld(url: str, max_len: int, keys: LStr, params: LDAny) -> LDAny:
    ...
    for params_d in params:
        ...
        # need slice
        params_sliced: LDAny = slice_params_d(
            url=url,
            max_len=max_len,
...
def get_key_of_longest_value(params_d: DAny) -> str:
