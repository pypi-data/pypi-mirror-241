...
{% for i in range(1, NofSets+1) %}
{
    ...
    Entity EdgesOf[
                DOM_insulationSurface_<<i>>,
                Not {DOM_insulationBoundaryCurves, DOM_insulationSurface_<<i-1>>}
                ];
    {% elif type == "thermal" %}
    Entity NodesOf[
                DOM_insulationSurface_<<i>>,
...
{% if dm.magnet.geometry.ii.tsa %}
    ...
    DOM_insulationBoundaryCurves = Region[{ <<rm.insulator.curve.numbers|join(', ')>> }];
{% else %}
...
{% endif %}
    ...
    DOM_airPoints = Region[{ <<rm.air.point.number>> }];
...
{% if dm.magnet.solve.ii.resistivity == "perfectlyInsulating" %}
    DOM_airCut = Region[{ <<rm.air.cochain.numbers[1]>> }];
{% else %}
    DOM_airCut = Region[{ <<rm.air.cochain.numbers[0]>> }];
{% endif %}
...
{% else %}
                Support DOM_total;
                Entity NodesOf[{DOM_Phi}, Not DOM_insulationBoundaryCurves];
{% endif %}
{% else %}
                Support DOM_total;
...
{% else %}
                Support DOM_total;
{% endif %}
                Entity GroupsOfEdgesOf[DOM_airCut];
            }
{% if dm.magnet.geometry.ii.tsa and dm.magnet.solve.ii.resistivity != "perfectlyInsulating" %}
            ...
            // TSA contribution on boundary of the thin layer
            {
                ...
                Entity NodesOf[DOM_insulationBoundaryCurves];
            }
        }
        SubSpace{
            <<FUNCTIONSPACE_TSASubSpaces(["BASISFUN_sc", "BASISFUN_gradpsinBnd"])|indent(12)>>
        }
{% else %}
        }
{% endif %}
...
{% for tolerance in (dm.magnet.solve.nls.postOperationTolerances + dm.magnet.solve.t.adaptive.postOperationTolerances)|unique(attribute="quantity") %}
    ...
    {% elif tolerance.quantity in ["voltageBetweenTerminals", "currentThroughCoil"] %}
    {
        ...
        LastTimeStepOnly 1;
        Operation {
            Print[RESULT_<<tolerance.quantity>>, OnRegion DOM_airCut];
        }
    }
    {% else %}
    {
