...
class regions:
    ...
    def addEntities(
        ...
        if regionType is regionType.powered:
            ...
            if entityType is entityType.cochain:
...
class Mesh(Base):
    ...
    def generate_mesh(self):
        ...
        if self.mesh.ai.structured:
            ...
            removedAirVolumeDimTags.extend(airInnerCylinderVolumeDimTags)
        ...
        if self.geo.ii.tsa:
            ...
            # Add the contact layer surfaces to the cohomology request:
        ...
        cutEntities = []
        for tag in cutTags:
            cutEntities.extend(list(gmsh.model.getEntitiesForPhysicalGroup(1, tag)))
        cutEntities = [(1, tag) for tag in cutEntities]
        self.dimTags[self.geo.ai.cutName] = [cutEntities[0]]
        self.dimTags["CutForPerfectContactLayer"] = [cutEntities[1]]

        # Remove newly created physical groups because they will be created again in
        # generate_regions method.
        gmsh.model.removePhysicalGroups(
            [dummyContactLayerRegionDimTag] + [dummyAirRegionDimTag] + cuts
        )
    ...
    def generate_regions(self):
        ...
        self.regions.addEntities(
            "AirPoints",
            [outerAirPointTag, innerAirPointTag],
            regionType.air,
            entityType.point,
        )
        ...
        if "CutForPerfectContactLayer" in self.dimTags:
            cutTags = [dimTag[1] for dimTag in self.dimTags["CutForPerfectContactLayer"]]
            self.regions.addEntities(
                "CutForPerfectContactLayer", cutTags, regionType.air, entityType.cochain
            )
        # ==============================================================================
        # CUTS ENDS ====================================================================
        # ==============================================================================
        ...
        Pancake3DBoundaryDimTags = list(
            set(Pancake3DAllBoundary) - set(topSurfaceDimTags) - set(bottomSurfaceDimTags)
        )
