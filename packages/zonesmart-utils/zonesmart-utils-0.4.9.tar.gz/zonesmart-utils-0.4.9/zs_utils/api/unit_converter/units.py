...
class UnitPrefix(object):
    ...
    def __eq__(self, other_prefix):
        return (
            self.symbol == other_prefix.symbol and self.name == other_prefix.name and self.factor == other_prefix.factor
        )
...
class Unit(object):
    def __init__(
        self, symbol, name, plural_name=None, L=0, M=0, T=0, I=0, THETA=0, N=0, J=0, coef=D("1"), offset=D("0")
    ):
        self.symbol = symbol
        self.name = name
    ...
    def is_same_dimension(self, other_unit):
        return (
            self.L == other_unit.L
            and self.M == other_unit.M
            and self.T == other_unit.T
            and self.I == other_unit.I
            and self.THETA == other_unit.THETA
            and self.N == other_unit.N
            and self.J == other_unit.J
        )

    def __eq__(self, other):
        return self.is_same_dimension(other) and self.coef == other.coef and self.offset == other.offset
    ...
    def __pow__(self, power):
        if type(power) in (int, float, D):
            ...
            final_unit = self.__class__(
                symbol=self.symbol + "^" + str(power),  # TODO: attention manque des parenthèses etc..
